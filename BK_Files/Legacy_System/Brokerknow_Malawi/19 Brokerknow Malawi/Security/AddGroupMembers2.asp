...
<title>Add Group Member/s</title>
...
<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/webparts.css">
 ...
 <script language='javascript'>
		function forceSubmit()
		{
			...
			//setOpener();
			//document.frmAddGroupMembers.method='post';
			document.frmAddGroupMembers.target='_self';
			document.frmAddGroupMembers.submit();		
		}		
		function setOpener()
		{
...
<%
   ...
   Const dataMark = "~"
	...
	'response.write "<br>" & action
	Set conn = GetActiveConnection("KBroker")
	Set GroupRs = Conn.Execute("SELECT * FROM Groups ORDER BY GroupName")    
	
	If action = "EXECUTE" then
		Set StaffRs = Conn.Execute("SELECT * FROM Users")
		'response.write "<br> Hapa" & ""
		If Not (StaffRs.EOF OR StaffRs.BOF) Then
			Do Until StaffRs.EOF
				thisStaffID = trim(StaffRs("UserID"))
				hasRights = False
				'response.write "<br> Hapa2" & ""
				If Not (groupRs.EOF Or groupRs.BOF) Then
					Do Until groupRs.EOF   
		      		
			      		groupID = trim(groupRs("GroupID"))
						
						withRights = Request.Form("RightsMove" & groupID)
						'response.write "<br> --" & withRights & "::::" & DataMark & thisStaffID & DataMark & "<Br>"
						If withRights <> "" Then
						...
						If hasRights = True Then
							...
							Else
								...
								'sqlstr="<br><br>INSERT INTO UserGroups (GroupID, UserID) VALUES (" & groupID & ", " & thisStaffID & ")"
								'response.write sqlstr
								Conn.Execute ("INSERT INTO UserGroups (GroupID, UserID) VALUES (" & groupID & ", " & thisStaffID & ")")
							End If	
						Else
							'sqlstr="<br><br>DELETE FROM UserGroups WHERE GroupID = " & groupID & " AND UserID = " & thisStaffID 
...
<form name = 'frmAddGroupMembers' method = 'post' action = 'AddGroupMembers.asp'>
...
<table border="0" cellspacing="1" cellpadding="2" id="mainTable">
	...
	If Not (GroupRs.EOF Or GroupRs.BOF) Then
		...
		Do Until GroupRs.EOF
			...
			<tr>
			<td width="100%"><input type="button" value=" &lt; " name="Move<%= groupID %>" Class=Buttons OnClick="JavaScript: Move(this)">&nbsp;&nbsp;&nbsp;<input type="button" value=" &gt; " name="Move<%= groupID %>" Class=Buttons OnClick="JavaScript: Move(this)"></td>
			<td width="100%">&nbsp;&nbsp;
			<select size="2" name="RightsMove<%= groupID %>" multiple  OnKeyPress="JavaScript: if (event.keyCode==46) Move(this)">
			<%
	...
	If Not (Users.BOF Or Users.EOF) Then%>
		<select size="1" name="Users" multiple>
  ...
  <tr>
   <td width="100%" colspan="3" align=right>
		...
		<input type = 'button' Class=Buttons name ='cmdAdd' id = 'cmdAdd' value=" Save "  OnClick="vbscript:SelectForm">
...
<Script Language="JavaScript">
<!--Begin
	document.all.item("Users").size = Math.round(document.all.item("userTD").clientHeight / 10) ;
...
<Script Language="VBScript">
	...
	Function SelectForm
		For Each Thing In frmAddGroupMembers
