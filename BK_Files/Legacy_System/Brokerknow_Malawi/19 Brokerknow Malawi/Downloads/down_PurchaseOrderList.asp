<%
	Const report_ViewName = "PurchaseOrderList"
	Const reportPage = "down_PurchaseOrderList.asp"
	Const headerColCount = 3
	Const groupingHeaderCol = 0
	Const reportTitle = "Purchase Orders"
%>
<html>
<head>
...
Dim fso, txtLine, txtStream, txtStreamOut, filename
			...
			filename = "PurchaseOrder_" & FormatDate(date) & ".csv"
...
<table border="0" cellspacing="2" cellpadding="2" style="font-family: Arial Narrow" width="100%">
    <tr>
		...
		<%txtLine=txtLine & reportTitle & chr(13) & chr(10) %>
...
<table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="100%">
    <tr>
      ...
      <td width="42%"><%= FormatDate(Date) %></td>
	  <%txtLine=txtLine & "Date: " & FormatDate(Date) &  chr(13) & chr(10) %>
    </tr>
	<%For i = 1 To headerColCount%>
			<tr>
			  <td ><b><%= Rs.Fields(i - 1).Name %>:</b></td>
			  <td ><%= Rs.Fields(i - 1).Value %></td>
			</tr>
			<%txtLine=txtLine & Rs.Fields(i - 1).Name  & ":" & Rs.Fields(i - 1).Value & "," &  chr(13) & chr(10)%>
	<%	next
...
Loop
	...
	conn.execute ("DELETE FROM down_File WHERE     (Filename = '" & filename & "') or (cast(floor(cast(TimeCreated as float)) as datetime) < cast(floor(cast(getdate() as float)) as datetime))")
		...
		sqlStr=sqlStr & " SELECT '" & filename & "' AS Filename, '" & "Purchase Order Schedule" & "' AS Report, " & Session("UserID") & " AS CreatedBy,  "
...
<SCRIPT Language="JavaScript">
	alert ("Purchase order schedule generated!");
