...
<title>Traded Levies (Commissions)</title>
...
If genReport <> "1" Or Not IsDate(selectedFromDate)  Or Not IsDate(selectedToDate)Then%>
	...
	<form method="POST" action="TradedLeviesCommission.asp" Name="frmMain" id="frmMain">
...
headerDescription = formatdate(selectedFromDate) & " to " & formatdate(selectedToDate) %>
	...
	<%
		...
		filename = "TradedLeviesCommission_" & FormatDate(selectedFromDate) & ".csv"

		Set fso = Server.CreateObject("Scripting.FileSystemObject")
		Set txtStreamOut = fso.CreateTextFile(SERVER.MAPPATH(".") & "\" &  filename,true)
...
<table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="100%">
     <tr>
		<td nowrap><b><font face="Arial Narrow" size="4">Traded Levies (Commissions)</font></b></td>
		<%txtLine="Traded Levies (Commissions)" & chr(13) & chr(10) %>
    ...
    <tr>
       ...
       <%txtLine="for the period" & Replace(headerDescription,"," ," ")& chr(13) & chr(10) %>
	</tr>
    ...
    <tr bgColor="#000000">
	  ...
	  <%txtLine=txtLine & "Contract"%>
      <td nowrap><b><font color="#FFFFFF">Slip</font></b></td>
	  <%txtLine=txtLine & "Slip"%>
      ...
      <%
		...
		TrNox = 0 
		procstr= "ContractLeviesCrossTabCommission '"& selectedFromDate & "', '" & selectedToDate & "'" 
		...
		for i = beginLeviesCol to rs.fields.count - 2
				...
				IF (trim(lcase(rs.fields(i).name)) = "commission") or (trim(lcase(rs.fields(i).name)) = "agent") then
					'ignore all levies except commission or Agent
					...
					IF (trim(lcase(rs.fields(i).name)) = "transfer") AND (skipTransfer = true) then
							...
							%>
								<td width="60px" nowrap align="right"><b><font color="#FFFFFF"><%=rs.fields(i).name%></font></b></td>	<%txtLine=txtLine & "," & rs.fields(i).name%>	
							<%
							'i = i + 1
							Icount =Icount+1
					end if
      ...
      <td style="BORDER-RIGHT: #C0C0C0 1px inset; BACKGROUND-COLOR: transparent">&nbsp;</td>
           <%	levyTotals = 0
				...
				for i = beginLeviesCol to rs.fields.count - 2
						...
						End If
						IF (trim(lcase(rs.fields(i).name)) = "commission") or (trim(lcase(rs.fields(i).name)) = "agent") then
							'ignore tall other levies
						
							IF (trim(lcase(rs.fields(i).name)) = "transfer") AND (skipTransfer = true) then
									'do nonthing
							else
									
									if trim(lcase(rs.fields(i).name)) = "agent"  then
									%>
				...
				dailyTotalsArray(ICount) = dailyTotalsArray(ICount) + levyTotals
			 ...
			 <td nowrap align="right"><%'If instr(1,rs.Fields("ContractNumber"),"S") > 0 Then
						'netAmt = grossAmt - levyTotals 						
				  ' Else
						netAmt = levyTotals 						
				   'End If 
