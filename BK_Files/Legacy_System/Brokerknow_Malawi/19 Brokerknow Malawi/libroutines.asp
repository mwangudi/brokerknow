...
Function WriteDialogRelocateScript2(pageTo)
	With Response
		.write "<Script Language=""JavaScript"">" & vbCrLf
		.write " alert(); " & vbCrLf	
		.write "              window.opener.location= window.opener.location;" & vbCrLf	
		'.write " alert(); " & vbCrLf	
		.write "		this.location='" & pageTo & "';" & vbCrLf
		'.write " alert(); " & vbCrLf
		.write "	}" & vbCrLf
		'.write "	catch(e){window.parent.closeDocOpener();}	" & vbCrLf
        .write "</Script>" & vbCrLf
...
Function WriteDialogRelocateScriptNew(pageTo)
	With Response
		.write "<SCRIPT LANGUAGE='JAVASCRIPT'>	"	& vbCrLf			
		.write "		this.location="& pageTo &";"& vbCrLf
		.write "</SCRIPT>"& vbCrLf
...
Function WritefraEnabledDialogCloseScript
	With Response
		...
		'.write "				alert('Location identified 1');" & vbCrLf
		.write "				try" & vbCrLf
		.write "				{" & vbCrLf
		.write "					window.parent.parent.frames[""maininfo""].location.href=window.parent.parent.frames[""maininfo""].location.href;" & vbCrLf
		'.write "				alert('Location identified 2');" & vbCrLf
		.write "				}" & vbCrLf
		.write "				catch(err)" & vbCrLf
		.write "				{" & vbCrLf
		.write "					//alert(err.description);" & vbCrLf
		.write "				}" & vbCrLf
...
Function WritefraEnabledDialogCloseScript2()
	With Response
		.write "<script language=""javascript"">" & vbCrLf		
		.write "window.opener.location= window.opener.location;" & vbCrLf	
...
With Response
		.write "<script language='javascript'>" & vbCrLf
		'.write "			alert();" & vbCrLf						
		.write "			window.location=window.location+""?id=" & ID & """;" & vbCrLf			
		.write "		</script>" & vbCrLf	
End With
end Function
...
With Response
		.write "<script language='javascript'>" & vbCrLf	
		.write "			window.self.location=window.location;" & vbCrLf						
		.write "		</script>" & vbCrLf	
End With
...
Function FormatDateandTime(theDate)
	On Error Resume Next
	timePart = Timevalue(theDate)
	
	FormatDateandTime = Day(theDate) & "-" & MonthName(Month(theDate), True) & "-" & Year(theDate) & " " & timePart
	If Err.Number > 0 Then
		FormatDateandTime = theDate
	End If
End Function


Function FormatDate1(theDate)
	On Error Resume Next
	datevar= split(rs.Fields("tradeDate"),"/")
			correctDt =datevar(1) &"-" & datevar(0) &"-"& datevar(2)
			FormatDate1= formatdate(correctDt)
	If Err.Number > 0 Then
		FormatDate1 = theDate
	End If
End Function
...
Function Paging(ByVal intPage, ByVal intPageCount, ByVal intRecordCount)
	...
	strRet = strRet & "		try{grid.setSelectionValues([myRowIDs[0]]);ItemSelected(myRowIDs[0]);} catch(e){} " & Chr(13)
...
Function UpdateAuditTrail(sqlStr)
  ...
  'Response.End
    ...
    auditSQLStr =  "INSERT INTO AuditTrail (UserID, AuditTrailAction, AuditTrailTable, AuditTrailSQL, AuditTrailCriteria,AuditTrailmoment) " & _
				" SELECT '" & Session("UserID") & "', '" & sqlParse.SQLType & "', '" & sqlParse.TableName  & "', '" & Replace (sqlStr, "'", "''") & "', '" & Replace (sqlParse.Criteria, "'", "''") & "',GetDate()"
...
Function DrawPageFunctions2(includePrint, includeEmailThisPage, includeBackBtn, PathTo)
	Dim clientSideScript
	Dim sName, sPort, sPath, thisPath
...
function holiday(todaysdate)
  'dim sqlstr
  dim thisrs
		'sqlStr = "SELECT * FROM [BrokerList] Order By BrokerCode"
		sqlStr3 = "SELECT Holiday AS holiday FROM Holidays " & _
                  " WHERE (Holiday ='" & todaysdate & "') OR " & _
                  " (DATEPART(weekday, '" & todaysdate & "') = 1) OR " & _
                  " (DATEPART(weekday, '" & todaysdate & "') = 7)"           
        ...
        holiday=holiday(todaysdate)
        else
			if(NotHoliday=1) then
			HolidayFailed=1
			else        
			if(holiday1(todaysdate-1)=1 and Cint(thisholiday)<>1) then
				thisholiday=1			
				todaysdate=todaysdate-1     
				holiday=holiday(todaysdate)        
				else
				holiday=todaysdate-1
				end if  
			end if
			end if
        if(Cint(thisholiday)=1) then
        holiday=todaysdate
        else
		holiday=todaysdate-1	              
        end if
        
        if(Cint(HolidayFailed)) then
        holiday=todaysdate
        end if
  end function
  function holiday1(todaysdate)
  'dim sqlstr
  dim thisrs
		'sqlStr = "SELECT * FROM [BrokerList] Order By BrokerCode"
		sqlStr3 = "SELECT Holiday AS holiday FROM Holidays " & _
