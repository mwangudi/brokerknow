...
<%
	...
	Dim BatchedItems
	Dim BatchNull
	sqlStr = "SELECT * FROM ForwardsList WHERE Offering_DPA_ = " & Request("ID")
	...
	Set conn = Nothing
		'WriteDialogRelocateScript "EditBatchedForward.asp?ID=" & ID	WriteDialogCloseScript	
	Response.End	
	end if	
end if
...
<form name = 'frmBatched' method = 'post' action = 'EditBatchedForward.asp' id='frmMain' >
...
<%
		...
		sqlStr = "SELECT * FROM ForwardsList WHERE Batch_No = " & BatchID & " AND Offering = "& SecurityDPA &" ORDER BY BatchSeq ASC"
		...
		if(SearchSerial<>"") then
		  sqlStr = "SELECT * FROM ForwardsList where Batch_no is null AND (PAL_No like '%" & SearchSerial & "%') AND ClosingDate > cast(Floor(cast(GetDate() as Float)) as datetime) order by pal_no asc"
		else
		  sqlStr = "SELECT * FROM ForwardsList where Batch_no is null AND ClosingDate >= cast(Floor(cast(GetDate() as Float)) as datetime) order by pal_no asc"
		end if	
