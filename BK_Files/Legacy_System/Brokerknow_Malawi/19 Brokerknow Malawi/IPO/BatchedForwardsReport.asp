...
SqlStr = "SELECT ForwardsList.*, ISNULL(Client.ClientAddr, '') AS ClientAddr, ISNULL(Residency.ResidencyDescription, '') AS Residence, ISNULL(Class.ClassDescription, " & _
	...
	" Client ON ForwardsList.Client_DPA_ = Client.Client_DPA_ LEFT OUTER JOIN " & _
	...
	" WHERE (ForwardsList.Offering = "& Offering &") AND (ForwardsList.Batch_No = "& selectedBatch &") " & _
	" ORDER BY CAST(FLOOR(CAST(ForwardsList.Offerings_Date AS float)) AS datetime), RTRIM(LTRIM(ForwardsList.ClientName))"	
...
<table border="0" cellspacing="2" cellpadding="2" style="font-family: Arial;" width="95%" align=center>
	...
	<br>
	<tr>
		<td width="100%" nowrap align="left"><font face="Impact" size="4">BATCHED FORWARDS</td>      
	</tr>
	<tr>
