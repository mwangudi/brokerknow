...
If action = "EXECUTE" Then
	...
	clientcode = Request.Form("txtClientCode")
...
END IF
	...
	SqlStr = "SELECT MAX(ISNULL(Offerings.Batch_No, 0)) AS BatchNo FROM Offerings INNER JOIN Security ON Offerings.Offering = Security.Security_DPA_" & _
		" WHERE (Offerings.ChangedBy = "& UserID &") AND (Offerings.Offering = "& offering &") AND Offerings.Deleted = 0 and Offerings.Forward<>1 AND (CAST(FLOOR(CAST(Offerings.Offerings_Date AS float)) AS DateTime) = '" & FormatDate(Date) &  "' )  "
	...
	If Not (Rst.EOF Or Rst.BOF) Then
		...
		If IsNull(Rst("BatchNo")) or len(trim(Rst("BatchNo") ))= 0 Then
			theBatchNo = 0
		Else
			theBatchNo = Rst("BatchNo")
		End If
		'Response.write theBatchNo :response.end
	...
	Else
		SqlStr = "SELECT MAX(ISNULL(Offerings.Batch_No, 0)) AS BatchNo FROM Offerings INNER JOIN Security ON Offerings.Offering = Security.Security_DPA_" & _
			" WHERE (Offerings.ChangedBy = "& UserID &") AND (Offerings.Offering = "& offering &") AND Offerings.Deleted = 0 and Offerings.Forward<>1"
	...
	conn.BeginTrans		
		sqlStr = " INSERT INTO Offerings (PAL_No,Client_DPA_, " & _
				 ...
				 " Additional,Batch_No,DateCreated,CreatedBy,ReceivingBroker,PaymentType,status) " & _
				 " VALUES ('" & palno & "'," & ClientName & "," & offering & "," & Price & "," & AlRights & "," & UserId & _
				 "," & Accept & "," & AddNew & "," & BatchNo & ",GetDate(),"& UserID &","& Broker &","& PaymentType &",1)"
			
			'Response.Write sqlStr
			'Response.End 
		
			conn.Execute sqlStr
			sqlStr = "UPDATE ClientBalances SET CurrentBal = CurrentBal - " & payable & " WHERE Client_DPA_ = " & clientcode 
		    conn.Execute(SqlStr)
	conn.CommitTrans
	conn.Close
       ...
       <tr>
			...
			<td width="80%" nowrap>
				<select name = 'cboPaymentType' id = 'cboPaymentType' size="1">
					<option  selected value = "2">GGS</option>
