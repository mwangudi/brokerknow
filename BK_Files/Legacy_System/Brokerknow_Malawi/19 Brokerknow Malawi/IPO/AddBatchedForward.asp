...
<%
	...
	if (action = "Save") then
		if instr(1,buttonAction,"SEARCH") > 0 then
			...
			WriteDialogRelocateScript "AddBatchedForward.asp?SearchSerial=" & SearchSerial
		else
				...
				For i=0 to UBound(itemids)			
					...
					IF THISISENABLED THEN
						...
						SqlStr = "SELECT MAX(ISNULL(Offerings.Batch_No, 0)) AS BatchNo FROM Offerings INNER JOIN Security ON Offerings.Offering = Security.Security_DPA_" & _
							" WHERE (Offerings.Offering = "& offering &") AND Deleted = 0 AND Forward=1"
						...
						If Not (Rst.EOF Or Rst.BOF) Then
							...
							If theBatchNo = 0 Then
								...
								Else
									SqlStr = "SELECT MAX(ISNULL(Offerings.Batch_No, 0)) AS BatchNo FROM Offerings INNER JOIN Security ON Offerings.Offering = Security.Security_DPA_" & _
									" WHERE (Offerings.Offering = "& offering &") AND Deleted = 0 AND"
									Set Rst3 = conn.Execute(SqlStr)
							...
							Else
								...
								" WHERE (Offerings.CreatedBy = "& UserID &") AND (Offerings.Offering = "& offering &") AND Offerings.Batch_No = "& theBatchNo &" AND Deleted = 0 AND Forward=1" & _
...
<head>
<title>Add Batched Forward</title>
...
<form name = 'frmBatch' method = 'post' action = 'AddBatchedForward.asp' id='frmMain' >
	<br>
	...
	<table align=center border="0" cellspacing="1" cellpadding="1" width="98%">
		<tr>
			...
			<td width="80%" nowrap>
				<select name = 'cboOfferings' id = 'cboOfferings' size="1" onChange="window.location.href='AddBatchedForward.asp?Offering='+this.value">
				<% 
	...
	<%if Offering <> "" then
		if(SearchSerial<>"") then
			sqlStr = "SELECT * FROM ForwardsList WHERE Forward = 1 AND Offering = "& Offering &" AND Batch_no is null AND (PAL_No like '%" & SearchSerial & "%') AND ClosingDate > cast(Floor(cast(GetDate() as Float)) as datetime) order by pal_no asc"
		else
			sqlStr = "SELECT * FROM ForwardsList WHERE Forward = 1 AND Offering = "& Offering &" AND Batch_no is null AND ClosingDate >= cast(Floor(cast(GetDate() as Float)) as datetime) order by pal_no asc"
		end if	
	...
	If Offering = "" Then%>
	<script language="javascript">
		window.location.href = 'AddBatchedForward.asp?Offering='+ document.all.item("cboOfferings").options[document.all.item("cboOfferings").selectedIndex].value;
	</script>
	<%End If%>
