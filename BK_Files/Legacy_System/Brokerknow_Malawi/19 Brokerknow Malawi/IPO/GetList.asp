...
Select Case LCase(Trim(whatToDo))
	...
	Case "confirmserial"
	    Response.Write "<!--ENDPAGEFUNCS-->" &  ConfirmSerial	
	Case ""

End Select 

function ConfirmSerial
	SerialNo =  trim(Request.QueryString("SerialNo"))
	Offering = Request.QueryString("Offering")

	SqlStr = "Select Client_DPA_,Offering_DPA_ from Offerings Where ltrim(rtrim(PAL_No)) = '" & SerialNo & "'" & _
			  " and Offering = " & Offering & " and Deleted=0"
	

	'Response.Write SqlStr
	'Response.End

	Set rsAccount = conn.Execute(sqlStr)	
	If Not (rsAccount.EOF and rsAccount.BOF) Then
		Found = 1
		ClientCode = rsAccount("Client_DPA_")
	else
		Found = 0
		ClientCode = 0
	end if
	ConfirmSerial = Found & "<->" & ClientCode
End Function
