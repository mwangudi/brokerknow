<html>
<head>
<title>Edit IPO Application</title>
...
If action = "EXECUTE" Then
	...
	'clientName = Request.Form("cboClient")'REFF
...
END IF
	...
	If Accept = "" Then Accept = 1
		...
		AlRights=replace(AlRights,",","") 
		addnew=replace(addnew,",","") 
	...
	conn.BeginTrans	
	     ...
	     '----------------------------------------------------------------------------------------------------------------		
		   ...
		   'Delete application...
		   '--------------------------------------------------------------
			'SQL="UPDATE Offerings SET  Deleted = 1, status = 0  WHERE (Offering_DPA_ = " & ID &")"
			'conn.Execute(SQL)
		...
		'Application Successfully Deleted
		   ...
		   '---------------------------------------------------------------	
               ...
               '--------------------------------------------------------------------
				if maintain = 1 then				
					sqlStr = " INSERT INTO Offerings (PAL_No,Client_DPA_, " & _
					         ...
					         " Additional,Batch_No,Forward,DateCreated,"& _
					         " CreatedBy,ReceivingBroker,status,PaymentType) " & _
					 " VALUES ('" & replace(palno,"'","''") & "'," & clientcodes  & "," & offering & "," & Price & "," & AlRights & _
					 "," & UserId & "," & Accept & "," & AddNew & "," & batchNo & ",0,GetDate(),"& UserID & _
					 ","& broker &","& status &","& PaymentType &")"
				else
					sqlStr = " INSERT INTO Offerings (PAL_No,Client_DPA_, " & _
					         " Offering,Offering_Price,Alloted_Rights,ChangedBy,AcceptanceType," & _
					         " Additional,Batch_No,Forward,DateCreated,"& _
					         " CreatedBy,ReceivingBroker,status,PaymentType) " & _
					 " VALUES ('" & replace(palno,"'","''") & "'," & clientcodes  & "," & offering & "," & Price & "," & AlRights & _
					 "," & UserId & "," & Accept & "," & AddNew & "," & NextBatch & ",0,GetDate(),"& UserID & _
					 ","& broker &","& status &","& PaymentType &")"
				end if
			...
			conn.Execute "ClientBalanceProcedure " & clientcodes
			'conn.Execute "ClientTotalProcedure " & clientcodes
...
If Not (rs.EOF Or rs.BOF) Then
		...
		availcredit = rs("AvailableCredit")
		rs.MoveNext
End If

If THISISNOTINUSE = TRUE then		
...
<form name = 'frmAddSecurity' method = 'post' id="frmMain" action = "EditOfferingN.asp" >
	<table border="0" width="80%" cellpadding=2 cellspacing=2>
		<tr>
			...
			<td width="80%" nowrap><input type="text" name="txtPalNo" id="txtPalNo" size="25" value="<%=palno%>" onblur="ConfirmSerial(this);" maxlength="15"></td>
		...
		<tr>
			...
			<td width="80%" nowrap>
				<input type = 'text'CLASS="READONLY" name ='txtClientCode' id = 'txtClientCode' value="<%=client%>" size="10">&nbsp;
			...
			<td width="80%" nowrap>
				<table>
					...
					<tr>
						...
						<td><input type = 'text' name ='txtAvailableCredit' id = 'txtAvailableCredit' readonly class="readonly" size="15" value="<%=FormatNumber(availcredit,2)%>"></td>
					</tr>
				</table>
			</td>	         
		</tr>
