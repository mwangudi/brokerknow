...
<title>Download Batch Applications</title>
...
'Prepare to note records downloaded last
 sqlstr = "UPDATE  Offerings " & _
		  " SET LastDownloaded =0 where Forward  = 0"
...
FileHeader = 1 & vbTab & CDANo & vbTab & "00" & vbTab & DNumber & vbTab & Year(Now()) & "-" & right("00" & Month(Now()),2) & "-" & right("00" & Day(Now()),2) & vbTab & vbcrlf
  ...
  for intCount = 0 to intRstCount-1 
   ...
   AppNo = trim(replace(astrRecs(3,intCount),vbTab," "))
   ...
   TotalAmt = trim(astrRecs(2,intCount))
    ...
    sqlstr = "SELECT OfferingsList.Batch_No, OfferingsList.Client_DPA_, Client.ClientName, OfferingsList.Offering_DPA_, OfferingsList.BatchSeq, OfferingsList.PAL_No As SerialNo,  " & _
			 "                       OfferingsList.Alloted_Rights as Qty, OfferingsList.Offering_Price As Price, OfferingsList.Offering_Price * OfferingsList.Alloted_Rights AS Amount " & _
			 " FROM         OfferingsList INNER JOIN " & _
			 "                       Client ON OfferingsList.Client_DPA_ = Client.Client_DPA_ " & _
			 " WHERE     (OfferingsList.Batch_No = " & BatchID & ") " & _
			 " ORDER BY OfferingsList.BatchSeq"
	...
	if intRsBatchCount > 0 then
	  ...
	  for count = 0 to intRsBatchCount-1
        ...
        AppSerialNo = right(replace(trim(batchdata(5,count)),vbTab," "),6) 'trim to 6 characters to the right
		batchlineNo = trim(batchdata(4,count))
		ClientName = replace(trim(batchdata(2,count)),vbTab," ")
	 ...
	 TotalAgentAmount = TotalAgentAmount + TotalAmt
	 TotalChequeAmount = 0
	 ...
	 'Mark records in batch as downloaded also update file name of download 
        sqlstr = "UPDATE    Offerings " & _
				 ...
				 " WHERE    (Batch_No = " & BatchID & ") AND Forward  = 0"
...
if trim(ToNo) <> "" AND trim(FromNo) <> "" then
...
if trim(filecontent) = "" then
 %>
	 <script language="javascript">
	  alert("No batch applications were found for downloading.")
...
FileDownloaded =  savefile(pathToFile,filecontent)
