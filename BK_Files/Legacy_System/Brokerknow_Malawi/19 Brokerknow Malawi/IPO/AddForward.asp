...
If action = "EXECUTE" Then
	...
	if (trim(broker)= "") then%>
        <script language = 'vbscript'>
			ShowMessage "Please enter the name of the receiving broker."
		</script>
	    <% response.end
	end if

	'-validate for null entry -cheque number...
    if(Trim(chkno) ="") then%>
    <script language = 'vbscript'>
	    ShowMessage"Please enter the cheque number."
	</script>
    <% Response.end
	end if

    '-validate for null entry -bank ref...
    if(Trim(narrative) ="") then
    %>
    <script language = 'vbscript'>
	     ShowMessage"Please enter the bank name."
	</script>
    <% Response.end
	end if
    
	'-validate for null entry - branchref...
    if(Trim(paymentbranchref)  ="") then
    %>
    <script language = 'vbscript'>
	     ShowMessage"Please provide the branch ref."
	</script>
    <% Response.end
	end if

	'-validate for null entry -sort code...
    if(Trim(paymentsortref)  ="") then
    %>
    <script language = 'vbscript'>
	     ShowMessage"Please provide the sort code"
	</script>
    <% Response.end
	end if
    
	
	'-validate for null entry -account ref...
    if(Trim(paymentaccountref)  ="") then
    %>
    <script language = 'vbscript'>
	     ShowMessage"Please enter the client account number."
	</script>
    <% Response.end
	end if
...
END IF
	...
	'GET BATCH NO
		...
		'response.write NextCount & "<BR>" & NextBatch :Response.end
		'------------------------------------------------------------------------------------------------------------------
	...
	sqlStr = " INSERT INTO Offerings (PAL_No,Client_DPA_, " & _   
	         " Offering,Offering_Price,Alloted_Rights,ChangedBy,AcceptanceType,Additional," & _
	         " Batch_No,Forward,PaymentRef,PaymentBankRef,DateCreated,CreatedBy,ReceivingBroker," & _
	         " PaymentBranchRef,PaymentSortCode,PaymentAccountNo,status,PaymentType) " & _
			 " VALUES ('" & palno & "'," & ClientName & "," & offering & "," & Price & "," & AlRights & "," & UserId & "," & Accept & _
			 "," & AddNew & "," & NextBatch & ",1,'" & chkno & "','" & narrative & "',GetDate(),"& UserID &","& Broker &",'" & _
			 paymentbranchref & "','" & paymentsortref & "','" & paymentaccountref & "',1," & PaymentType & ")"
...
End If
%>
<form name = 'frmAddSecurity' method = 'post' id="frmMain" action = "AddForward.asp" >
	<table border="0" width="80%" cellpadding=2 cellspacing=2>
		<tr>
			<td width="20%" nowrap>PAL NO</td>
			<td width="80%" nowrap><input type="text" name="txtPalNo" id="txtPalNo" size="25" value="" onblur="ConfirmSerial(this);"></td>
		</tr>
		...
		<tr>
			...
			<td width="80%" nowrap><input type = 'text' name ='txtPayable' id = 'txtPayable' size="20" value=0 readonly class="readonlyex"></td>
		</tr>
		
		<tr>
			<td width="20%">Cheque Number</td>
			<td width="80%"><input type = 'text' name ='txtchkNo' id = 'txtchkNo' size="20" value=''></td>    
		</tr>

		<tr>
			<td width="20%">Bank</td>
			<td width="80%"><input type="text"  name ='txtpaymentbankref' id = 'txtpaymentbankref'></td>   	
		</tr>
		 
		<tr>
			<td width="20%">Branch</td>
			<td width="80%"><input type ='text' name ='txtpaymentbranchref' id = 'txtpaymentbranchref'></td>   	
		</tr>
		
		<tr>
			<td width="20%">Sort Code</td>
			<td width="80%"><input type ='text' name ='txtpaymentsortref' id = 'txtpaymentsortref'></td> 	
		</tr>

		<tr>
			<td width="20%">Account Number</td>
			<td width="80%"><input type ='text' name ='txtpaymentaccountref' id = 'txtpaymentaccountref'></td>   	
		</tr>
