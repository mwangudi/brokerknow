		...
		<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/default.css">
			<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/webparts.css">
				...
				<style> 
					...
					.active-column-6 {width: 100px;  text-align: right;}
...
'======================= Begin_Alter_Across_Entities =================================
			'row ID
			entityid=rs("type") & "<->" & rs("EntryId")& "<->" & FormatDate(rs("Date"))
			rowData = rowData & quote & entityid & quote & " : " 
			
			if isnull(rs.Fields("TimeChanged")) or rs.Fields("TimeChanged") = "" then
			TimeChanged = " "
			else
			...
			'row data 
			rowData = rowData & "["			
			rowData = rowData & quote & FormatDate(rs.Fields("ReconcileDate")) & quote & ","			
			rowData = rowData & quote & rs.Fields("BankCode") & quote & "," 			
			rowData = rowData & quote & rs.Fields("AccountName") & quote & "," 			
			rowData = rowData & quote & FormatDate(rs.Fields("Date")) & quote & ","
			rowData = rowData & quote & FormatNum(rs.Fields("Reference")) & quote & ","
			rowData = rowData & quote & FormatNum(rs.Fields("Particulars")) & quote & ","			
			rowData = rowData & quote & FormatNum(rs.Fields("Debit")) & quote & ","
			rowData = rowData & quote & FormatNum(rs.Fields("Credit")) & quote & ","			
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = ["Reconcile Date","Bank Code", "Account", "Date", "Reference", 
			"Particulars", "Debit","Credit","Last Modified By","Time Changed"];
		    </script>
<%'======================= End_Alter_Across_Entities =================================%>			
		<script>
			//data
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="BankCode:Bank Code*0;AccountName:Account Name*0;Date:Transaction Date*3;Reference:Reference*0;Particulars:Particulars*0;Debit:Debit*4;Credit:Credit*4;ReconcileDate:Reconcile Date*3">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="BankCode:Bank Code;AccountName:Account Name;Date:Transaction Date;Reference:Reference;Particulars:Particulars;Debit:Debit;Credit:Credit;ReconcileDate:Reconcile Date">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="BankCode:Bank Code*0;AccountName:Account Name*0;Date:Transaction Date*1;Reference:Reference*0;Particulars:Particulars*0;Debit:Debit*4;Credit:Credit*4;ReconcileDate:Reconcile Date*1">
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:19em;width:23em">
