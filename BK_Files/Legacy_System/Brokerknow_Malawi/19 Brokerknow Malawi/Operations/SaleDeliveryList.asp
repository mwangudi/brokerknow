...
'======================= Begin_Alter_Across_Entities =================================
		...
		const DataSource = "SaleContractList" 'table or view in database
		const DataEntityList = "SaleDeliveryList" 'this page
		const DataEntity = "SDelivery"
			...
			<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/webparts.css">
				...
				<script language='vbscript'>
					...
					function SaveInPlaceEdit()
							frm<%=DataSource%>.elements("delAction").value = "Execute"
...
'======================= Begin_Alter_Across_Entities =================================
			if rs.Fields("ContractTransferNo") = "" or isnull(rs.Fields("ContractTransferNo")) then
			...
			rowData = rowData & quote & rs.Fields("ContractTransferNo") & quote & ","
			rowData = rowData & quote & rs.Fields("ContractCertificate") & quote & "," 
			rowData = rowData & quote & FormatDate(rs.Fields("ContractDeliveryDate")) & quote & "," 
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			//column titles 
			var colCount = 11;
			var colNames = ["", "txtTransferNo", "txtCertNo", "txtDDate", "", 
			"", "", "", "", "",""];
			
			var myColumns = ["Contract", "Transfer", "Certificate", "Delivery Date", "Broker", 
			"Trade Date", "Slip No", "Quantity", "Price", "Client",""];
		</script>
<%'======================= End_Alter_Across_Entities =================================%>			
		<script>
			//data
					...
					if(colNames[i] != "")
					{
						...
						else
						{
							if(colNames[i]=="txtDDate")
							{
								...
								myData[rowIndex][i] = "<INPUT TYPE='text' NAME='" + colNames[i] + "' ID='" + colNames[i] + "' VALUE='"+ myData[rowIndex][10]+"' onChange = 'EditInPlaceDataChanged();event.cancelBubble=true;' OnClick='event.cancelBubble=true;'>";
							}
							else
							{
...
/////==========================  End_CHanged   05 Apr 2004 =============================================================///////////////////////////////
			...
			function changeDateInterface(){
				calDDate = new ctlSpiffyCalendarBox('calDDate', 'frm<%=DataSource%>', 'txtDDate', 'cmdDDate','<%= FormatDate(Date+7) %>', 1); 
				calDDate.returnOutStringOnWrite(); 
				var parentDiv = document.all.item("txtDDate").parentNode;
				parentDiv.innerHTML = calDDate.writeControl();
		...
		<script> 
			...
			grid.getLayoutTemplate().action("adjustSize");
				...
				<input type = 'hidden' name ='EditPage' id = "EditPage" value = "<%=ActionFolder%>/EditSDelivery.asp">
				<input type = 'hidden' name ='AddPage' id = "AddPage" value = "<%=ActionFolder%>/EditSDeliveryInPlace.asp"> 
				<input type = 'hidden' name ='DeletePage' id = "DeletePage" value = "DeleteDelivery.asp">
				<input type = 'hidden' name ='ActionPage' id = "ActionPage" value = "<%=DataEntityList%>.asp">

<%'======================= Begin_Alter_Across_Entities =================================%>
				
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="ContractNumber:Contract*0;ContractTransferNo:Transfer*0;ContractDeliveryDate:Delivery Date*1;BrokerCode:Broker*0;LotTDate:Trade Date*1;LotSlipNo:Slip No*0;LotQty:Quantity*2;LotPrice:Price*2;OrdDetailClient:Client*0">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="ContractNumber:Contract;ContractTransferNo:Transfer;ContractDeliveryDate:Delivery Date;BrokerCode:Broker;LotTDate:Trade Date;LotSlipNo:Slip No;LotQty:Quantity;LotPrice:Price;OrdDetailClient:Client">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="ContractNumber:Contract*0;ContractTransferNo:Transfer*0;ContractDeliveryDate:Delivery Date*1;BrokerCode:Broker*0;LotTDate:Trade Date*1;LotSlipNo:Slip No*0;LotQty:Quantity*2;LotPrice:Price*2;OrdDetailClient:Client*0">
