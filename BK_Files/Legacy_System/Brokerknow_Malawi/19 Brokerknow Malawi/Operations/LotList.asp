...
'======================= End_Alter_Across_Entities =================================		
		...
		sqlStrOrig = "SELECT distinct [" & DataSource & "].*,ordDetail.Amount,datastream_SecurityPriceList.Price FROM [" & DataSource & "] inner join ordDetail on [" & DataSource & "].ordDetail_DPA_=ordDetail.ordDetail_DPA_ " & _
		         	 "  inner join Security on LotList.Security_DPA_=Security.Security_DPA_ inner JOIN dbo.datastream_SecurityPriceList on Security.Security_DPA_=datastream_SecurityPriceList.Security_DPA_where(Rtrim(ordDetailType) like %'Purchase'%)"
		
		sqlStr = "SELECT Distinct top 60 LotList.*,ordDetail.Amount,datastream_SecurityPriceList.Price FROM LotList inner join ordDetail on LotList.ordDetail_DPA_=ordDetail.ordDetail_DPA_ " & _
		         "  inner join Security on LotList.Security_DPA_=Security.Security_DPA_ inner JOIN dbo.datastream_SecurityPriceList on Security.Security_DPA_=datastream_SecurityPriceList.Security_DPA_ order by LotList.Order_DPA_ Desc,ContractNumber Desc"			

		If filterStr <> "" Then
			sqlStr = "SELECT Distinct TOP 60  LotList.*,ordDetail.Amount,dbo.datastream_SecurityPriceList.Price FROM LotList inner join ordDetail on LotList.ordDetail_DPA_=ordDetail.ordDetail_DPA_ " & _
		         	 "  inner join Security on LotList.Security_DPA_=Security.Security_DPA_ inner JOIN dbo.datastream_SecurityPriceList on Security.Security_DPA_=datastream_SecurityPriceList.Security_DPA_ WHERE LotList." & filterStr & " order by LotList.Order_DPA_ Desc,ContractNumber Desc	"
		End If
		...
		If searchStr <> "" Then
			...
			Else
				 sqlStr = "SELECT Distinct LotList.*,ordDetail.Amount,datastream_SecurityPriceList.Price FROM LotList inner join ordDetail on LotList.ordDetail_DPA_=ordDetail.ordDetail_DPA_ " & _
		         			      "  inner join Security on LotList.Security_DPA_=Security.Security_DPA_ inner JOIN dbo.datastream_SecurityPriceList on Security.Security_DPA_=datastream_SecurityPriceList.Security_DPA_ WHERE LotList." & searchStr & " order by LotList.Order_DPA_ Desc,ContractNumber Desc	"
			End If
		End If
		...
		<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/default.css">
			<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/webparts.css">
				...
				<style> 					
					...
					.active-column-10 {width: 30px;}
					.active-column-11 {width: 60px;}
					.active-column-12 {width: 80px;}	
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var colCount = 16;
			var colNames = ["", "", "", "", "", "SlipNo", "", "", "",
			"", "TDate","", "", "Quantity", "Price","Broker","","Commission"];
			
			var myColumns = ["Traded", "Qty", "Price","Order", "Date", "Code", "Client","Comm","Contract", "Slip", "Side",  "Security", "Balance Qty","Broker",];
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="Order_DPA_:Order No*2;OrderDate:Date*1;Client_DPA_:Client Code*0;OrdDetailClient:Client*0;ClientCDSNo:CDS NO*0;ContractNumber:Contract*0;LotSlipNo:Slip No*0;OrderTypeSale:Type*0;OrdDetailSecurity:Security*0;OrdDetailQty:Limit*2;BalanceQty:Balance*2;LotQty:Quantity*2;LotPrice:Price*2;BrokerCode:Broker*0;LotTDate:Trade Date*1;StatusDescription:Status*0">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="Order_DPA_:Order No;OrderDate:Date;Client_DPA_:Client Code*0;OrdDetailClient:Client;ClientCDSNo:CDS NO;ContractNumber:Contract;LotSlipNo:Slip No;OrderTypeSale:Type;OrdDetailSecurity:Security;OrdDetailQty:Limit;BalanceQty:Balance;LotQty:Quantity;LotPrice:Price;BrokerCode:Broker;LotTDate:Trade Date;StatusDescription:Status">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="Order_DPA_:Order No*2;OrderDate:Date*1;Client_DPA_:Client Code*0;OrdDetailClient:Client*0;ClientCDSNo:CDS NO*0;ContractNumber:Contract*0;LotSlipNo:Slip No*0;OrderTypeSale:Type*0;OrdDetailSecurity:Security*0;OrdDetailQty:Limit*2;BalanceQty:Balance*2;LotQty:Quantity*2;LotPrice:Price*2;BrokerCode:Broker*0;LotTDate:Trade Date*1;StatusDescription:Status*0">
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:32em;width:60em">
