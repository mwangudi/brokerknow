...
<%
	...
	if action = "EXECUTE" then
		...
		'validate size of Certificate No
		If Len(certNo) > 20 Then%>
		        <script language = 'vbscript'>
		        ShowMessage "Certificate No can only be 20 characters in length"
		        window.history.back
		        </script>
		        <% response.end
		End If
		...
		'sqlStr = "UPDATE OrdDetail SET  OrdDetailCertNo = '" & certNo & "' " & _
		'		"	WHERE(OrdDetail_DPA_ IN " & _
        '         "		(SELECT OrdDetail_DPA_ " & _
        '          "			FROM LotList" & _
        '         "			WHERE (Contract_DPA_ = "  & ID & ")))"
        
        sqlStr = "UPDATE [Contract] SET ContractCertNo = '" & certNo &  "'," & _
		    " ContractDeliveryDate = #" & FormatDate(dDate) & "#, " & _
		    " ContractDelivered = 1 WHERE Contract_DPA_ = " & ID
    ...
    'if isnull(rs.Fields("ContractDeliveryDate")) then
			DDate = Date
    'else
			DDate = rs.Fields("settlementdate")
    'end if
   %>
...
<form name = 'frm<%=DataSource%>' method = 'post' action = 'EditSDeliveryInPlace.asp' id='frmMain' >
<table border="0" cellspacing="1" cellpadding="1" height="218">
  ...
  <tr>
    <td nowrap height="25"> Certificate No</td>
    <td nowrap height="25"><input type = 'text' name ='txtCertNo' id = "txtCertNo" size="20" value = '<%=rs.Fields("ContractCertificate")%>'></td>
  </tr>
  <tr>
