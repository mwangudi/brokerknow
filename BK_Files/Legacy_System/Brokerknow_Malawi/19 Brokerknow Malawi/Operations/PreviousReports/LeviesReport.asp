...
If genReport <> "1" Then%>
	...
	<form method="POST" action="LeviesReport.asp" Name="frmMain" id="frmMain">
...
headerDescription = " Between " & FormatDate(selectedFromDate) & " and " & FormatDate(selectedToDate) 
 		...
 		sqlStr = "Select * From LeviesReport Where SystemMaintained="& selectedClient & " and " & _
			   " TransDate between '" & CDate(selectedFromDate) & "' and '" & CDate(selectedToDate) & "' order by OrdDetailType,TransDate"
...
<%		
	...
	Do Until rs.EOF
			...
			TotalGross=TotalGross+FormatNum(rs.Fields("LotPrice")) 
			OrderType1=rs.Fields("OrdDetailType")
			
			if(OrderType1<>OrderType2 and first<>0) then
			%>
			 <tr>
				<td colspan=7>&nbsp;</td>
			 </tr>        
			 <tr>
							<td colspan=4 align=right><font face="Arial" size="2"><b>Purchase Total:</b></font></td>
							<td align=right style="border-style: solid; border-color: #000000; border-width: 1" height="30px"><%= FormatNum(OrderGross) %></td>
							<td align=right style="border-style: solid; border-color: #000000; border-width: 1" height="30px"><%= FormatNum(OrderTypeTotal) %></td>
							<td>&nbsp;</td>
			 </tr>
			 <tr>
				<td colspan=7><b>SALES</b></td>
			 </tr>
			<%
			OrderTypeTotal=0
			OrderGross=0
			...
			%>
                ...
                <%
                OrderTypeTotal=OrderTypeTotal+rs.Fields("LevyAmount") 
				OrderGross=OrderGross+rs.Fields("LotPrice") 
				OrderType2=OrderType1
				first=1
		...
		<tr>
			 ...
			 <tr>
							<td colspan=4 align=right><font face="Arial" size="2"><b>Sales Total:</b></font></td>
							<td align=right style="border-style: solid; border-color: #000000; border-width: 1" height="30px"><%= FormatNum(OrderGross) %></td>
							<td align=right style="border-style: solid; border-color: #000000; border-width: 1" height="30px"><%= FormatNum(OrderTypeTotal) %></td>
							<td>&nbsp;</td>
		</tr>
        ...
        <tr>
         ...
         <tr>
						<td colspan=4 align=right><font face="Arial" size="2"><b>Totals:</b></font></td>
