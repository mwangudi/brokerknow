...
<title>Account Manager Statement</title>
...
selectedOwner = Request.Form("cboOwner")
selectedFromDate = Request.Form("transFromDate")

If genReport <> "1" Or selectedOwner = "" Or Not IsDate(selectedFromDate) Then%>
	<Script Language="JavaScript">
		report_SetBodyClass();
		function validateForm(frm){			
			if (frm.cboOwner.selectedIndex < 0){
				alert("Select an Account Manager");
				frm.cboOwner.focus();
				return;
			}
	...
	<form method="POST" action="OwnerStatement.asp" Name="frmMain" id="frmMain">
		<input type="hidden" value="1" name="genReport">
		<table>
			<tr>
				<td>Account Manager: </td>
				<td><select name = 'cboOwner' id = 'cboOwner' size="1">
...
<%
	...
	sqlStr = "SELECT * FROM OwnerStatement where Owner_DPA_ LIKE '" & selectedOwner & "' AND TransDate >= '" & FormatDate(selectedFromDate) & "'"
	...
	If rs.EOF Or rs.BOF Then%>
		<Script Language="JavaScript">
			alert("The specified account manager does not have any transaction using the specified date criterion")
			window.history.go(-1);
		</Script>
		<%Set Rs = Nothing
		Set Conn = Nothing
	...
	Set rsOwner = Conn.Execute ("SELECT * FROM OwnerList WHERE Owner_DPA_ = " & selectedOwner)
	
	If Not (rsOwner.EOF Or rsOwner.BOF) Then
		accountDesc = rsOwner.Fields("OwnerName").Value
		accountAddress = ""'rsOwner.Fields("OwnerAddr").Value
	End If
	...
	If Not IsOpeningBalance Then
		...
		cloneRs.Filter = ""
		cloneRs.Filter = "Owner_DPA_ LIKE '" & selectedOwner & "' AND TransDate < '" & FormatDate(selectedFromDate) & "'"
...
<table border="0" cellspacing="2" cellpadding="2" style="font-family: Arial Narrow" width="100%">
    <tr>
		<td width="10%" nowrap><font face="Impact" size="4">ACCOUNT MANAGERS STATEMENT</font></td>
