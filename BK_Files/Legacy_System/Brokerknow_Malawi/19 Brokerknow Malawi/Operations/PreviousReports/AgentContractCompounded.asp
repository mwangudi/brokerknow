...
If genReport <> "1" Or selectedContractDate = "" Then%>
	...
	<form method="POST" action="AgentContractCompounded.asp" Name="frmMain" id="frmMain">
...
<%
	...
	Do Until OrderRs.EOF
			...
			sqlStr = "SELECT   Order_DPA_, OrdDetailSecType,Agent_DPA_, AgentName, isnull(AgentAddr,'') as AgentAddr, LevyName, LevyShortName, CASE SystemMaintained WHEN 13 THEN MAX(LevyAmount) ELSE SUM(LevyAmount) " & _
					  ...
					  "GROUP BY Agent_DPA_, AgentName, AgentAddr, LevyName, LevyShortName, OrdDetailSecurity, OrdDetailClient, SystemMaintained, LevyRatePercentage,OrdDetailSecType,Order_DPA_,Client_DPA_"
...
<table border="0" cellspacing=2 cellpadding=2 class="ReportsTable" width="90%" align="center" valign="Top">
<tr><td>
<table border="0" cellspacing=2 cellpadding=2 class="ReportsTable" width="100%" >
	...
	<tr>
		<td align="Right" colspan=2 height="160">
	...
	<tr>
	<td colspan="2">
		<table  cellspacing=0 cellpadding=4 border=1  width="100%" class="ReportsTable"> 	
	<tr>
	<td>
		<table  cellspacing=0 cellpadding=0 border=0  width="100%" class="ReportsTable">
		<tr>
		<td  nowrap width="65%" >			
			<table cellspacing=0 cellpadding=0 border=0  width="100%" class="ReportsTable">
				<tr>
				...
				<tr>					
					<td  nowrap><b>Account:</b> <%= Rs.Fields("OrdDetailClient").Value %>&nbsp;&nbsp;[<%= Rs.Fields("Client_DPA_").Value %>]</td>
				</tr>
		...
		<td>
			<table cellspacing=0 cellpadding=0 border=0 width="100%" class="ReportsTable">
				<tr>
					...
					<td align="left"><%= FormatDate(selectedContractDate) %></td>
				</tr>
				<tr>
				...
				<tr>
					<td nowrap valign="top" align="right"> Slip Nos:&nbsp;</td>
					<td valign="top">
	...
	<tr>
		<td colspan=2 height="16">&nbsp;&nbsp;Dear Sir/Madam</td>
	</tr>
	<tr>
		<td colspan=2 height="16" align="center"><b><U>RE: <%= UCase(Rs.Fields("OrdDetailType").Value) %> OF SECURITIES<b></U></td>
	</tr>
...
<table border="0" cellspacing=0 cellpadding=2 class="ReportsTable"  width="100%">	
	...
	<tr>
	<td colspan="3">
	<table border="0" cellspacing=0 cellpadding=3 class="ReportsTable"  width="100%">	
	<tr>
		<td width="8%">&nbsp;</td>
	...
	<tr>
		<td width="8%">&nbsp;</td>
		<td STYLE="PADDING-LEFT: 0PX">Average Price</td>
	...
	<tr>
		...
		For i = 1 To UBound(levyArray) 
			...
			If thisLevyName <> "" Then
				...
				if lcase(rs.Fields("OrdDetailSecType")) = "fixed" OR trim(thisLevyname)="Broker Commission" Then
				levyPerc = FormatNumEx(levyPerc,4) & "%"
				else
				levyPerc = FormatNum(levyPerc) & "%"
				end if
				...
				<TR>
					<td width="8%">&nbsp;</td>
					<td nowrap width="30%" STYLE="PADDING-LEFT: 0PX"><%= thisLevyname %>&nbsp;&nbsp;</td>
					<td ALIGN="LEFT"><%= levyPerc  %></td>
					<td align=right colspan="2"><%= FormatNum(levyArray(i, 1)) %></td>		
				</TR>
		<% End If
		Next%>
	...
	<tr>
		...
		<td align=right colspan="2">
			<u>
			<%= Replace(Space(Len(FormatNum(totalLevies)) * 4), Space(1), "&nbsp;") %></u></td>
	</tr>
	...
	<%if trim(contractStampsDesc) <> "" then%>
			<tr>
				...
				<td align=right colspan="2">	
					 <%= FormatNum(contractStampsVal) %> </td>
			</tr>
	<%end if%>
	<tr>
		<td width="8%">&nbsp;</td>
	...
	<tr>
		...
		<td align=right colspan="2"><b>
			...
			<TR>		
				...
				<td ALIGN="LEFT">&nbsp;&nbsp;<%=  FormatNum(AgentRate * 100/brokerRate) & "%" %></td>
...
<tr>
	...
	<td width="6%" STYLE="PADDING-LEFT: 0PX">&nbsp;</td>	
		<td>
		<i><font face=Arial size=2>For and on behalf of</i><br>
