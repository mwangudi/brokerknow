...
If genReport <> "1" Or Not IsDate(selectedFromDate) Then%>
	...
	<form method="POST" action="AgentDebtors.asp" Name="frmMain" id="frmMain">
...
<%
   ...
   Dim rs
	...
	Rs.CursorLocation = adUseClient	
	    ...
	    sqlStr = "SELECT * FROM [AgentDebtors] WHERE Agent LIKE '" & selectedAgent & "'"		 
		'sqlStr = "DebtorsAndCreditors @TransDate = '" & selectedFromDate & "'" 
		'LastDate <= '" & CDate(selectedFromDate) & "' and
        ...
        Set rsClient = Conn.Execute ("SELECT * FROM AgentList WHERE Agent_DPA_ = " & selectedAgent)
	
		If Not (rsClient.EOF Or rsClient.BOF) Then
		accountDesc = rsClient.Fields("AgentName").Value & " " & rsClient.Fields("Agent_DPA_").Value
		accountAddress = rsClient.Fields("AgentAddr").Value
		End If
