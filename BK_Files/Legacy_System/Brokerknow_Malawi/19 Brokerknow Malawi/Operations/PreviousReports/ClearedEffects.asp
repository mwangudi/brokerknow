...
If genReport <> "1" Or selectedBank = "" Or Not IsDate(selectedFromDate) Then%>
	...
	<form method="POST" action="ClearedEffects.asp" Name="frmMain" id="frmMain">
...
'Response.end
	...
	sqlStr = "SELECT * FROM DB_Reconciliation where BankAccount_DPA_ =" & selectedBank & " AND Not (ReconcileDate is null ) and TransDate between '" & FormatDate(CDate(selectedFromDate)) & "' and '" & FormatDate(CDate(selectedToDate)+1) & "'"		
	...
	If Not (rsBank.EOF Or rsBank.BOF) Then
		accountDesc = rsBank.Fields("AccountName").Value & "&nbsp;[" & rsBank.Fields("Account_DPA_").value & "]"
	...
	If Not IsOpeningBalance Then
		...
		sqlStr = "SELECT CASE (DB_Reconciliation.IsOpeningBalance) WHEN 0 THEN SUM(ISNULL(dbo.DB_Reconciliation.Credit - dbo.DB_Reconciliation.Debit, 0))" & _ 
				 ...
				 " WHERE  not (dbo.DB_Reconciliation.ReconcileDate is null) and (dbo.DB_Reconciliation.TransDate < '" & FormatDate(selectedFromDate) & "') GROUP BY dbo.DB_Reconciliation.BankAccount_DPA_,DB_Reconciliation.IsOpeningBalance, dbo.Account.AccountOpeningBal having dbo.DB_Reconciliation.BankAccount_DPA_=" & selectedBank  		
...
<table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="600">
    <tr>
      <td width="10%" nowrap><font face="Impact" size="4">CLEARED EFFECTS</font></td>
...
<BR>
  ...
  <table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow; LEFT-MARGIN:100PX"  width="600">      
      <tr>
      ...
      <td style="border-top-style: solid; border-top-width: 1; border-bottom-style: solid; border-bottom-width: 1"><b><font face="Arial Narrow" size="3" width="150">Particulars:</font></b></td>
    ...
    Do Until Rs.EOF
		...
		<tr>	
		  ...
		  <% end if %>
		  <td width="150"><%= Rs.Fields("Particulars").Value %></td>
