...
Set conn = GetActiveConnection("KBroker") 
   ...
   sqlStr = "SELECT * FROM TradeAffirmation WHERE LotTDate = '" & FormatDate(selectedTradeDate) & "'"
   ...
   sqlStr = "SELECT distinct Order_DPA_,BalanceQty,OrderRef,AccountManager,Class_DPA_ FROM TradeAffirmation WHERE LotTDate = '" & FormatDate(selectedTradeDate) & "'"
   ...
   Do Until sumRs.EOF
	...
	'Response.write(SettleDate)
   	'Response.end   
        ...
        sqlStr1="SELECT distinct SettlementAmount,LotQty, LotSlipNo, SettlementGrossAmount, LotPrice FROM TradeAffirmation WHERE( Order_DPA_ = " & sumRs("Order_DPA_") & ") and LotTDate = '" & FormatDate(selectedTradeDate) & "'"
       ...
       set TotalRs=nothing 		
		...
		sqlStr = "SELECT TradeAffirmation.*,'' as AccountManager FROM TradeAffirmation WHERE LotTDate = '" & FormatDate(selectedTradeDate) & "' And OrderRef = '" & OrderRef & "' AND Order_DPA_ = " & sumRs.Fields("Order_DPA_").Value		
		Set Rs = Conn.Execute (SQLServerFormat(HandleQuote(sqlStr)))
