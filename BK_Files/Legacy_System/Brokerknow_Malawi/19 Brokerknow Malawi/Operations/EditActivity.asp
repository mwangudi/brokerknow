...
<%
   ...
   Dim rsEdit
	...
	if action = "EXECUTE" then
        ...
        ActvtyClass = Request.Form("cboActvtyClass")
        ...
        'validate Name
        If Trim(ActvtyClass) = "" Then%>
                <script language = 'vbscript'>
                		ShowMessage "Please specify the Type"
                		window.self.close
                </script>
                <% response.end
        End If
        ...
        'save data
        sqlStr = "UPDATE Activity SET ActivityDate = " & "#" & FormatDate(actDate) & "#" & ", ActivityNotes = " & "'" & Notes & "'" & "" & _
                "       ,ActvtyClass_DPA_ = "  & ActvtyClass  & ",Client_DPA_ = "  & client & _
                ", ChangedBy = " & session("UserID") & " , TimeChanged = #" & now() & "#  WHERE Activity_DPA_  = " & ID
...
<table border="0" width="100%">
  <tr>
    ...
    <td width="83%"><select name = 'cboClient' id = 'cboClient' 
			onKeypress="return (dodefaultaction()==''); " 
			onKeydown="return (dodefaultaction()==''); " 
			onKeyup="return (change(cboClient));" 
...
<%
		...
		sqlStr = "SELECT ActivityDate,ActivityNotes,Activity_DPA_,Activity.ActvtyClass_DPA_, ClientList.Client_DPA_ FROM [ClientList] INNER JOIN [ActvtyClassList] INNER JOIN [Activity] ON ActvtyClassList.ActvtyClass_DPA_ = Activity.ActvtyClass_DPA_ ON ClientList.Client_DPA_ = Activity.Client_DPA_ WHERE Activity_DPA_  = " & ID
        ...
        If Not (rsEdit.EOF Or rsEdit.BOF) Then
                ...
                Do Until rsEdit.EOF
                		if rsEdit.Fields("Client_DPA_") = rs.Fields("Client_DPA_") Then%>
                			<option selected SearchCode = "<%=rsEdit.Fields("Client_DPA_")%>" SearchText = "<%=rsEdit.Fields("ClientName")%>" value = '<%=rsEdit.Fields("Client_DPA_")%>'><%=rsEdit.Fields("ClientNameEx")%></option>
                		<%else%>
							<option  SearchCode = "<%=rsEdit.Fields("Client_DPA_")%>" SearchText = "<%=rsEdit.Fields("ClientName")%>" value = '<%=rsEdit.Fields("Client_DPA_")%>'><%=rsEdit.Fields("ClientNameEx")%></option>
                     <%end if
