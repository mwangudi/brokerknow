...
<%
	...
	select case  action 
		...
		case"EXECUTE"
			...
			buttonAction = Trim(Ucase(Request("buttonAction")))
			...
			if instr(1,buttonAction,"SAVE") > 0 then
					...
					if instr(1,buttonAction,"PRINT") > 0 then
						...
						<SCRIPT LANGUAGE="JAVASCRIPT">
							...
							//alert('hapa');
							window.self.location.href='ReceiptForm.asp?ID=<%=ID%>';
				...
				if instr(1,buttonAction,"PRINT") > 0 then
					...
					<SCRIPT LANGUAGE="JAVASCRIPT">
						//window.parent.parent.frames['maininfo'].location.reload();
						window.opener.location= window.opener.location;
						window.location='ReceiptForm.asp?ID=<%=ID%>';
					</SCRIPT>
					<%
					'WriteDialogRelocateScript "ReceiptForm.asp?ID=" & ID
				else
...
<script language = "javascript" >
		...
		function LoadClient()
		{
		 var clientcode = document.frmEditReceipt.elements("txtClientCode").value
		 var clientcobo = document.getElementById("cboAccount");		 
				
				xmlhttp = createXMLHTTPObj();
				
				url="../Reports/GetList.asp?clientcode="+clientcode+"&cdsno=&action=SLoadClient";
				...
				xmlhttp.onreadystatechange=function() 
				  {
							if (xmlhttp.readyState==4) 
							{
								...
								document.frmEditReceipt.elements("txtClientCode").value = myArray[0];
...
<form name = 'frm<%=DataSource%>' method = 'post' action = '<%=DataSource%>N.asp' id = "frm<%=DataSource%>">
...
%>
	...
	<%
	if currentEntityType=5 then 
		
	%>
		 <input type = 'text' name ='txtClientCode' id = 'txtClientCode' size="10" value='<%=Ccode%>'   onBlur="LoadClient(this.value, this.name);">

	<script language="javascript">
		LoadAccount(<%=Ccode & "," & currentEntityType%>);
	</script>
	...
	<%
	 ...
	 %>
    <input type = 'text' name ='txtClientCode' id = 'txtClientCode' size="10" value='<%=Ccode%>'" onBlur="LoadClient(cboAccount, this.name);">
...
%>
  ...
  <tr>
	  <td width="100%" colspan=3 align="center" valign=absBottom>
		<BR><BR>
		<input type = 'button' Class=Buttons name ='cmdAdd' id = 'cmdPrint' value=" Save & Print " onclick = "document.getElementById('buttonAction').value='Save & Print';AllowedNavigation();">
		<input type = 'button' Class=Buttons name ='cmdAdd'  value="&nbsp;&nbsp;&nbsp;&nbsp;Save&nbsp;&nbsp;&nbsp;&nbsp;" onclick = "document.getElementById('buttonAction').value='Save';AllowedNavigation();">
		<input type = 'button' Class=Buttons name ='cmdAdd' id = 'cmdAddPrint' value="&nbsp;&nbsp;&nbsp;&nbsp;Print &nbsp;&nbsp;&nbsp;&nbsp;" onclick = "document.location.href='receiptForm.asp?ID=<%=ID%>'">
