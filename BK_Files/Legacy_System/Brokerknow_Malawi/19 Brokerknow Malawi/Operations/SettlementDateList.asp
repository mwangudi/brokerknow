...
'======================= Begin_Alter_Across_Entities =================================
		...
		const DataSource = "DB_Affirmation2"
		const ActionPage = "SettlementDateList"
...
'======================= End_Alter_Across_Entities =================================		
		...
		If searchStr <> "" Then
			...
			Else
				sqlStr = "SELECT top 100 * FROM [" & DataSource & "] WHERE " & searchStr
			End If
		End If
		
		If filterStr <> "" Then
			sqlStr = "SELECT top 100  * FROM [" & DataSource & "] WHERE " & filterStr
		End If
		
		If sortQryStr <> "" Then		
			sqlStr = "SELECT top 100  * FROM [" & DataSource & "] ORDER BY " & sortQryStr	
		End If 
...
<html>
		...
		<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/default.css">
			<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/webparts.css">
				...
				<style> 
					...
					.active-column-8 {width: 100px; text-align: right;}	
					.active-column-9 {width: 100px; text-align: right;}	
					.active-column-10 {width: 100px; text-align: right;}
					.active-column-11 {width: 100px; text-align: right;}										
				</style>
	...
	<form name='frm<%=ActionPage%>' method='post' id='frmMain' action='Edit<%=DataEntity%>InPlace.asp'>
		...
		intRecord = 1
		Do Until rs.EOF 
		Commission=rs.Fields("Levytotal") + rs.Fields("Comm")	
'======================= Begin_Alter_Across_Entities =================================
			'row ID 
			...
			rowData = rowData & quote & rs("Account") & quote & "," 
			...
			rowData = rowData & quote & FormatNumEx(rs.Fields("LotPrice"),4) & quote & ","  
			rowData = rowData & quote & FormatNum(rs.Fields("LotQty")) & quote & "," 
			rowData = rowData & quote & FormatNum(rs.Fields("SettlementGrossAmount")) & quote & "," 		
			rowData = rowData & quote & FormatNum(Commission) & quote & "," 
			rowData = rowData & quote & FormatNum(rs.Fields("SettlementAmount")) & quote 		
			rowData = rowData & "]" 
			'build the row IDs array 
			rowIDs = rowIDs & quote & rs.Fields("Contract_DPA_") & quote 
			rowCount = rowCount + 1
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = ["Settlement Date","Trade Date",  "Client","Order No","Contract","Security", "Slip No", "Price", "Quantity","Gross",  
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="SettlementDate:Settlement Date;LotTDate:Trade Date;Account:Client;Order_DPA_:Order No;ContractNumber:Contract;SecurityCode:Security;LotSlipNo:Slip No;LotPrice:Price;LotQty:Quantity;SettlementGrossAmount:Gross;SettlementAmount:Net Amount">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="SettlementDate:Settlement Date;LotTDate:Trade Date;Account:Client;Order_DPA_:Order No;ContractNumber:Contract;SecurityCode:Security;LotSlipNo:Slip No;LotPrice:Price;LotQty:Quantity;SettlementGrossAmount:Gross;SettlementAmount:Net Amount">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="SettlementDate:Settlement Date;LotTDate:Date;Account:Client;Order_DPA_:Order No;ContractNumber:Contract;SecurityCode:Security;LotSlipNo:Slip No;LotPrice:Price;LotQty:Quantity;SettlementGrossAmount:Gross;SettlementAmount:Net Amount">
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:25em;width:30em">
