   ...
   Dim rs
	...
	if action = "EXECUTE" then	  
       ...
       ID = Request("ID")
		...
		SqlStr = "select offeringDPA from tblApplDetails Where (ScdID =" & ID & ")"
		...
		do while Rs.eof=false
		 conn.Execute("UPDATE offerings set Paid=0 Where  Offering_DPA_=" & Rs("offeringDPA"))		 
		Rs.MoveNext
		loop			        
         
                
		conn.Execute("Delete From tblschedule where ScheduleID=" & ID)
		conn.Execute("Delete From tblApplDetails where ScdID = " & ID)
