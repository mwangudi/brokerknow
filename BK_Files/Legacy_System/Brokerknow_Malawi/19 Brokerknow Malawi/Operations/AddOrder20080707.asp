...
function updateCDA(){
	...
	//alert(client[client.selectedIndex].Iscustodian);
	...
	if action = "EXECUTE" then					
		...
		if instr(1,buttonAction,"SAVE") > 0 Or instr(1,buttonAction,"ADD") > 0 then
				...
				if cdate(orderDate) < Date() then%>
				 ...
				 sqlStr = "INSERT INTO [tbOrder] (OrderDate,OrderHold,OrderRef,Order_DPA_,Order_EIT_,Branch_DPA_,OrderSecType_DPA_,Client_DPA_,OrderType_DPA_,OrderAutoReleaseDate,OrderHoldType_DPA_,OrderCompounded,InterBank,ChangedBy,Iscustodian) SELECT " & "#" & FormatDate(orderDate) & "#" & " as OrderDate," & " " & hold & " " & " as OrderHold" & _
...
<select name = 'cboOrderSecType' id = 'cboOrderSecType' size="1" currentSecType = "S" onchange='UpdateSecurityListing(this)'>
  ...
  <tr>
    ...
    <select name = 'cboClient' id = 'cboClient' size="1" 
			...
			onChange="UpdateCode(true,cboClient,txtClientCode);updateCDA();"
			onfocus="txtval = '';inputIsItemCode = 1;" 
			onblur="txtval = '';inputIsItemCode = 1;" readonly>
    	<option selected SearchCode = "" SearchText = "" value = '' Iscustodian='0'></option>
		<%
		dim ClientName
		dim NameClient
        ...
        If Not (rs.EOF Or rs.BOF) Then
			    ...
			    NameClient=rs.Fields("Client_DPA_") & " " & Mid(ClientName,1,20)
					    %>                    
                        <option SearchCode = "<%=rs.Fields("Client_DPA_")%>" SearchText = "<%=rs.Fields("ClientName")%>" value = '<%=rs.Fields("Client_DPA_")%>' Iscustodian='<%=rs.Fields("isCustodian")%>'><%=ClientName%></option>
                        <%rs.MoveNext
                Loop
        End If
		%>
  ...
  <tr>
   ...
   <tr>
    <td>CDA</td>
    <td><input type=checkbox   value='1' name='chkCDA' > 
      </td>
  </tr>
  <tr>
  <td colspan = '2'>
  ...
  <table border="0">
    <tr>
	  ...
	  <td ><b><font color="#000080">Limit</font></b></td>
...
%>
	...
	<td>	
	  ...
	  <td><input type = 'text' name ='txtLimit' id = 'txtLimit' size="9" value="0" disabled OnBlur="JavaScript: formatnumber(this)"></td>
      ...
      <td>
	  <input type = 'text' name ='txtAmount' id = 'txtAmount' size="9" disabled OnBlur="JavaScript: format2Number(this)"></td>
