...
'======================= Begin_Alter_Across_Entities =================================
		...
		const DataSource = "UnSubmitOfferingList"
		const ActionPage = "UnSubmitOfferingList"
		const DataEntity = "SubmitOffering"
		const DataEntityPlural = "SubmitOfferingLists"
...
<html>
	...
	<form name='frm<%=ActionPage%>' method='post' id='frmMain' action='SubmitOffering.asp'>
			<!-- grid data -->
		<% 'row data
		Dim rowCount
		Dim orderHeld
...
'======================= Begin_Alter_Across_Entities =================================						
			if rs.Fields("Submitted")=True then
					orderHeld = "<input type=checkbox class='BorderLess' ReleaseDate = '" & rs.Fields("Submission_Date") &  "' style=BorderLess  value='False' name='chkHold' onClick = 'UpdateHoldStatus(this, " & rs.Fields("Offering_DPA_") & ");'>"
			else
					orderHeld = "<input type=checkbox class='BorderLess' ReleaseDate = '" & rs.Fields("Submission_Date") &  "' style=BorderLess checked value='True' name='chkHold' onClick = 'UpdateHoldStatus(this, " & rs.Fields("Offering_DPA_") & ");'>"
			end if
			...
			rowData = rowData & quote & rs.Fields("ID_No") & quote & "," 
			rowData = rowData & quote & rs.Fields("Offering_Price") & quote & ","
			rowData = rowData & quote & rs.Fields("Alloted_Rights") & quote & "," 						
			rowData = rowData & quote & rs.Fields("Amount_Payable") & quote & ","
			rowData = rowData & quote & rs.Fields("Accepted_Rights") & quote & "," 
			rowData = rowData & quote & rs.Fields("Renouncee") & quote & "," 
			rowData = rowData & quote & rs.Fields("PaymentReceiptNo") & quote & ","
			rowData = rowData & quote & rs.Fields("PaymentReference") & quote & "," 									
			rowData = rowData & quote & FormatDate(rs.Fields("Submission_Date")) & quote & "" 		
			rowData = rowData & "]" 
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = ["Submit",   
			"PAL NO", "Code", "Client", "Offering", "ID_No","Price","Alloted","Payable","Accepted","Renouncee","Receipt No","Cheque No","Submission Date"];
		</script>
<%'======================= End_Alter_Across_Entities =================================%>			
		<script>
			//data
			...
			{
				var holdVal = "0"; //order to be held
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="Submission_Date:Date*3;PAL_No:PAL NO*0;Client_DPA_:Client Code*0;ClientName:Client Name*0;SecurityName:Offering Name*0;Id_No:ID NO*0;Offering_Price:Offering Price*1;Alloted_Rights:Alloted Rights*0;Amount_Payable:Amount Payable*0;Accepted_Rights:Accepted Rights;Renouncee:Renouncee*0;PaymentReceiptNo:Receipt No*0;PaymentReference:Cheque No*0">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="Submission_Date:Date;PAL_No:PAL NO;Client_DPA_:Client Code;ClientName:Client Name;SecurityName:Offering Name;Id_No:ID NO;Offering_Price:Offering Price;Alloted_Rights:Alloted Rights;Amount_Payable:Amount Payable;Accepted_Rights:Accepted Rights;Renouncee:Renouncee;PaymentReceiptNo:Receipt No;PaymentReference:Cheque No">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="OrderDate:Date*3;OrderTypeName:Type*0;Order_DPA_:Order No*0;OrderRef:Reference*0;Client_DPA_:Client Code*0;ClientName:Client*0;OrderAutoReleaseDate:Release Date*1;OrderHold:Released*6">
