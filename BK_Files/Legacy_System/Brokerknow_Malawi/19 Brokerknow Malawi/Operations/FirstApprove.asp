...
'======================= End_Alter_Across_Entities =================================		
		...
		sqlStr = "SELECT     MAX(PaymentRequests.Request_DPA_) AS Request_DPA_, PaymentRequests.Client_DPA_, Client.ClientName,  " & _
				...
				"                       PaymentTypes ON PaymentRequests.PaymentType_DPA_ = PaymentTypes.PaymentTypes_DPA_ LEFT OUTER JOIN " & _
				"                       Account ON PaymentRequests.BankAccount_DPA_ = Account.Account_DPA_ " & _
				" WHERE     (PaymentRequests.Deleted = 0) AND (PaymentRequests.Approved = 0) and FirstApproval<>1 and BankAccount_DPA_ is not null" & _
				...
				"                       PaymentTypes.Description, PaymentRequests.PaymentReference, PaymentRequests.Approved, PaymentRequests.Processed_DPA_ " & _
				" ORDER BY PaymentRequests.RequestDate DESC"
		...
		sqlStr = "SELECT     MAX(PaymentRequests.Request_DPA_) AS Request_DPA_, PaymentRequests.Client_DPA_, Client.ClientName,  " & _
				...
				"                       PaymentTypes ON PaymentRequests.PaymentType_DPA_ = PaymentTypes.PaymentTypes_DPA_ LEFT OUTER JOIN " & _
				"                       Account ON PaymentRequests.BankAccount_DPA_ = Account.Account_DPA_ " & _
				" WHERE     (PaymentRequests.Deleted = 0) AND (PaymentRequests.Approved = 0) and FirstApproval<>1  and BankAccount_DPA_ is not null and " & filterStr & " " & _
				" GROUP BY PaymentRequests.Client_DPA_, Client.ClientName, Users.Surname, Users.OtherNames, Account.AccountName, cast(floor(cast(PaymentRequests.PaidDate as float)) as Datetime),  " & _
				"                       PaymentTypes.Description, PaymentRequests.PaymentReference, PaymentRequests.Approved, PaymentRequests.Processed_DPA_ " & _
				" ORDER BY PaymentRequests.RequestDate DESC"
		...
		If searchStr <> "" Then
			...
			sqlStr = "SELECT     MAX(PaymentRequests.Request_DPA_) AS Request_DPA_, PaymentRequests.Client_DPA_, Client.ClientName,  " & _
				...
				"                       PaymentTypes ON PaymentRequests.PaymentType_DPA_ = PaymentTypes.PaymentTypes_DPA_ LEFT OUTER JOIN " & _
				"                       Account ON PaymentRequests.BankAccount_DPA_ = Account.Account_DPA_ " & _
				" WHERE     (PaymentRequests.Deleted = 0) AND (PaymentRequests.Approved = 0) and FirstApproval<>1  and BankAccount_DPA_ is not null and " & searchStr & " " & _
				" GROUP BY PaymentRequests.Client_DPA_, Client.ClientName, Users.Surname, Users.OtherNames, Account.AccountName, cast(floor(cast(PaymentRequests.PaidDate as float)) as Datetime) ,  " & _
				"                       PaymentTypes.Description, PaymentRequests.PaymentReference, PaymentRequests.Approved, PaymentRequests.Processed_DPA_ " & _
				" ORDER BY PaymentRequests.RequestDate DESC"
...
<html>
	...
	<form name='frm<%=ActionPage%>' method='post' id='frmMain' action='FirstApproval.asp''>
			<!-- grid data -->
		<% 'row data
		Dim rowCount
		quote = chr(34) 
...
'======================= Begin_Alter_Across_Entities =================================
			...
			rowData = rowData & quote & FormatDateTime(rs.Fields("TimeModified"),vbshorttime) & "  " & FormatDate(rs.Fields("TimeModified")) & quote   
