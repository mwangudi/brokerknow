...
<%
   ...
   Dim rs
	...
	if action = "EXECUTE" then
        ...
        'save data
        sqlStr = "INSERT INTO [Activity] (ActivityDate,ActivityNotes,Activity_DPA_,ActvtyClass_DPA_,Client_DPA_) SELECT " & "#" & FormatDate(actDate) & "#" & " as ActivityDate," & "'" & Notes & "'" & " as ActivityNotes" & _
...
<form name = 'frmAddActivity' method = 'post' action = 'AddActivity.asp'>
...
<%
        ...
        If Not (rs.EOF Or rs.BOF) Then
                rs.MoveFirst
                Do Until rs.EOF%>
                        <option SearchCode = "<%=rs.Fields("Client_DPA_")%>" SearchText = "<%=rs.Fields("ClientName")%>" value = '<%=rs.Fields("Client_DPA_")%>'><%=rs.Fields("ClientNameEx")%></option>
                        <%rs.MoveNext
                Loop
        End If
%>
...
%>
  ...
  <tr>
    <td width="100%" COLSPAN=2 align="right" valign=absBottom>
		...
		&nbsp;
		<input type = 'button' Class=Buttons name ='cmdClose' id = 'cmdClose' value=" Cancel " OnClick="JavaScript: window.close()">		
		&nbsp;
