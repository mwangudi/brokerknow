...
<title>Contract Settlement</title>
...
<SCRIPT language=Javascript src="Calendar/calendar.js"></SCRIPT>
 ...
 function ItemSelected(itemID)
 		frmContractSettlement.elements("ID").value = itemID
 		frmContractSettlement.submit
 end function
 </script>
</head>
		...
		<TABLE class="ListNuggetHeader" cellPadding="0" cellSpacing="0" width="100%" name="AdvSearchtestHeader"> 
			...
			<TD class="ListNuggetTitleCellWhite"
					...
					<A class=ListNuggetTitle onclick="return PartWrapperToggle('AdvSearchHead');"  
					 href="javascript:PartWrapperToggle('AdvSearchHead');">Contract Settlement
					</A>
				</TD>
   ...
   Dim rs
	...
	if action = "EXECUTE" then
       ...
       ID = Request.Form("ID")
		...
		If Trim(ID) = "" Then%>
                <script language = 'vbscript'>
                		MsgBox "No contract specified for Settlement"
        ...
        'save data
        sqlStr = "UPDATE [Contract] SET ContractSettlementDate = " & "#" & date & "#" &  _
...
<tr><td>
<form name = 'frmContractSettlement' method = 'post' action = 'ContractSettlement.asp' >
<table border="0" width="100%">
 ...
 <%
	...
	Set conn = GetActiveConnection("KBroker")
 	sqlStr = "SELECT * FROM PendingSettlementList"			
	Set rs = conn.Execute(SQLServerFormat(HandleQuote(sqlStr)))
     If Not(rs.EOF Or rs.BOF) Then
        'rs.MoveFirst
        Do Until rs.EOF%>
        		<tr>
        			<td><a href = 'vbScript:ItemSelected(<%=rs.Fields("Contract_DPA_")%>)'>Settle</a></td>
