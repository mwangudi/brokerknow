...
'======================= Begin_Alter_Across_Entities =================================
		...
		const DataSource = "ActivityList"
		const DataEntity = "Activity"
		const DataEntityPlural = "Activities"
...
'======================= Begin_Alter_Across_Entities =================================
        if isdate(rs.Fields("ActivityDate")) then
			ActivityDate =  FormatDate(rs.Fields("ActivityDate"))
		else
			ActivityDate=""
		end if
		...
		if isdate(rs.Fields("TimeChanged")) then
			...
			'row ID 
			rowData = rowData & quote & rs.Fields("Activity_DPA_") & quote & " : " 
			
			'row data 
			rowData = rowData & "[" 
			rowData = rowData & quote & ActivityDate & quote & "," 			
			rowData = rowData & quote & rs.Fields("Client_DPA_") & quote & "," 
			rowData = rowData & quote & rs.Fields("ClientName") & quote & "," 
			rowData = rowData & quote & rs.Fields("ActivityName") & quote & "," 						
			rowData = rowData & quote & replace(rs.Fields("Notes"),vbCrLf," ,") & quote & ","
			rowData = rowData & quote & rs.Fields("ActivityName") & quote & "," 						
			...
			rowIDs = rowIDs & quote & rs.Fields("Activity_DPA_") & quote 
			rowCount = rowCount + 1
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = ["Date", "Code","Client", "Activity","Notes","Status","Last Modified By","Time Changed"];
		</script>
<%'======================= End_Alter_Across_Entities =================================%>			
		<script>
			//data
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="Client_DPA_:Client Code*0;ClientName:Client*0;ActivityName:Activity*0;ActivityDate:Date*3">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="Client_DPA_:Client Code;ClientName:Client;ActivityName:Activity;ActivityDate:Date">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="Client_DPA_:Client Code*0;ClientName:Client*0;ActivityName:Activity*0;ActivityDate:Date*3">
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:18em;width:30em">
