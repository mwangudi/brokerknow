   ...
   Dim rsEdit
	...
	select case action 
		case "EXECUTE_HEADER"
			...
			Iscustodian =Request.Form("chkCDA")	
			
			if trim(Iscustodian)="" then Iscustodian="0"
...
'                        " WHERE Order_DPA_  = " & ID
	...
	Function GetOrderDetailID(OrderID)
		Dim getRs
		Set getConn = GetActiveConnection("KBroker")
		sqlStr = "SELECT OrdDetailList.OrdDetail_DPA_ FROM OrdDetailList WHERE " & _
                "  OrdDetailList.Order_DPA_=" & OrderID
                
        set getRs = getConn.Execute(sqlStr)
        If Not (getRs.EOF OR getRs.BOF) Then
			GetOrderDetailID = getRs("OrdDetail_DPA_")
		Else
			GetOrderDetailID = detailID
		End If	        
...
<script language="javascript">
	...
	function setOpener()
		{
			//alert ()
			window.self.opener = window.parent.dialogArguments.opener;
		}
</script>
<script language='vbscript'>				
...
<body Class="Dialog" SCROLL="NO" onLoad="setOpener();">
...
<%
        ...
        sqlStr = "SELECT tbOrder.OrderCompounded,tbOrder.InterBank, tbOrder.OrderDate, tbOrder.OrderHold, tbOrder.OrderRef, tbOrder.Order_DPA_,tbOrder.IsCustodian, tbOrder.OrderAutoReleaseDate,  " & _
		...
		sqlStr = "SELECT     tbOrder.OrderCompounded, tbOrder.InterBank, tbOrder.OrderDate, tbOrder.OrderHold, tbOrder.OrderRef, tbOrder.Order_DPA_, tbOrder.IsCustodian,  " & _
				 "                       tbOrder.OrderAutoReleaseDate, BranchList.BranchName, ClientList.ClientName, OrderSecTypeList.OrderSecTypeDisplayName,  " & _
				 "                       OrderTypeList.OrderTypeName, OrderTypeList.OrderTypeSale, OrderHoldTypeList.OrderHoldTypeName,                    isnull(   ClientBalances.CurrentBal,0) + isnull(ClientList.CreditLimit,0)- isnull(ClientTotal.Total,0) as Total" & _
				 ...
				 "                       tbOrder ON BranchList.Branch_DPA_ = tbOrder.Branch_DPA_ ON ClientList.Client_DPA_ = tbOrder.Client_DPA_ ON  " & _
				 "                       OrderSecTypeList.OrderSecType_DPA_ = tbOrder.OrderSecType_DPA_ ON OrderTypeList.OrderType_DPA_ = tbOrder.OrderType_DPA_ ON  " & _
				 "                       ClientTotal.Client_DPA_ = tbOrder.Client_DPA_ LEFT OUTER JOIN " & _
				 "                       ClientBalances ON tbOrder.Client_DPA_ = ClientBalances.client_DPA_ LEFT OUTER JOIN " & _
				 "                       OrderHoldTypeList ON tbOrder.OrderHoldType_DPA_ = OrderHoldTypeList.OrderHoldType_DPA_ " & _
				 " WHERE     (tbOrder.Order_DPA_ = " & ID & ")"
...
<table border="0" width="100%">
  <tr>
    ...
    <td width="35%"><input readonly = 'true' class=readonly STYLE="WIDTH: 100px;" type = 'text' name ='txtOrderSecType' id = "txtOrderSecType" value = '<%=rs.Fields("OrderSecTypeDisplayname")%>' size="20">
	 ...
	 <td width="15%">Available Credit</td>
    <td width="35%"><input readonly = 'true' class=readonly STYLE="WIDTH: 100px;" type = 'text' name ='txtAvailCredit' id = "txtAvailCredit" value = '<%=formatnum(rs.Fields("Total"))%>' size="20">
    </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<td>CDA</td><td><input type="checkbox" name ="chkCDA" value="1" <%if cbool(rs("IsCustodian"))= true then response.write "checked" %>></td>
  </tr>
  <tr><td></td><td></td>
  
    <td width="15%" style="display:none">Inter Bank&nbsp;</td>
  ...
  <tr>
    ...
    <!-- <input type = 'button' Class=Buttons name ='cmdPrint' id = 'cmdPrint' value=" Print  " onClick="javascript: window.parent.location.href='OrderForm.asp?order_id=<%'=ID%>'"> -->
	 ...
	 <input type = 'button' Class=Buttons name ='cmdPrint' id = 'cmdPrint' value=" Print  " onClick="javascript: window.parent.location.href='OrderForm.asp?order_id=<%=ID%>&OrdDetailID=<%=GetOrderDetailID(ID)%>&returnPath=EditOrder.asp'">
     <input type = 'button' Class=Buttons name ='cmdCancel' id = 'cmdCancel' value=" Close " onClick="window.opener.location= window.opener.location;window.close();">
