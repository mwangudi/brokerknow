	...
	if action = "EXECUTE" then
       ...
       ID = Request("ID")
		...
		if isnull(rs.fields("BrokerReceiptVoucher_DPA_")) then
				...
				voucher = rs.fields("BrokerReceiptVoucher_DPA_")
				voucherType = 3
		end if
				...
				if voucher <> 0 then
						if (voucherType = 3) then
								...
								If Not (rs.BOF Or rs.EOF) Then
								        ...
								        Do Until rs.EOF
								        		tableName = rs.Fields("Child")
								                sqlStr = "SELECT TOP 1 * FROM [" & tableName & "] WHERE BrokerReceiptVoucher_DPA_ = " & voucher
								...
								'delete from database
								sqlStr = "DELETE FROM [BrokerReceiptVoucher] WHERE BrokerReceiptVoucher_DPA_ = " & receiptVoucher
								conn.Execute SQLServerFormat(HandleQuote(sqlStr))
						end if
				end if
		conn.CommitTrans
