...
<%
	...
	case "EXECUTE" 
	   Dim palno
					...
					conn.BeginTrans		
							...
							if instr(1,buttonAction,"REJECT") > 0 then		
							sqlStr = "INSERT INTO [PaymentRequests] (Request_DPA_,Client_DPA_, " & _
									"PaymentAmount,RequestNarrative,RequestPayDate,ModifiedBy,CreatedBy,Approved,Contract_DPA_,ApprovedBy) " & _
									...
									"," & " " & Amount & " " & " as PaymentAmount" & _						                                    
									"," & "'" & narrative & "'" & " as RequestNarrative" & _				
									"," & "#" & PayDate & "#" & " as RequestPayDate" & _
									"," & " " & UserId & " " & " as ModifiedBy" & _	
									"," & " " & UserId & " " & " as CreatedBy" & _		
									",2 as Approved " & _
									"," & " " & Contract & " " & " as Contract_DPA_" & _
									"," & " " & UserId & " " & " as ApprovedBy" & _
									" FROM [PaymentRequests]"			
							else
							sqlStr = "INSERT INTO [PaymentRequests] (Request_DPA_,Client_DPA_, " & _
									"PaymentAmount,RequestNarrative,RequestPayDate,ModifiedBy,CreatedBy,Contract_DPA_,ApprovedBy) " & _
									" SELECT " & " " & "iif(isnull(max(Request_DPA_)),(SELECT InitialID FROM _Initial_Table_ID_ WHERE TableName = 'PaymentRequests'),max(Request_DPA_) + 1)" & " " & " as Request_DPA_" & _									
									"," & "" & ClientName & "" & " as Client_DPA_" & _
									"," & " " & Amount & " " & " as PaymentAmount" & _						                                    
									"," & "'" & narrative & "'" & " as RequestNarrative" & _				
   	...
   	sqlStr = "SELECT     Client.Client_DPA_, Client.ClientCDSNo, Client.ClientName, tbOrder.Order_DPA_,lot.Contract_DPA_, Security.SecurityCode, ClientBalances.CurrentBal,  " & _
				...
				"                        Contract.ContractSettlementDate " & _
				...
				"                       Contract ON Lot.Contract_DPA_ = Contract.Contract_DPA_ " & _
				...
				"                       (tbOrder.PayOption <> 1) AND (PaymentRequests.Request_DPA_ IS NULL) and lot.Contract_DPA_= " & ID & " order by lot.contract_DPA_"
   ...
   SaleNarrative = "Payment Request For " & rsEdit("ContractNumber")
%>
<body Class="Dialog">
...
<SCRIPT language="JavaScript">
	var cal=new ctlSpiffyCalendarBox("cal", "frm<%=DataSource%>", "txtDate","cmdDate","<%= FormatDate(rsEdit("ContractSettlementDate")) %>",1);
</SCRIPT>

<form name = 'frmAddOffering' method = 'post' id="frmMain" action = "EditSaleRequest.asp" >
	<table border="0" width="100%" cellpadding=2 cellspacing=2>
		<tr>
           <td>Client</td>
		   <td>&nbsp;
				<input type = 'text' name ='txtClientCode' id = 'txtClientCode' size="10" onBlur="txtval = this.value; selectItem(cboClient);UpdateCodes(true,cboClient,txtCdsNo);UpdateBalances();" readonly class="readonlyex" value="<%=rsEdit("Client_DPA_")%>">&nbsp;
				<input type = 'text' name ='txtCdsNo' id = 'txtCdsNo' size="16" onBlur="txtval = this.value; selectItems(cboClient);UpdateCode(true,cboClient,txtClientCode);UpdateBalances();" readonly class="readonlyex" value="<%=rsEdit("ClientCDSNo")%>">&nbsp;
					<select name = 'cboClient' id = 'cbocboClient' disabled size="1" 
     ...
     <tr>
		...
		<tr>
			...
			<td><input type = 'text' name ='txtAmount' id = 'txtAmount' readonly class="readonlyex" size="25" onblur='UpdatePayable()' value='<%=FormatNum(rsEdit("Amount"))%>'></td>
		</tr>
		<tr>
     ...
     <tr>
		...
		<td><textarea name ='txtNarrative' id = 'txtNarrative' rows="5" cols="30" ><%=SaleNarrative%></textarea>
		</td>     
     </tr>                         
  <tr>
     <td width="100%" colspan="2" align=right>		
		...
		<input type = 'hidden' name ='Contract' id = 'Contract' value="<%=rsEdit("Contract_DPA_")%>">
      </td>
  </tr>
</table>
</form>
