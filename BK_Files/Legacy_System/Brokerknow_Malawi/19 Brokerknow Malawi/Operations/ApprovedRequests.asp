...
'======================= Begin_Alter_Across_Entities =================================
		...
		const ActionPage = "ApprovedRequests"		
		const DataSource = "ApprovedRequestList"
		const DataEntity = "ApprovedRequest"
		const DataEntityPlural = "ApprovedRequests"
...
'======================= End_Alter_Across_Entities =================================		
		...
		sqlStr = "SELECT     MAX(PaymentRequests.Request_DPA_) AS Request_DPA_, PaymentRequests.Client_DPA_, Client.ClientName,  " & _
				...
				"                       MAX(PaymentRequests.TimeModified) AS TimeModified, Account.AccountName, Max(PaymentRequests.PaidDate) as PaidDate, PaymentTypes.Description,  " & _
				...
				"                       Account ON PaymentRequests.BankAccount_DPA_ = Account.Account_DPA_ " & _
				" WHERE     (PaymentRequests.Deleted = 0) AND (PaymentRequests.Approved <> 0) " & _
				" GROUP BY PaymentRequests.Client_DPA_, Client.ClientName, Users.Surname, Users.OtherNames, Account.AccountName," & _
		...
		If filterStr <> "" Then	
			...
			sqlStr = "SELECT     MAX(PaymentRequests.Request_DPA_) AS Request_DPA_, PaymentRequests.Client_DPA_, Client.ClientName,  " & _
				...
				"                       PaymentTypes ON PaymentRequests.PaymentType_DPA_ = PaymentTypes.PaymentTypes_DPA_ LEFT OUTER JOIN " & _
				"                       Account ON PaymentRequests.BankAccount_DPA_ = Account.Account_DPA_ " & _
				" WHERE     (PaymentRequests.Deleted = 0) AND (PaymentRequests.Approved <> 0) and " & filterStr & " " & _
				" GROUP BY PaymentRequests.Client_DPA_, Client.ClientName, Users.Surname, Users.OtherNames, Account.AccountName, " & _
				"                       PaymentTypes.Description, PaymentRequests.PaymentReference, PaymentRequests.Approved, PaymentRequests.Processed_DPA_ " & _
				" ORDER BY PaymentRequests.RequestDate DESC"
		End If
		...
		If searchStr <> "" Then
			...
			sqlStr = "SELECT     MAX(PaymentRequests.Request_DPA_) AS Request_DPA_, PaymentRequests.Client_DPA_, Client.ClientName,  " & _
				"                       SUM(PaymentRequests.PaymentAmount) AS PaymentAmount, MAX(PaymentRequests.RequestDate) AS RequestDate,  " & _
				"                       MAX(PaymentRequests.RequestPayDate) AS RequestPayDate, Users.Surname + ' ' + Users.OtherNames AS ModifiedBy,  " & _
				"                       MAX(PaymentRequests.TimeModified) AS TimeModified, Account.AccountName, PaymentRequests.PaidDate, PaymentTypes.Description,  " & _
				"                       PaymentRequests.PaymentReference, PaymentRequests.Approved, isnull(PaymentRequests.Processed_DPA_,0) as Processed_DPA_ " & _
				" FROM         PaymentRequests INNER JOIN " & _
				"                       Client ON PaymentRequests.Client_DPA_ = Client.Client_DPA_ INNER JOIN " & _
				"                       Users ON PaymentRequests.ModifiedBy = Users.UserID LEFT OUTER JOIN " & _
				"                       PaymentTypes ON PaymentRequests.PaymentType_DPA_ = PaymentTypes.PaymentTypes_DPA_ LEFT OUTER JOIN " & _
				"                       Account ON PaymentRequests.BankAccount_DPA_ = Account.Account_DPA_ " & _
				" WHERE     (PaymentRequests.Deleted = 0) AND (PaymentRequests.Approved <> 0) and " & searchStr & " " & _
				" GROUP BY PaymentRequests.Client_DPA_, Client.ClientName, Users.Surname, Users.OtherNames, Account.AccountName," & _
				"                       PaymentTypes.Description, PaymentRequests.PaymentReference, PaymentRequests.Approved, PaymentRequests.Processed_DPA_ " & _
				" ORDER BY PaymentRequests.RequestDate DESC"
		...
		<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/default.css">
			<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/webparts.css">
				...
				<style> 
					...
					.active-column-0 {width: 80px; text-align: left;}
					.active-column-1 {width: 100px; text-align: left;}
					...
					.active-column-6 {width: 75px; text-align: right;}
					.active-column-7 {width: 90px; text-align: left;}					
					.active-column-8 {width: 70px;  text-align: left;}
					.active-column-9 {width: 150px;}	
...
'======================= Begin_Alter_Across_Entities =================================
			...
			if(Cint(Rs("Approved"))=1) then
				ApprovedMessage = "Approved"	
			else
				ApprovedMessage = "Rejected"
			end if
			
			'rowData = rowData & quote & rs.Fields("Request_DPA_") & quote & " : " 
			rowData = rowData & quote & rs.Fields("Request_DPA_") & "<->" & rs.Fields("Client_DPA_") & "<->" & rs("Processed_DPA_") & quote & " : "
			'row data 
			rowData = rowData & "["
			rowData = rowData & quote & ApprovedMessage & quote & ","
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = ["Status","Payment Date","Bank","Reference","Code","Client Name", "Amount", "Request Date", "Pay Date", 			
