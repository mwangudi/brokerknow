	...
	if action = "EXECUTE" then
	  ...
	  if not(rs.eof or rs.bof) then
		...
		if(Fieldtype=1) then				
						...
						'Calaculate Vat on Commission
						vatComm = levyAmount * (dblVATRate/100.00)
						conn.BeginTrans					
						sqlStr="update LevyContract Set LevyAmount="& RoundPoint05(levyAmount) &",LevyRate="& commission & "," & _
  							 "LevyRatePercentage='" & commission & "%" & "' ,ChangedBy="& UserId & ",TimeChanged=GetDate(),LevyVATAmount=" & RoundPoint05(vatComm )& " where SystemMaintained=11 and Contract_DPA_=" & ID
