...
'======================= Begin_Alter_Across_Entities =================================
		...
		const DataEntityPlural = "Cheque Collections"
...
'======================= Begin_Alter_Across_Entities =================================
			'row ID 
			rowData = rowData & quote & rs.Fields("Payment_DPA_") & quote & " : " 
			
			if not isNull(rs.Fields("ChequeCollectionDate")) then 
				CollectionDate = FormatDate(rs.Fields("ChequeCollectionDate")) 
			else 
				CollectionDate = ""
			end if
			
			if not isNull(rs.Fields("ChequeCollectionModifyDate")) then 
				CollectionModifyDate = FormatDate(rs.Fields("ChequeCollectionDate")) 
			else 
				CollectionModifyDate = ""
			end if
						
			if not isnull(rs.Fields("ChequeCollectionNarrative")) then
			 ChequeCollectionNarrative = Replace(rs.Fields("ChequeCollectionNarrative"),VbCrLf,"")
			else
			 ChequeCollectionNarrative = ""
			end if			
			...
			rowData = rowData & quote & rs.Fields("ChequeNo") & quote & "," 
			rowData = rowData & quote & CollectionDate & quote & "," 
			rowData = rowData & quote & ChequeCollectionNarrative & quote & "," 
			
			rowData = rowData & quote & rs.Fields("OriginalUser") & quote & "," 
			rowData = rowData & quote & rs.Fields("ModifyUser") & quote & "," 
			rowData = rowData & quote & CollectionModifyDate & quote & "," 			
			 
			rowData = rowData & "]" 
			'build the row IDs array 
			rowIDs = rowIDs & quote & rs.Fields("Payment_DPA_") & quote 
			rowCount = rowCount + 1
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = ["Code","Client Name", "Cheque No", "Date",
			                 "Narrative", "Created By", "Last Modified By", "Last Modified Date"];
		</script>
<%'======================= End_Alter_Across_Entities =================================%>			
		<script>
			//data
...
/////==========================  End_CHanged   05 Apr 2004 =============================================================///////////////////////////////
		...
		<script> 
			...
			grid.getLayoutTemplate().action("adjustSize");
				...
				<input type = 'hidden' name ='ActionPage' id = "ActionPage" value = "<%=DataEntity%>.asp">

<%'======================= Begin_Alter_Across_Entities =================================%>
				
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="Client_DPA_:Client Code*0;ClientName:Client Name;ChequeNo:Cheque No;PaymentAmount:Amount">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="Client_DPA_:Client Code;ClientName:Client Name;ChequeNo:Cheque No;PaymentAmount:Amount">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="Client_DPA_:Client Code*0;ClientName:Client Name;ChequeNo:Cheque No;PaymentAmount:Amount">
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:25em;width:35em">
