...
'======================= Begin_Alter_Across_Entities =================================
		...
		const DataSource = "DB_InterBankTransferList"
		const DataEntity = "InterBank"
		const DataEntityPlural = "InterBanks"
...
'======================= End_Alter_Across_Entities =================================		
		...
		'sqlStrOrig = "SELECT distinct [" & DataSource & "].*,ordDetail.Amount,datastream_SecurityPriceList.Price FROM [" & DataSource & "] inner join ordDetail on [" & DataSource & "].ordDetail_DPA_=ordDetail.ordDetail_DPA_ " & _
		 '        	 "  inner join Security on [" & DataSource & "].Security_DPA_=Security.Security_DPA_ inner JOIN dbo.datastream_SecurityPriceList on Security.Security_DPA_=datastream_SecurityPriceList.Security_DPA_where(Rtrim(ordDetailType) like %'Purchase'%)"
		
		'sqlStr = "SELECT Distinct [" & DataSource & "].*,ordDetail.Amount,ordDetail.Best,datastream_SecurityPriceList.Price FROM [" & DataSource & "] inner join ordDetail on [" & DataSource & "].ordDetail_DPA_=ordDetail.ordDetail_DPA_ " & _
		 '        "  inner join Security on [" & DataSource & "].Security_DPA_=Security.Security_DPA_ inner JOIN dbo.datastream_SecurityPriceList on Security.Security_DPA_=datastream_SecurityPriceList.Security_DPA_ order by [" & DataSource & "].Order_DPA_ Desc,ContractNumber Desc"			

		'Response.Write(sqlStr)
		'Response.End
		
		If filterStr <> "" Then
			sqlStr = "SELECT Distinct [" & DataSource & "].*,ordDetail.Amount,ordDetail.Best,dbo.datastream_SecurityPriceList.Price FROM [" & DataSource & "] inner join ordDetail on [" & DataSource & "].ordDetail_DPA_=ordDetail.ordDetail_DPA_ " & _
		         	 "  inner join Security on [" & DataSource & "].Security_DPA_=Security.Security_DPA_ inner JOIN dbo.datastream_SecurityPriceList on Security.Security_DPA_=datastream_SecurityPriceList.Security_DPA_ WHERE [" & DataSource & "]." & filterStr & " order by [" & DataSource & "].Order_DPA_ Desc,ContractNumber Desc	"
		End If
		...
		If searchStr <> "" Then
			...
			Else
				sqlStr = sqlStr = "SELECT Distinct [" & DataSource & "].*,ordDetail.Best,ordDetail.Amount,datastream_SecurityPriceList.Price FROM [" & DataSource & "] inner join ordDetail on [" & DataSource & "].ordDetail_DPA_=ordDetail.ordDetail_DPA_ " & _
		         			      "  inner join Security on [" & DataSource & "].Security_DPA_=Security.Security_DPA_ inner JOIN dbo.datastream_SecurityPriceList on Security.Security_DPA_=datastream_SecurityPriceList.Security_DPA_ WHERE [" & DataSource & "]." & searchStr & " order by [" & DataSource & "].Order_DPA_ Desc,ContractNumber Desc	"
			End If
		End If
		
		If sortQryStr <> "" Then
			If InStr(1, sqlStr, " ORDER BY ", vbTextCompare) > 0 Then					
				sqlStr = sqlStr & " , " & DataSource & "." & sortQryStr	
        ...
        if not (rsCurrent.EOF and rsCurrent.BOF) then
			...
			'Response.write(rsCurrent("Best"))
			'if(rsCurrent("Best")=True) then				
			'	if(rsCurrent("SecurityMktPrice")=0)then
			'	LimitAmount=rsCurrent("Amount")/1.105
			'	else
			'	LimitAmount=rsCurrent("Amount")/(rsCurrent("SecurityMktPrice")*1.105)
			'	end if
			'dim sqlstrLimit
			'sqlstrLimit="update ordDetail set ordDetailQty=" & LimitAmount & ",ordDetailPrice=" & rsCurrent("SecurityMktPrice")*1.1 & " where(ordDetail_DPA_=" & rsCurrent("ordDetail_DPA_") & ")"
		...
		<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/default.css">
			<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/webparts.css">
				...
				<style> 					
					...
					.active-column-10 {width: 30px;text-align: center;}	
					.active-column-11 {width: 80px;text-align: right;}	
					.active-column-12 {width: 90px;text-align: right;}	
					.active-column-13 {width: 90px;}	
					.active-column-14 {width: 50px;}	
					.active-column-15 {width: 60px;text-align: right;}	
					.active-column-16 {width: 50px;text-align: right;}	
					.active-column-17 {width: 40px; text-align: center;}		
					.active-column-18 {width: 80px;}	
					.active-column-19 {width: 80px;text-align: left;}	
					.active-column-20 {width: 110px; text-align: left;}		
					.active-column-21 {width: 100px;}	
					.active-column-22 {width: 60px;text-align: right;}	
					.active-column-23 {width: 60px; text-align: right;}							
					.active-column-24 {width: 120px;}	
...
'======================= Begin_Alter_Across_Entities =================================
			...
			entryID = rs.Fields("Contract_DPA_")' & "<->" & rs.Fields("Lot_DPA_")
			...
			rowData = rowData & quote & ApplyDisplayColor(FormatDate(rs.Fields("InterBankDate"))) & quote & "," 
			rowData = rowData & quote & ApplyDisplayColor(rs.Fields("InterBankReference")) & quote & "," 
			rowData = rowData & quote & ApplyDisplayColor(rs.Fields("InterBankNarrative")) & quote & "," 
			...
			rowData = rowData & quote & ApplyDisplayColor(rs.Fields("BondDescription")) & quote & "," 			
			else			
			...
			if(rs("Best")=True and Rs("Amount")<>0) then
			rowData = rowData & quote & ApplyDisplayColor(FormatNumEx((rs.Fields("Amount")/(rs("Price")*1.105)),0)) & quote & "," 
			else
			...
			rowData = rowData & quote & ApplyDisplayColor(FormatDate(rs.Fields("LotTDate"))) & quote & ","			
			...
			if lcase(rs.Fields("OrdDetailSecType")) = "fixed" Then
					rowData = rowData & quote & ApplyDisplayColor(FormatNumEx(rs.Fields("LotPrice"),4)) & quote & ","
			else
			...
			rowData = rowData & quote & ApplyDisplayColor(rs.Fields("CommissionRate")) & quote & "," 
			rowData = rowData & quote & ApplyDisplayColor(rs.Fields("ClientCDSNo")) & quote 
			rowData = rowData & "]" 
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var colCount = 25;
			var colNames = ["","","","", "", "", "", "", "", "", "", 
			"", "TDate","", "SlipNo", "Quantity", "Price","Broker","","OrderType","OrderSecType",
			"OrderIsSaleType","SecurityID","Commission",""];
			
			var myColumns = ["Contract","Settlement date","Reference","Narrative","Order No","Date", "Type", "Code", "Client", "Security", "Best", 
			 "Limit","Balance", "Trade Date","Slip No", "Quantity", "Price","Broker",
			 "Status","OrderType","OrderSecType","OrderIsSaleType","SecurityID","Commission","Client CDS No"];
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:23em;width:20em">
