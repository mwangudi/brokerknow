...
'======================= Begin_Alter_Across_Entities =================================
		...
		const ActionPage = "PaymentRequests"		
		const DataSource = "PaymentRequestList"
		const DataEntity = "PaymentRequests"
...
'======================= End_Alter_Across_Entities =================================		
		...
		sqlStr = "SELECT     PaymentRequests.Request_DPA_,PaymentRequests.Client_DPA_, Client.ClientName, PaymentRequests.PaymentAmount, PaymentRequests.RequestDate,  " & _
				"                       PaymentRequests.RequestPayDate, Users.Surname + ' ' + Users.OtherNames AS ModifiedBy, PaymentRequests.TimeModified " & _
				" FROM         PaymentRequests INNER JOIN " & _
				"                       Client ON PaymentRequests.Client_DPA_ = Client.Client_DPA_ INNER JOIN " & _
				"                       Users ON PaymentRequests.ModifiedBy = Users.UserID Where PaymentRequests.deleted = 0 " & _
				" AND (PaymentRequests.Approved = 0) and BankAccount_DPA_ is null AND Contract_DPA_ is null ORDER BY PaymentRequests.RequestDate DESC"
		...
		If filterStr <> "" Then	
			...
			sqlStr = "SELECT     PaymentRequests.Request_DPA_,PaymentRequests.Client_DPA_, Client.ClientName, PaymentRequests.PaymentAmount, PaymentRequests.RequestDate,  " & _
				...
				"                       Client ON PaymentRequests.Client_DPA_ = Client.Client_DPA_ INNER JOIN " & _
				"                       Users ON PaymentRequests.ModifiedBy = Users.UserID Where PaymentRequests.deleted = 0 " & _
				" AND (PaymentRequests.Approved = 0) and BankAccount_DPA_ is null AND Contract_DPA_ is null and " & filterStr & " ORDER BY PaymentRequests.RequestDate DESC"
		...
		If searchStr <> "" Then
			If InStr(1, sqlStr, " WHERE ") > 0 Then
				...
				"                       Client ON PaymentRequests.Client_DPA_ = Client.Client_DPA_ INNER JOIN " & _
				"                       Users ON PaymentRequests.ModifiedBy = Users.UserID Where PaymentRequests.deleted = 0 " & _
				" AND (PaymentRequests.Approved = 0) and BankAccount_DPA_ is null AND Contract_DPA_ is null and " & searchStr & " ORDER BY PaymentRequests.RequestDate DESC"
...
'======================= Begin_Alter_Across_Entities =================================
			...
			rowIDs = rowIDs & quote & rs.Fields("Request_DPA_") & quote 
			rowCount = rowCount + 1
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = ["Code","Client Name", "Amount", "Request Date", "Pay Date", 			
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:20em;width:38em">
