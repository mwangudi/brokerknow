		...
		case "EXECUTE_DETAIL"
			...
			conn.BeginTrans
				...
				if itemID = "-1" then
					...
					If Not (levyRS.EOF) Or not(levyRS.BOF) Then
						levyRS.MoveFirst
						Do Until levyRS.EOF
					...
					If Len(UpdatedContractDPA & "") > 0 then
						...
						dblVATRate = levyVATRS.Fields("LevyAmount") 'conn.execute(sqlStr)(0)
...
'======================= End_Alter_Across_Entities =================================
	...
	End if
		...
		rowData = rowData & quote & "<input type='text' name='txtSDate' size=40 value='" & FormatDate(conn.execute("SELECT     CONVERT(varchar(20), CASE WHEN DATEPART(dw, GETDATE() - 0) = 2 THEN getdate() - 0 + 4 WHEN DATEPART(dw, GETDATE() - 0) = 1 THEN getdate() - 0 + 7 - 3 WHEN DATEPART(dw, GETDATE() - 0) = 7 THEN getdate() - 0 + 7 - 2 ELSE getdate() - 0 + 6 END, 106) AS SettlementDate")(0)) & "' OnClick='event.cancelBubble=true;'>" & quote  & ","
