...
<%
	...
	ReconDate = Request.Form("txtReconStartDateDate")
	ReconOpeningBal = Request.Form("txtReconOpeningBal")
	...
	if action = "EXECUTE" then
			...
			if buttonAction = "SAVE" then
					...
					If Not IsNumeric(ReconOpeningBal) then%>
					         <script language = 'vbscript'>
					         		ShowMessage "The Reconciliation Opening Balance can only be numeric"					         		
					         </script>
					         <% ReloadPage(ID)
							 response.end
					End If		 
					...
					conn.BeginTrans
							...
							sqlStr = "INSERT INTO [Account] (AccountCode,AccountTypeLevel1,AccountTypeLevel2,AccountTypeLevel3,AccountName" & _
									"       ,Account_DPA_,AccountOpeningBal,ReconStartDate,ReconOpeningBal) SELECT " & "'" & Code & "'" & " as AccountCode," & " " & Level1 & " " & " as AccountTypeLevel1" & _
									...
									"       ," & " " & CDate(ReconDate) & " " & " as ReconStartDate" & _
									"       ," & " " & Ccur(ReconOpeningBal) & " " & " as ReconOpeningBal" & _	
									"        FROM [Account]"
...
<script language='javascript'>
		...
		var cal=new ctlSpiffyCalendarBox("cal", "frm<%=DataSource%>", "txtReconStartDateDate","cmdDate","<%= FormatDate(Date) %>",1);
...
<form name = 'frm<%=DataSource%>' method = 'post' action = '<%=DataSource%>.asp' id = "frmMain">
  <table border="0" width="100%" cellspacing="1" cellpadding="1">
    ...
    <tr> 
      <td>Reconciliation Opening Balance</td>
      <td><input type = 'text' name ='txtReconOpeningBal' id = 'txtReconOpeningBal' size="20" value="<%=OpeningBal%>"></td>
      <td></td>
    </tr>
    <tr>
      <td>Reconciliation Starting Date</td>
...
%>
    ...
    <tr> 
      <td width="100%" colspan=3 align="right" valign=absBottom> <BR> <BR> <input type = 'button' Class=Buttons name ='cmdAdd' id = 'cmdAdd' value="Save" onclick = "AllowedNavigation()"> 
        <input type = 'button' Class=Buttons name ='cmdCancel' id = "cmdCancel" value="Cancel" onclick = "JavaScript: window.self.close()"> 
        <input type = 'hidden' name ='action' id = 'action' value="Execute"> </td>
		 <input type = 'hidden' name ='buttonAction' id = 'buttonAction' value="Save"> </td>
    </tr>
  </table>
