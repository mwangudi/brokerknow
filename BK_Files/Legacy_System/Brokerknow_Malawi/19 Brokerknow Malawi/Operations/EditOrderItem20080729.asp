	...
	select case action 
		case "EXECUTE_DETAIL"
			...
			sqlstr ="SELECT     TOP 100 PERCENT ISNULL(dbo.ClientBalances.CurrentBal, 0) + ISNULL(dbo.Client.CreditLimit, 0) - ISNULL(dbo.ClientTotal.Total, 0) AS AvailableCredit,  " & _
					"                       dbo.tbOrder.Order_DPA_, dbo.Client.Agent_DPA_ " & _
			...
			if not (rs.eof or rs.bof) then
			 AvailableBalance = Cdbl(trim(rs.fields("AvailableCredit")))
			 AgentDPA = rs.fields("Agent_DPA_")
			end if
			...
			if Cint(orderType1)=1 then
			   ...
			   if trim(OrderAmount) = "" then OrderAmount = 0
			    ...
			    AvailableBalance =  Cdbl(AvailableBalance) + Cdbl(OrderAmount)
			     	
				if Cint(Best) = 0 then
					if AvailableBalance < 0 AND Qty > 0 then
					  %>
						<script language = 'vbscript'>
							ShowMessage "Order cannot be registered! Insufficient funds available."
						</script>
						<% response.end
					end if
				end if
