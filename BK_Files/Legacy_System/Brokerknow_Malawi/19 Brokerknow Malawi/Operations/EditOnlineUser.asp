	...
	if action = "EXECUTE" then
       ...
       SecretAnswerConfirm = Request.Form("txtSecretAnswerConfirm")
		...
		if(Isnull(ClientEmail) or ClientEmail="") then
			%>
			<script language="vbscript">
            		ShowMessage "Login details will not be sent because the client does not have an Email address"
			</script>
			<% 
		end if

		if(Isnull(ClientEmail) or ClientEmail="" or Accepted=0) then
		else
   	...
   	Set rsEdit = Conn.Execute("SELECT * FROM [OnlineUserList] WHERE UserID = " & ID)
...
<form name = 'frmEditUser' method = 'post' action = 'EditOnlineUser.asp' >
