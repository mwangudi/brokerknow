...
if VerifyAction = "" and UCase(Trim(Request.Form("action"))) = "" then
	...
	Response.Write "<select name='AllClients' size='7' style='width: 150px;display: none'>" & vbCrLf
	if not (myRs.BOF or myRs.EOF) then
		do until myRs.EOF
			DataStr = myRs("ClientIDPass") & "|" & myRs("ClientCDSNo") & "|" & myRs("ClientName") & "|" & Cdbl(trim(myRs("Client_DPA_")))
			Response.Write "<option id='" & DataStr & "' value='" & myRs("Client_DPA_") & "'>" & myRs("Client_DPA_") & "</option>" & vbCrLf
			myRs.MoveNext
		loop
	end if
	...
	%>
	<body bgcolor="#878787">
	<table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse;bgcolor: &" bordercolor="#878787" width="75%">
		<tr>
			<td width="100%" colspan="4"><b>Client Verification</b></td>
		</tr>
		<tr>
			<td width="100%" colspan="4">&nbsp;</td>
		</tr>
		<tr>
			<td width="20%" id="ClientCodeTitle" nowrap></td>
			<td width="80%" id="ClientCode" colspan="3"></td>
		</tr>
		<tr>
			<td width="20%" id="IDPassTitle" nowrap></td>
			<td width="80%" id="IDPass" colspan="3"></td>
		</tr>
		<tr>
			<td width="20%" id="UserNameTitle" nowrap></td>
			<td width="80%" id="UserName" colspan="3"></td>
		</tr>
		<tr>
			<td width="20%"><input type="button" name="Previous" value=" << Prev " style="display: none" onClick="javascript: goPrevious()"></td>
			<td width="30%" id="LabelField" nowrap>Enter Client Code</td>
			<td width="40%"><input type="text" name="txtQueryItem" id="txtQueryItem" value="" size="20"></td>
			<td width="10%"><input type="button" name="Next" value=" Next >> " onClick="javascript: goNext()"></td>
		</tr>
		<tr>
			<td width="100%" colspan="4">&nbsp;</td>
		</tr>
			...
			for (var i=0;i<DataCombo.length;i++)
			{
				...
				switch (step)
				{
					...
					case '3':
						...
						{
							window.location.href = "AddOrderFO.asp?v_action=go_on&client=" + ComboData[3];
							increment = true;
							i = DataCombo.length;
						}
						break;
		...
		if instr(1,buttonAction,"SAVE") > 0 Or instr(1,buttonAction,"ADD") > 0 then
				...
				if cdate(orderDate) < Date() then%>
				 ...
				 End If
					'validate detail info
				         ...
				         'validate size of Order Detail Certificate Number
							...
							If (chkprice = "P") And (Not IsNumeric(Price)) Then%>
				         		<script language = 'vbscript'>
				 ...
				 conn.BeginTrans
						...
						'save detail data
						sqlStr = "INSERT INTO [OrdDetail] (OrdDetailCertNo,OrdDetailPrice,Amount,Best,BondDescription,OrdDetailQty,OrdDetailValidity" & _
								",OrdDetail_DPA_,Order_DPA_,Security_DPA_) SELECT " & "'" & cert & "'" & " as OrdDetailCertNo" & _
								"," & "'" & price & "'" & " as OrdDetailPrice" & _
								"," & " " & amount & " " & " as Amount" & _
								"," & " " & Best & " " & " as Best" & _
								"," & "'" & bond & "'" & " as BondDescription" & _
				...
				elseif instr(1,buttonAction,"ADD NEW") > 0 then%>
						...
						<%
						WriteDialogRelocateScript "AddOrderFO.asp"
				else
...
<body Class="Dialog" onLoad="javascript: document.all.item('txtClientCode').value = <%=FilteredClient%>; selectItem(<%=FilteredClient%>)">

<div id="spiffycalendar" class="text" STYLE="z-Index: 1000"></div>


<form name = 'frmAddOrder' method = 'post' action = 'AddOrderFO.asp' id = 'frmMain'>
...
<select name = 'cboOrderSecType' id = 'cboOrderSecType' size="1" currentSecType = "S" onchange='UpdateSecurityListing(this)'>
  ...
  <tr>
    <td>Client</td>
    <td>
    <input type = 'text' name ='txtClientCode' id = 'txtClientCode' size="10" onBlur="txtval = this.value; selectItem(cboClient);" readonly>
    <select name = 'cboClient' id = 'cboClient' size="1" 
        ...
        If Not (rs.EOF Or rs.BOF) Then
                rs.MoveFirst
                Do Until rs.EOF%>
                        <option SearchCode = "<%=rs.Fields("Client_DPA_")%>" SearchText = "<%=rs.Fields("ClientName")%>" value = '<%=rs.Fields("Client_DPA_")%>'><%=rs.Fields("ClientName")%></option>
                        <%rs.MoveNext
                Loop
        End If
		%>
...
%>
	...
	<td>	
      ...
      <td><SCRIPT language="JavaScript">calValidity.writeControl();</SCRIPT></td>
    </tr>
  </table>
...
<table>
    ...
    <td width="20%" colspan=4 align=right>
        ...
        If Not (rs.EOF Or rs.BOF) Then
                ...
                Do Until rs.EOF
					%>
					<option id='<%=rs("Security_DPA_")%>' value='<%=rs.Fields("Bond_DPA_")%>'><%=rs.Fields("Issue")%></option>
					<%
					rs.MoveNext
                Loop
        End If
