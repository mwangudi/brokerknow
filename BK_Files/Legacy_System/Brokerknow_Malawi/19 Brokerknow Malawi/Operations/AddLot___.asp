		...
		if buttonAction = "SAVE" then
			...
			'ensure Order Detail Estimated Price is numeric
				...
				settlementDate=LTdate(CDate(tDate),NoOfDays)
				 ...
				 sqlStr = "INSERT INTO [Contract] (Contract_EIT_, Status_DPA_,IsInterBank,ContractSettlementDate) " & _
				          " VALUES(#" & guidStr & "#" & _
				          ",1" & _
				          "," & interbank & _
						  ",#" & ContractSettlementDate & "#" & _
				          ")"
				 ...
				 conn.BeginTrans
						...
						sqlStr = "SELECT Contract_DPA_ FROM Contract WHERE Contract_EIT_ = " & "'" & guidStr & "'"
				     ...
				     ContractDPA=rs("Contract_DPA_")
						...
						<%
						'Check for unique slip no/ CDS Reference nos
							sqlstr = "SELECT     dbo.Lot.LotSlipNo " & _
									...
									"                           (SELECT     dbo.tbOrder.OrderType_DPA_ " & _
									"                             FROM          dbo.OrdDetail INNER JOIN " & _
									"                                                    dbo.tbOrder ON dbo.OrdDetail.Order_DPA_ = dbo.tbOrder.Order_DPA_ " & _
									"                             WHERE      (dbo.OrdDetail.OrdDetail_DPA_ = " & ID & ")) OrdType ON dbo.tbOrder.OrderType_DPA_ = OrdType.OrderType_DPA_ " & _
									" WHERE     (dbo.Lot.Deleted = 0) AND (dbo.Lot.LotSlipNo = '" & slip & "')"
									
                            set rsunique = conn.execute(sqlstr)
                            
                            if not (rsunique.eof or rsunique.bof) then
								%>
				         			<script language = 'vbscript'>
				         					ShowMessage "The slip number already exists! Slip number must be unique to proceed."
						...
						' Get Maximum CDSImport_DPA_
						
							sqlstr = "SELECT     Max(dbo.Lot.CDSImport_DPA_) + 1 as CDSImport_DPA_ " & _
									" FROM         dbo.Lot "
									
                            set rsunique = conn.execute(sqlstr)
                            
                           ' CDSImportDPA = rsunique("CDSImport_DPA_")					
						
				
						sqlStr = "INSERT INTO [Lot] (Contract_DPA_,OrdDetail_DPA_,LotPrice" & _
								 ",LotQty,LotSlipNo,UniqueSlip,LotTDate,Broker_DPA_,ContractNumber, LotGrossAmount,CDSTransaction,ChangedBy)" & _
								 " VALUES(" & rs.fields("Contract_DPA_") & _
								 "," & ID & _
								 "," & price & _
								 "," & qty & _
								 "," & slip & _
								 ",'" & left(orderType,1) & slip & "'" & _							 			 
								 ",#" & FormatDate(tDate) & "#" & _
								 "," & broker & _
								 ",'" & left(orderType,1) & rs.fields("Contract_DPA_") & "'" & _								
								 "," & grossAmount & _
								 "," & CDSFlag & _
								 "," & UserId & _
								 ")"
				     'response.write sqlstr : response.end
						...
						Set levyRS = conn.Execute(sqlStr)
						...
						If Not (levyRS.EOF Or levyRS.BOF) Then													
								Do Until levyRS.EOF
										...
										if abs(levyRS.Fields("Vatable")) = 1 then 
											vatAmount= vatAmount + (levyAmount *levyVATRS("LevyBlock") )
										end if 
										sqlStr = "INSERT INTO [LevyContract] (Contract_DPA_,LevyName,LevyAmount,LevyRate,LevyBlock,SystemMaintained,LevyShortName,LevyRatePercentage) " & _
												 " VALUES(" & rs.Fields("Contract_DPA_") & _
						...
						if Not(orderIsSaleType) then
							...
							Set transRS = conn.Execute(sqlStr)
							
							sqlStr = "INSERT INTO [LevyContract] (Contract_DPA_,LevyName,LevyAmount,LevyRate,LevyBlock,LevyShortName,SystemMaintained) " & _
									 " VALUES(" & rs.Fields("Contract_DPA_") & _
									 ",'Transfer Fee'" & _
						...
						if orderSecType = "Fixed" then 
						    mylevyAmount = Round(levyAmount,2)
						else
						    mylevyAmount = RoundPoint05(levyAmount)
						end if
						
						sqlStr = "INSERT INTO [LevyContract] (Contract_DPA_,LevyName,LevyAmount,LevyRate,LevyBlock,LevyShortName,LevyRatePercentage,SystemMaintained) " & _
								 " VALUES(" & rs.Fields("Contract_DPA_") & _
								 ",'Broker Commission'" & _
								 "," & RoundPoint05(mylevyAmount) & _
								 "," & commission & _
								 ",0" & _
								 ",'Commission'" & _
								 ",'" & commission & "%" & "'" & _
						...
						if secImob then
								...
								if orderIsSaleType then
										...
										Set payRS = conn.Execute(HandleQuote(sqlStr))
										...
										sqlStr = "INSERT INTO [Payment] ( PaymentAmount, PaymentPDate, BankAccount_DPA_, " & _
												"Payment_EIT_,PayType_DPA_, EntityType_DPA_, PaymentReference, " & _
												"PaymentNarrative, Entity_DPA_, Order_DPA_,BrokerReceiptVoucher_DPA_,Contract_DPA_,ChangedBy) " & _
												"VALUES ( " &_ 
												" " & ccur(amount) & " " & _
												"," & "'" & FormatDate(settlementDate) & "'" & _
										...
										sqlStr = "INSERT INTO [Payment] ( PaymentAmount, PaymentPDate, BankAccount_DPA_, " & _
												"PayType_DPA_, EntityType_DPA_, PaymentReference, PaymentNarrative, " & _
												"Entity_DPA_, Voucher_DPA_,ClientVoucher_DPA_,Contract_DPA_,ChangedBy) " & _
												"VALUES ( " & _
												" " & CCur(amount) & " " & _
												"," & "'" & FormatDate(settlementDate) & "'" & _
  ...
  <table border="0" width="100%">
    ...
    <tr>
      ...
      <td width="33%" valign="top"><select name="cboBroker" id="cboBroker" size="1" 
          ...
          <%
		Set conn = GetActiveConnection("KBroker")
        sqlStr = "SELECT BrokerCode,BrokerName,Broker_DPA_,BrokerNameEx FROM [BrokerList] Order By BrokerCode"
        Set rs = conn.Execute(sqlStr)
