   ...
   'WriteDialogRelocateScript "AddOffering.asp" 	
	...
	case "EXECUTE" 
					...
					conn.BeginTrans		
							...
							if(Cint(ProcessID)=0) then
								...
								sqlStr = "UPDATE PaymentRequests SET BankAccount_DPA_= " & "" & bank & "" & "," & _
										 ...
										 "ApprovedBy = " & "" & UserId & "" & "," & _
										 "ApprovalDate = GetDate() " & _									 
										 " Where Processed_DPA_ = " & ProcessID
								else
								sqlStr = "UPDATE PaymentRequests SET BankAccount_DPA_= " & "" & bank & "" & "," & _
										 "PaymentType_DPA_ = " & "" & PaymentType & "" & "," & _
										 "ModifiedBy = " & "" & UserId & "" & "," & _
   	...
   	sqlstr ="SELECT     Client.ClientCDSNo,MAX(PaymentRequests.Request_DPA_) AS Request_DPA_, PaymentRequests.Client_DPA_, Client.ClientName,  " & _
			...
			" WHERE     (PaymentRequests.Deleted = 0) AND (PaymentRequests.Approved <> 0) and (PaymentRequests.Client_DPA_ = " & ClientID & ")" & _
...
<form name = 'frmAddOffering' method = 'post' id="frmMain" action = "EditApprovedRequest.asp" >
