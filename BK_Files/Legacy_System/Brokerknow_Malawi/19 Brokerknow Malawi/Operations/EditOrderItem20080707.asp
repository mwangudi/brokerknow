...
<%
 Set conn = GetActiveConnection("KBroker")
	...
	if(trim(from)="") then
	sqlStr = "SELECT  isnull(dbo.OrdDetail.Limit,0) as Limit, dbo.Security.SecurityCode, dbo.OrdDetail.Best, dbo.OrdDetail.Amount, dbo.OrdDetail.OrdDetailCertNo, dbo.Bond.BondIssue AS bondDescription," & _ 
    ...
    sqlStr = "SELECT Distinct isnull(dbo.OrdDetailList.Limit,0), [OrdDetailList].SecurityCode,[OrdDetailList].OrderTypeSale,[OrdDetailList].OrdDetailCertNo,[OrdDetailList].BondDescription,[OrdDetailList].OrdDetailPrice,[FullOrderList].Amount as Amount,[OrdDetailList].OrdDetailQty,[OrdDetailList].OrdDetailValidity,[OrdDetailList].OrdDetail_DPA_," & _
             "	[OrdDetailList].SecurityName,[OrdDetailList].Security_DPA_,[OrdDetailList].OrderSecType_DPA_,'' as cboprice,'' as CalendarChk,dbo.OrdDetailList.limit,[OrdDetailList].ordDetailSecType,[OrdDetailList].ordDetailType FROM [OrdDetailList] inner join [FullOrderList] on [OrdDetailList].OrdDetail_DPA_ =[FullOrderList].OrdDetail_DPA_  WHERE([OrdDetailList].Order_DPA_ =" & ID & ")"                
    end if
...
<SCRIPT language=Javascript src="Calendar/calendar.js"></SCRIPT>
		...
		<style>
			...
			.active-column-9 {width: 30px;}
			.active-column-10 {width: 110px;}
			.active-grid-row,
	...
	document.body.onload = changeDateInterface;	
		...
		<script language="javascript">
			...
			var colNames = ["", "Security","Bond","Price",  
					 "txtLimitInPlace","txtPriceInPlace", "txtAmountInPlace", "txtQtyInPlace","txtNewCertInPlace","Chk","txtCalendar", ""];
			
			var myColumns = ["Line No", "Security","Bond","",  
					 "Limit","Price", "Amount", "Quantity","Certificate","","Validity", ""];
