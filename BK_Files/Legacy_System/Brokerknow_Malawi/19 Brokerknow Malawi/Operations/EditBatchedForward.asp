...
'======================= End_Alter_Across_Entities =================================
	...
	BatchID = trim(Request("ID"))

	action = Request.Form("action")
	SearchSerial = Request.QueryString("SearchSerial")

	BatchedItems = trim(Request.Form("CommitParams"))	
	...
	if(action ="Save") then
		...
		SearchSerial = trim(Request.Form("txtSearch"))
		WriteDialogRelocateScript "EditBatchedForward.asp?SearchSerial=" & SearchSerial & "&ID=" & BatchID
		else
	...
	WriteDialogRelocateScript "EditBatchedForward.asp?ID=" & BatchID
	Response.End	
	end if	
 end if
	%>
...
<form name = 'frm<%=DataSource%>' method = 'post' action = 'EditBatchedForward.asp' id='frmMain' >
<table>
...
<table border="0" cellspacing="1" cellpadding="1"  width="100%">
  <tr>
	...
	<td  valign="top" nowrap><b><font color="#000080">Offering&nbsp;Name</font></b></td>
...
sqlStr = "SELECT * FROM [" & DataSource & "] where Batch_no=" & BatchID & " order by BatchSeq asc"
        ...
        If MaxRs.EOF Or MaxRs.BOF Then		
			...
			do until MaxRs.eof
				commitTrade = "<input type='checkbox' class='BorderLess' value='" & MaxRs.Fields("Offering_DPA_") & "' name='chkCommit' id='chkCommit' onClick = 'javascript: CommitCDSTrade(this, " & MaxRs.Fields("Offering_DPA_") & ", " & MaxRs.Fields("Offering_DPA_") & "," & MaxRs.Fields("BatchSize") & ");'>"
				...
				<td  valign="top"><%=FormatNumEx(MaxRs("Alloted_Rights"),0)%></td>
				<td  valign="top"><%=FormatNum(MaxRs("Offering_Price"))%></td>
				<td  valign="top"><%=FormatNum(MaxRs("amount_Payable"))%></td>
			    </tr>
			<%
			MaxRs.movenext
			loop
