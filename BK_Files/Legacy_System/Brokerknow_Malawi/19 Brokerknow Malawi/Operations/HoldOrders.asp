...
'======================= Begin_Alter_Across_Entities =================================
		...
		const DataSource = "HoldOrderList"
		const ActionPage = "HoldOrders"
...
'======================= End_Alter_Across_Entities =================================		
		...
		If filterStr = "" AND sortQryStr = "" AND searchStr = "" Then
			...
			sqlStr = sqlStr & " Order By OrderDate DESC" ' Default Sort
		...
		<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/default.css">
			<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/webparts.css">
				...
				<style> 
					...
					.active-column-8 {width: 70px;}
					.active-column-9 {width: 60px;  text-align: center;}										
				</style>
										
</head>
	<body leftMargin=0 topMargin=0 marginheight="0" marginwidth="0"> 
	<form name='frm<%=ActionPage%>' method='post' id='frmMain' action='ReleaseOrder.asp?sourcePage=HoldOrders.asp'>
			<!-- grid data -->
		<% 'row data
		Dim rowCount
		Dim orderHeld
...
'======================= Begin_Alter_Across_Entities =================================
			...
			if rs.Fields("OrderHold") then
					orderHeld = "<input type=checkbox class='BorderLess' ReleaseDate = '" & rs.Fields("OrderDateReleased") &  "' style=BorderLess checked value='True' name='chkHold' onClick = 'UpdateHoldStatus(this, " & rs.Fields("Order_DPA_") & ");'>"
			else
					orderHeld = "<input type=checkbox class='BorderLess' ReleaseDate = '" & rs.Fields("OrderDateReleased") &  "' style=BorderLess  value='False' name='chkHold' onClick = 'UpdateHoldStatus(this, " & rs.Fields("Order_DPA_") & ");'>"
			end if
			
			'row ID 
			
			rowData = rowData & quote & rs.Fields("Order_DPA_") & "<->" & rs("OrdDetail_DPA_") & quote & " : " 
			
			'row data 
			rowData = rowData & "[" 
			rowData = rowData & quote & FormatDate(rs.Fields("OrderDate")) & quote & ","
			rowData = rowData & quote & IIf(rs.Fields("OrderTypeName")="Sale","S","P") & quote & ","
			rowData = rowData & quote & rs.Fields("Order_DPA_") & quote & "," 
			
		    if ucase(trim(rs.Fields("SecurityType"))) = "FIXED" then
              rowData = rowData & quote & FormatDate(rs.Fields("BondIssue")) & quote & "," 	
			else
              rowData = rowData & quote & FormatDate(rs.Fields("SecurityName")) & quote & "," 	
			end if

			rowData = rowData & quote & rs.Fields("OrderRef") & quote & ","
			rowData = rowData & quote & rs.Fields("ClientName") & quote & ","
			
			if Ucase(trim(rs.Fields("OrderTypeName"))) = "SALE" then
			  cash=""
			  rowData = rowData & quote & cash & Quote & ","
			else
			...
			if Ucase(trim(rs.Fields("OrderTypeName"))) = "SALE" then
				...
				if rs.Fields("BalanceFree")="Y" then
				
				  rowData = rowData & quote & FormatNumEx(rs.Fields("Quantity"),0) & Quote & ","
				
				else
					if isnull(rs.Fields("Quantity")) or rs.Fields("Quantity")="" then
				...
				rowData = rowData & quote & Quantity & Quote & ","
				end if
			...
			rowIDs = rowIDs & quote & rs.Fields("Order_DPA_") & "<->" & rs("OrdDetail_DPA_") & quote 
			rowCount = rowCount + 1
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = ["Date",  "Type","Order No", "Security",   
			"Reference", "Client", "Cash","Portfolio", "Release Date","Hold"];
		    </script>
