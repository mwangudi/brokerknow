...
'======================= Begin_Alter_Across_Entities =================================
		...
		const ActionPage = "ProcessedRequests"		
		const DataSource = "ProcessedRequestList"
		const DataEntity = "ProcessedRequest"
		const DataEntityPlural = "ProcessedRequests"
...
'======================= End_Alter_Across_Entities =================================		
		...
		sqlStr = "SELECT     MAX(PaymentRequests.Request_DPA_) AS Request_DPA_, PaymentRequests.Client_DPA_, Client.ClientName,  " & _
				...
				"                       PaymentRequests.PaymentReference " & _
				" FROM         PaymentRequests INNER JOIN " & _
				"                       Client ON PaymentRequests.Client_DPA_ = Client.Client_DPA_ INNER JOIN " & _
				"                       Users ON PaymentRequests.ModifiedBy = Users.UserID LEFT OUTER JOIN " & _
				"                       PaymentTypes ON PaymentRequests.PaymentType_DPA_ = PaymentTypes.PaymentTypes_DPA_ LEFT OUTER JOIN " & _
				"                       Account ON PaymentRequests.BankAccount_DPA_ = Account.Account_DPA_ " & _
				" WHERE     (PaymentRequests.Deleted = 0) AND (PaymentRequests.Approved = 0) " & _
				" GROUP BY PaymentRequests.Client_DPA_, Client.ClientName, Users.Surname, Users.OtherNames, Account.AccountName, PaymentRequests.PaidDate,  " & _
				"                       PaymentTypes.Description, PaymentRequests.PaymentReference " & _
				" ORDER BY PaymentRequests.RequestDate DESC"
		...
		<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/default.css">
			<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/webparts.css">
				...
				<style> 
					...
					.active-column-2 {width: 75px; text-align: right;}					
					.active-column-3 {width: 50px; text-align: left;}
...
'======================= Begin_Alter_Across_Entities =================================
			'row ID 
			rowData = rowData & quote & rs.Fields("Request_DPA_") & "<->" & rs.Fields("Client_DPA_") & quote & " : " 
			
			'row data 
			rowData = rowData & "[" 
			rowData = rowData & quote & FormatDate(rs.Fields("PaidDate")) & quote & ","
			rowData = rowData & quote & rs.Fields("AccountName") & quote & "," 
			rowData = rowData & quote & FormatNum(rs.Fields("PaymentReference")) & quote & ","			
			...
			rowIDs = rowIDs & quote & rs.Fields("Request_DPA_") & "<->" & rs.Fields("Client_DPA_") & quote 
			rowCount = rowCount + 1
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = ["Payment Date","Bank","Reference","Code","Client Name", "Amount", "Post Date", "Pay Date", 			
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:35em;width:38em">
