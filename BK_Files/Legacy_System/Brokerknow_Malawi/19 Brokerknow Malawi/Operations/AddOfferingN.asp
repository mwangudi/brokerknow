...
<%
   ...
   dim first
	...
	if(action="Next") then
	if(Cint(first)<>1) then
	WriteDialogRelocateScript "AddOfferingN.asp?action=" & action & "&first=1&palno=" & palno &"&clientName=" & clientName & _	
	...
	if(action="Previous") then
		if(Cint(first)<>1) then
			WriteDialogRelocateScript "AddOfferingN.asp?first=1&palno=" & palno &"&clientName=" & clientName & _	
			                          "&price=" & price & "&offering=" & offering &"&narrative=" & narrative & "&AlRights=" & AlRights & _
			                          "&AcRights=" & AcRights & "&bank=" & bank & "&Renouncee=" & Renouncee & "&pdate=" & PDate & "&idno=" & idno & _
			                          "&ChkNo=" & ChkNo & "&ChkAmt=" & ChkAmt & _
									  "&payable=" & payable & _
	...
	case "EXECUTE" 
	   ...
	   Dim ENABLED 
		...
		entity=1
		ENABLED = true
		palno = trim(Request.Form("txtpalno"))
		clientName = trim(Request.Form ("cboClient"))
		price = trim(Request.Form("txtprice"))
		offering = trim(Request.Form("cboOfferings"))
		narrative = trim(Request.Form("txtNarrative"))
		AlRights = trim(Request.Form("txtAlloted"))
		AcRights = trim(Request.Form("txtAccepted")) 
		bank = trim(Request.Form("cboBank"))
		Renouncee = trim(Request.Form("txtRenouncee"))
		PDate = Trim(Request.Form("txtDate"))					
		idno=trim(Request.Form("txtidno"))
		ChkNo=trim(Request.Form("txtChkNo"))
		ChkAmt=trim(Request.Form("txtChkAmt"))					
		PaymentTypes = trim(Request.Form("cboPaymentTypes"))
	...
	'Dim palno
					...
					%>
					<script language="javascript">
					alert('This amount Should be aleast equal to the payable amount which is <%=payable%>');
					</script>
					<%
					Response.end
					end if
   ...
   Set conn = GetActiveConnection("KBroker")   
	...
	var cal=new ctlSpiffyCalendarBox("cal", "frm<%=DataSource%>", "txtDate","cmdDate","<%= FormatDate(PDate) %>",1);
   	...
   	<form name = 'frmAddOffering' method = 'post' id="frmMain" action = "AddOfferingN.asp" >
				...
				<input type = 'text' name ='txtCdsNo' id = 'txtCdsNo' size="16" onBlur="txtval = this.value; selectItems(cboClient);UpdateCode(true,cboClient,txtClientCode);UpdateBalances();">&nbsp;
				    ...
				    rs.movefirst
				     ...
				     for intcount = 0 to intcountrs-1
				       ...
				       if clientID = trim(clientName) then
				        %>
				           <option Credit="<%=FormatNumEx(AvailableCredit,2)%>" CurrentBal="<%=FormatNumEx(CreditBal,2)%>" SearchCode = "<%=clientID%>" SearchText = "<%=thisClientName%>" SearchCds = "<%=CDSNo%>"  selected value = '<%=clientID%>'><%=thisClientName%></option>
				        <%
				       else
				        %>
