...
Function DrawPageFunctions(includePrint, includeEmailThisPage, includeBackBtn, includePDF)
	...
	With Response
		...
		If includePDF Then
			''PDF WRITER
			.Write "    <td " & barTdClientScript & "  onclick=""launchMailerPDFDoc()""><img src=""../pdf.gif"" width=""20"" height=""20"">Email PDF" & vbCrLf
			.Write "	<form method='POST' style='position:absolute;visibility:hidden;display:none;' name='frmPDF' id='frmPDF'>" & vbCrLf
			.Write "		<input type=hidden name=to2>" & vbCrLf
			.Write "		<input type=hidden name=cc2>" & vbCrLf
			.Write "		<input type=hidden name=bcc2>" & vbCrLf
			.Write "		<input type=hidden name=subject2>" & vbCrLf
			.Write "		<input type=hidden name=bodyText2>" & vbCrLf
			.Write "		<input type=hidden name=attachment>" & vbCrLf
			.Write "		<input type=hidden name=PDFDoc>"  & vbCrLf
			.Write "		<input type=hidden name=PDFOrient>"  & vbCrLf
			.Write "	</form>" & vbCrLf
			.Write "	<div style='display: none'>" & vbCrLf
			.Write "	<IFRAME marginwidth=0 marginheight=0 FRAMEBORDER=0 ID='hiddenEmailFrame' NAME='EmailPDFFrame'></IFRAME>" & vbCrLf
			.Write "	</div>" & vbCrLf			
			.Write "    </td>" & vbCrLf	
			
			clientSideScript = clientSideScript & "	function emailPDFDoc(){"  & vbCrLf
			clientSideScript = clientSideScript & "		document.all.item('frmPDF').action = '" & sPath & "WriteToPDF.asp';"  & vbCrLf
			clientSideScript = clientSideScript & "		document.all.item('frmPDF').submit();"  & vbCrLf
			clientSideScript = clientSideScript & "	}" & vbCrLf		
			
			clientSideScript = clientSideScript & "	function launchMailerPDFDoc(){"  & vbCrLf
			clientSideScript = clientSideScript & "		var thisPDFDoc = window.toPDF.innerHTML;"  & vbCrLf
			clientSideScript = clientSideScript & "		var thisPDFOrient = window.toPDFOrient.innerHTML;"  & vbCrLf
			clientSideScript = clientSideScript & "		document.all.item('PDFDoc').value = thisPDFDoc;"  & vbCrLf
			clientSideScript = clientSideScript & "		document.all.item('PDFOrient').value = thisPDFOrient;"  & vbCrLf
			clientSideScript = clientSideScript & "		window.showModalDialog('" & sPath & "EmailPDF.asp?enterDetailsPointer=1', window.self, 'dialogWidth:30em;dialogHeight:22em;status:0;dialogHide:false;help:no;scroll:yes;resizable:no;edge:sunken;unadorned:yes');"  & vbCrLf
			clientSideScript = clientSideScript & "	}" & vbCrLf		
		End If
...
Function DrawPageFunctions2(includePrint, includeEmailThisPage, includeBackBtn, PathTo, includePDF)
	Dim clientSideScript
	Dim sName, sPort, sPath, thisPath
	...
	With Response
		...
		If includeBackBtn Then
			.Write "    <td " & barTdClientScript & " OnClick=""javascript: window.location.href='" & PathTo & "'""><font color=blue face=""Webdings"" size=1>3</font>Back" & vbCrLf
			.Write "    </td>" & vbCrLf		
		End If
...
function LTdate(todaysdate,NoOfDays)
  dim h
  dim Days
  ...
  dim thisrs	
  do while h<NoOfDays			
		sqlStr3 = "SELECT Holiday AS holiday FROM Holidays " & _
                  " WHERE (Holiday ='" & FormatDate(todaysdate) & "') OR " & _
                  " (DATEPART(weekday, '" & FormatDate(todaysdate) & "') = 1) OR " & _
                  " (DATEPART(weekday, '" & FormatDate(todaysdate) & "') = 7)"           
        
        
        Set thisrs = conn.Execute(sqlStr3)
        
        if not(thisrs.EOF and thisrs.BOF) then         
         NoOfDays=NoOfDays+1
