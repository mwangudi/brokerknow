...
<%
	...
	If Len(DocNo)>0 Then
		sqlStr = "SELECT * FROM SaleContracts WHERE (ContractNumber LIKE '%"& DocNo &"%') ORDER BY ContractNumber"	
	End If
	
	If Len(Client)>0 Then
		sqlStr = "SELECT * FROM SaleContracts WHERE (Client_DPA_ = "& Client &") AND (LotTDate BETWEEN '" & FormatDate(selectedFromDate) & "' AND '"& FormatDate(selectedToDate)  &"')"
	End If
	...
	If Len(DocNo)>0 Then
		Set groupRs = Conn.Execute("SELECT DISTINCT ContractNumber FROM SaleContracts WHERE (ContractNumber LIKE '%"& DocNo &"%') GROUP BY ContractNumber")
	End If
	
	If Len(Client)>0 Then
		Set groupRs = Conn.Execute("SELECT DISTINCT ContractNumber FROM SaleContracts WHERE (Client_DPA_ = "& Client &") AND (LotTDate BETWEEN '" & FormatDate(selectedFromDate) & "' AND '"& FormatDate(selectedToDate)  &"')")
	End If
	...
	sqlStr = "SELECT     tbOrder.*, Client.ClientAddr,Client.ClientCDSNo,client.Client_DPA_ FROM Lot INNER JOIN " & _
	...
	If Not (temprs.EOF Or temprs.BOF) Then
		...
		code =  temprs.Fields("Client_DPA_").Value
	End If
...
<table border="0" cellspacing=2 cellpadding=2 class="ReportsTable" align ="center" width="90%"> 
	...
	<tr>
		...
		<td valign="top" align="center" width="30%">
			<table cellspacing=0 cellpadding=0 border=0>
				<tr>
					<td nowrap>Trade Date:&nbsp;&nbsp;&nbsp;<%= FormatDate(Rs.Fields("LotTDate").Value) %></td>
				</tr>
				<tr>
