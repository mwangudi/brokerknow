...
<%
	...
	sqlStr = "SELECT DISTINCT Order_DPA_,Orddetail_DPA_ FROM AgentContractCompounded WHERE (LotTDate = '" & FormatDate(selectedContractDate) & "') ORDER BY Orddetail_DPA_"
	...
	Do Until OrderRs.EOF
			groupRs.Filter = "Orddetail_DPA_ = '" & OrderRs.Fields("Orddetail_DPA_").Value & "'"
			...
			sqlStr = "SELECT   Order_DPA_, OrdDetailSecType,Agent_DPA_, AgentName, AgentAddr, LevyName, LevyShortName, CASE SystemMaintained WHEN 13 THEN MAX(LevyAmount) ELSE SUM(LevyAmount) " & _
					  ...
					  "AS ContractNumber, OrdDetailSecurity, SUM(LotQty) AS LotQty, SUM(LotPrice) AS LotPrice, OrdDetailClient, SystemMaintained, LevyRatePercentage ,ClientCDSNo ,Client_DPA_, SUM(LevyVATAmount) AS LevyVATAmount,Max(ContractSettlementDate) as ContractSettlementDate  " & _
					  "FROM         dbo.AgentContractCompounded " & _
					  "WHERE     (cast(floor(cast(LotTDate as float)) as datetime) = '" & FormatDate(selectedContractDate) & "') And LevyName <> '' AND OrdDetailClient = '" & prevClient & "' AND (OrdDetail_DPA_ = " & OrderRs.Fields("OrdDetail_DPA_").Value & ")" & _
					  "GROUP BY Agent_DPA_, AgentName, AgentAddr, LevyName, LevyShortName, OrdDetailSecurity, OrdDetailClient, SystemMaintained, LevyRatePercentage,OrdDetailSecType,Order_DPA_,ClientCDSNo,Client_DPA_  "
...
<table border="0" cellspacing=2 cellpadding=2 class="ReportsTable"  align ="center" width="90%">	
	...
	<tr>
		...
		For i = 1 To UBound(levyArray) 
			...
			levyPerc = FormatNumber((levyArray(i, 1) / totalGross) * 100,2)			
	...
	<%
					 if brokerRate = 0 or brokerRate = "" then
					   ...
					   'response.write AgentCommission & "'''''' " & BrokerCommission
					   returncommission = cdbl(AgentCommission/BrokerCommission * 100)
					 end if
