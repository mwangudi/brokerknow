...
If genReport <> "1" Or selectedClient = "" Or Not IsDate(selectedFromDate) Then%>
	...
	<form method="POST" action="ClientStatementTEST.asp" Name="frmMain" id="frmMain">
		<input type="hidden" value="1" name="genReport">
		<table>
			<tr>
				<td colspan=2><b>Client Statement Test</b></td>
			</tr>
			<tr>
				<td>Client: </td>
...
<%
	...
	If Not (rsClient.EOF Or rsClient.BOF) Then
		...
		If Len(rsClient("ClientContact")) > 0 Then	
			accountDesc = rsClient.Fields("ClientContact").Value & " [  " & rsClient.Fields("Client_DPA_").Value & " ]" &  rsClient.Fields("ClientCDSNo").Value 
		Else
			accountDesc = rsClient.Fields("ClientName").Value & " [  " & rsClient.Fields("Client_DPA_").Value & " ]" &  rsClient.Fields("ClientCDSNo").Value 
		End If
