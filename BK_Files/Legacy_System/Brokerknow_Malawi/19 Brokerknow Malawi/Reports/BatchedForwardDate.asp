...
<%
	'Get Details					        
	sqlStr = "SELECT * FROM ForwardsList where (NOT (Batch_No IS NULL)) AND Cast(floor(cast(Offerings_Date as float)) as DateTime) Between '" & FormatDate(CDate(selectedFromDate)) & "' and '" & FormatDate(CDate(selectedToDate)) & "' " & _
