...
<%
	...
	sqlStr = "SELECT * FROM AgentStatement where Agent_DPA_ LIKE '" & selectedAgent & "' AND TransDate Between '" & FormatDate(CDate(selectedFromDate)) & "' and '" & FormatDate(CDate(selectedToDate)) & "' Order by isopeningbalance desc,transdate"
	'response.write sqlstr : response.end
	Rs.CursorLocation = adUseClient	
	Rs.Open SQLServerFormat(HandleQuote(sqlStr)), conn.ConnectionString, adOpenKeyset, adLockOptimistic
	'Rs.Filter = "Agent_DPA_ LIKE '" & selectedAgent & "' AND TransDate >= '" & FormatDate(selectedFromDate) & "'"
...
sqlStr = "SELECT agent_DPA_, '" & FormatDate(selectedFromDate) & "' AS TransDate, '' AS REF, 'Opening Balance' AS Particulars, " & _
				" 0 AS Debit, 0 AS Credit, 1 AS IsOpeningBalance, SUM(Credit - Debit) " & _
				" AS Balance FROM  AgentStatement " & _
				" WHERE     (agent_DPA_ = " & selectedAgent & ") AND (TransDate < '" & FormatDate(selectedFromDate) & "')" & _
				" GROUP BY Agent_DPA_"
'response.write sqlstr : response.end
...
'					Response.Write  FormatNum(CreditDebitValue(runningBal)) 
					Response.Write  FormatNum(runningBal)
				Else	
