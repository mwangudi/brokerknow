...
Dim BalanceTitle
	...
	if(Selectedtype="Account") then		
		...
		sqlStr = "SELECT * FROM BalanceAccountList WHERE Owner_DPA_ = " & selectedAccount & " AND (Day(TransDate) = " & _
		" Day(#" &  FormatDate(selectedFromDate) & "#))"
	else
		BalanceTitle="Agent Balances"
		accountDesc="Agent"				        
				        
		If Len(selectedAgent & "") = 0 then
		%>
			<script>
		...
		sqlStr = "SELECT * FROM BalanceAgentList WHERE agent_DPA_ = " & selectedAgent & " AND (Day(TransDate) = " & _
		" Day(#" &  FormatDate(selectedFromDate) & "#))"
	end if
