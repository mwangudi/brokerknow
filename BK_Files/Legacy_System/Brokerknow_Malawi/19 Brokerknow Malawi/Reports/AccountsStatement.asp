	...
	<form method="POST" action="AccountsStatement.asp" Name="frmMain" id="frmMain">
		...
		<table border="0" width="60%">
			<tr>
				...
				<td width="20%"><select name = 'cboEntity' id = 'cboEntity' size="1" onchange='evaluateEntity(this.value,this)'>
			...
			<tr>
				...
				<td width="20%"><select name = 'cboAccount' id = "cboAccount" size="1" 
			...
			<tr>
				<td width="10%">Select date from:</td>
				<td width="20%">
			...
			<tr>
				<td width="10%">&nbsp;</td>
				<td width="20%"><input type="Button" class="Buttons" OnClick="JavaScript: validateForm(document.all.item('frmMain'))" Value=" Generate... ">&nbsp;&nbsp;</td>
				<td width="20%"><input type = 'button' Class=Buttons name ='cmdRefresh' id = 'cmdRefresh' value=" Refresh " OnClick="JavaScript: window.location.replace('AccountsStatement.asp')"></td>				
			</tr>
		</table>
...
Select Case(SelectedType)
	case 1						        
		sqlStr = "SELECT * FROM StatementList WHERE Client_DPA_ = " & selectedBank & " AND TransDate Between '" & FormatDate(CDate(selectedFromDate)) & "' and '" & FormatDate(CDate(selectedToDate)) & "'"
	case 2
	...
	case 3
		sqlStr = "SELECT * FROM BrokerStatement1 where Broker_DPA_ LIKE '" & selectedBank & "' AND TransDate Between '" & FormatDate(CDate(selectedFromDate)) & "' and '" & FormatDate(CDate(selectedToDate)) & "' ORDER BY TransDate ASC"
	case 4
...
Do while Cint(intPage) < intPageCount	
		...
		If Not IsOpeningBalance Then
			Select Case(SelectedType)
				...
				sqlStr = "SELECT SUM(ISNULL(dbo.StatementList.Credit - dbo.StatementList.Debit, 0)) + dbo.Client.ClientOpeningBal AS CurrentBal, dbo.StatementList.Client_DPA_" & _
						 ...
						 " WHERE (dbo.Client.Deleted = 0) AND (dbo.StatementList.TransDate < '" & FormatDate(selectedFromDate) & "') GROUP BY dbo.StatementList.Client_DPA_, dbo.Client.ClientOpeningBal having dbo.StatementList.Client_DPA_=" & selectedBank 
				case 2
	...
	<table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="600">
		<tr>
			...
			<td width="50%" nowrap align=right><font face="Impact" size="3"><%= Session("CompanyName") %></font></td>
		</tr>
	</table>
	...
	<table border="0" cellspacing="1" cellpadding="1" style="font-family: Arial Narrow; LEFT-MARGIN:100PX"  width="600">        
		<%
		if(SelectedType<>5) then
			...
			<tr>
				...
				<td style="border-top-style: solid; border-top-width: 1; border-bottom-style: solid; border-bottom-width: 1"><b><font face="Arial Narrow" size="3">&nbsp;</font></b></td>
		...
		For intRecord = 1 to Rs.PageSize
			...
			if(SelectedType=2 or SelectedType=1 or SelectedType=7 or SelectedType=6 or SelectedType=4 or SelectedType=8 or SelectedType=100)  then
				if(m=0) then
			...
			If Cdbl(Rs.Fields("Credit").Value) =0 and Cdbl(Rs.Fields("Debit").Value) =0 then
				...
				<tr>	
					...
					<td align="right">
						<%
						If Rs.Fields("Credit").Value Then
					...
					<td align="right">
						...
						Else
							Select Case SelectedType
								Case 1
									runningBal = runningBal + rs("balance")
								Case 2
									runningBal = runningBal + rs("balance")
								Case 8
									runningBal = runningBal + rs("balance")
							...
							'Response.End 			
							Response.Write CreditDebitValue(FormatNum(runningBal))
						End If 				
						%>
					</td>
				</tr>
		...
		if Cint(intPage) <> intPageCount then
			if(SelectedType<>5) then 	
		  		%>
		  		<tr>
		      		<td colspan="7" align="right" style="border-bottom-style: solid; border-bottom-width: 1">&nbsp;</td>
		    	</tr>

		    	<tr>
		      		<td colspan="6" align="right"><font size="1">Opening Balance:</font></td>
		    	...
		    	<tr>
		      		<td colspan="3">&nbsp;</td>
		      		<td align="right" COLSPAN=3><b><font size="1">Running Balance:</font></b></td>
	...
	function ClientBalance(Days)
		Select Case(SelectedType)
			case 1						        	
				sqlStr = "SELECT SUM(ISNULL(dbo.StatementList.Credit - dbo.StatementList.Debit, 0)) + dbo.Client.ClientOpeningBal AS CurrentBal, dbo.StatementList.Client_DPA_" & _
				" FROM  dbo.StatementList INNER JOIN dbo.Client ON dbo.StatementList.Client_DPA_ = dbo.Client.Client_DPA_" & _
				" WHERE (dbo.Client.Deleted = 0) AND (dbo.StatementList.TransDate < '" & FormatDate(CDate(selectedToDate)-Days) & "') GROUP BY dbo.StatementList.Client_DPA_, dbo.Client.ClientOpeningBal having dbo.StatementList.Client_DPA_=" & selectedBank 
			case 2
				sqlStr = "SELECT SUM(ISNULL(dbo.AgentStatement1.Credit - dbo.AgentStatement1.Debit, 0)) + dbo.Agent.AgentOpeningBal AS CurrentBal, dbo.AgentStatement1.Agent_DPA_" & _
				" FROM  dbo.AgentStatement1 INNER JOIN dbo.Agent ON dbo.AgentStatement1.Agent_DPA_ = dbo.Agent.Agent_DPA_" & _
