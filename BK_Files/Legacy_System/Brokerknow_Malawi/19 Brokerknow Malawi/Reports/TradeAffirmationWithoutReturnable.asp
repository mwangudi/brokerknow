...
If genReport <> "1" Or Not IsDate(selectedTradeDate) Then%>
	...
	<form method="POST" action="TradeAffirmationWithoutReturnable.asp" Name="frmMain" id="frmMain">
			...
			<tr style="display: none;" id="ownerSelectRow" align="right">
				<td colspan=2>
					<select name="AllSelOwners" style="display: none">
						...
						sqlStr = "SELECT DISTINCT LotTDate, Owner FROM TradeAffirmationWithoutReturnable"   
...
<%
   ...
   sqlStr = "SELECT Order_DPA_ FROM TradeAffirmationWithoutReturnable WHERE LotTDate = '" & FormatDate(selectedTradeDate) & "'"
   ...
   sqlStr = "SELECT Order_DPA_, OrderRef FROM TradeAffirmationWithoutReturnable WHERE LotTDate = '" & FormatDate(selectedTradeDate) & "'"
   ...
   Do Until sumRs.EOF
		...
		sqlStr = "SELECT * FROM TradeAffirmationWithoutReturnable WHERE LotTDate = '" & FormatDate(selectedTradeDate) & "' And OrderRef = '" & OrderRef & "' AND Order_DPA_ = " & sumRs.Fields("Order_DPA_").Value		
		Set Rs = Conn.Execute (SQLServerFormat(HandleQuote(sqlStr)))
