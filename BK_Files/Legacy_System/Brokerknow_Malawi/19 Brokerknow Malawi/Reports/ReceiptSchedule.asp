...
If genReport <> "1" Or Not IsDate(selectedFromDate) Then%>
	...
	<form method="POST" action="ReceiptSchedule.asp" Name="frmMain" id="frmMain">
...
<%
   ...
   set TimeLimitRs=Conn.Execute(sqlStr) 
	...
	SettlementDate=LTdate(CDate(selectedFromDate),5)	
	    ...
	    sqlStr = "SELECT * FROM ReceiptSchedule  WHERE (Day(Payment_Date) = Day(" & "#" & CDate(SelectedFromDate) & "#" & ")) AND (Month(Payment_Date) = Month(" & "#" & CDate(SelectedFromDate) & "#" & ")) AND (Year(Payment_Date) = Year(" & "#" & CDate(SelectedFromDate) & "#" & "))and CdsGroup<>1"
...
<table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="100%">
    <tr>
      <td width="10%" nowrap><b><font face="Arial" size="4"><b>RECEIPTS SCHEDULE</b></font></b></td>
...
<br>
 <table border="0" cellspacing="0" cellpadding="3" style="font-family: Arial Narrow; LEFT-MARGIN:100PX">
    <tr>
	  ...
	  <td bgcolor="#000000"><b><font color="#FFFFFF" face="Arial Narrow" size="3">Narrative</font></b></td>
	...
	if (Cds1<>Cds2) and (first<>0) then %>
		...
		first=1		 	
			...
			<tr>
			  ...
			  <td align="left"><%= rs("PaymentNarrative") %>&nbsp;</td>
