...
<title>Excluded Sale Orders</title>
...
<%
	...
	Dim Security2
	     ...
	     '========================================================
	     ' Filter : 1 : Sales only
	     '          2 : Order validity within specified Date Range
	     '          3 : Exclude bond orders
	     '========================================================
	     
		 sqlStr = "SELECT * FROM TotalTradingSchedule WHERE OrdDetailType = 'sale' and ((Validity >= { fn CURDATE() }) OR " & _
             "(Validity IS NULL)) AND ordDetailSecType NOT LIKE 'Fixed'  AND Iscustodian = 0 ORDER BY Code,Client,Order_DPA_"
	   ...
	   'Response.Write(sqlstr)
	    ...
	    If (groupRs.EOF=true Or groupRs.BOF=true) Then %>
				<Script Language="JavaScript">
...
<table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="100%">
     <tr>
		<td nowrap><b><font face="Arial Narrow" size="4">Excluded Sale  Orders</font></b></td>
  ...
  <table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="100%">        
	...
	'groupRs.Fields("BalanceFree")="Y" and
	...
	if(groupRs.Fields("BalanceFree")="N" or groupRs.Fields("Excess") > 0) then		
	  Exclude=true
	else
	  Exclude=false
	end if

	if(Saleya and Exclude) then
	Security1=OrdDetailSecurity 
	'if(groupRs.Fields("Order_DPA_")=1170) then
    ...
    <tr>
      ...
      <td colspan="13" width="970"><b><%
	  
	   if groupRs.Fields("ClientCDSNo") <> "none" then
	    response.write groupRs.Fields("ClientCDSNo")
	   end if
    ...
    %>
			<tr>
			  <td width="30">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
			  <td width="50"><%= groupRs.Fields("SecurityCode").Value %></td>			  				
			  <td width="50"><%= groupRs.Fields("Order_DPA_").Value %></td>
			  <td width="70"><%= FormatDate(groupRs.Fields("OrderDate").Value) %></td>
