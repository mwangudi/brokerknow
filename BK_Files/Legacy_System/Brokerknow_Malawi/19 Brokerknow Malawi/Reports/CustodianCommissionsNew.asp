...
If genReport <> "1" Or Not IsDate(selectedFromDate) Then%>
	...
	<form method="POST" action="CustodianCommissionsNew.asp" Name="frmMain" id="frmMain">
...
<table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="100%">
    ...
    <tr bgColor="#000000">
      ...
      <%
		...
		Set conn = GetActiveConnection("KBroker")
 		sqlStr = "CustodianContractLevies"	
		
		set rs = conn.execute( SQLServerFormat(HandleQuote(sqlStr)))
		
		Rs.Filter = "LotTDate >= '" & FormatDate(selectedFromDate) & "'"
		
		if selectedToDate <> "" then
		 Rs.Filter = "LotTDate <= '" & FormatDate(selectedToDate) & "'"
		end if
		...
		for i = beginLeviesCol to rs.fields.count - 2
		    if i <> beginLeviesCol+2 then
				...
				%>
					...
					<%
					'Skip CDSC column
						If (Len(rs.fields(i).name)>4) Then
							Response.Write Mid(rs.fields(i).name,1,4)
						Else
 ...
 <%
     If Not(rs.EOF Or rs.BOF) Then
      ...
      <td nowrap align="left"><font size="1"><% If Len(rs.Fields("ClientName")) > 25 Then 
