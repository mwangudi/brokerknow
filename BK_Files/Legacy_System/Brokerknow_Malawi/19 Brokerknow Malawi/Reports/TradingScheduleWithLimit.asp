...
'sqlstr =" SELECT OrdDetailSecurity, OrdDetailSecType, Security_DPA_ FROM DB_FineTradingSchedule   " & _
	...
	'"       OR  " & _
	'"     (OrderTypeSale =1 AND (BalanceFree <> 'N' OR  Excess < = 0))) " & _
	'" Group By OrdDetailSecurity, OrdDetailSecType, Security_DPA_  " & _
	'" ORDER BY OrdDetailSecurity ASC"

Dim DateLimit
DateLimit = Request.QueryString("Limit")

If Len(DateLimit) = 0 Then
	sqlstr = " SELECT OrdDetailSecurity, OrdDetailSecType, Security_DPA_ FROM DB_FineTradingSchedule " & _
		"  WHERE ((Validity >= { fn CURDATE() }) OR (Validity IS NULL))  AND  " & _
...
Else
	sqlstr = " SELECT OrdDetailSecurity, OrdDetailSecType, Security_DPA_ FROM DB_FineTradingSchedule " & _
		"  WHERE ((Validity >= '"& DateLimit &"') OR (Validity IS NULL))  AND  " & _
		" ((OrderTypeSale = 0 AND Excess <= 0)  OR   " & _
		" (OrderTypeSale =1 AND (BalanceFree <> 'N' OR  Excess < = 0))OR (ordDetailSecType = 'Fixed') OR (Iscustodian = 1)) " & _
		" AND OrderDate >= '" & DateLimit &"'" & _
		" Group By OrdDetailSecurity, OrdDetailSecType, Security_DPA_   " & _
		" ORDER BY OrdDetailSecurity ASC"
	theDate = DateLimit 		
End If
...
<Script Language="JavaScript">
	...
	var cal=new ctlSpiffyCalendarBox("cal", "frmMain", "txtDate","cmdDate","<%= FormatDate(theDate) %>",1);
</Script>
	
<div id="spiffycalendar" class="text" STYLE="z-Index: 1000"></div>
	
<form method="" action="" Name="frmMain" id="frmMain">
...
<table border="0" cellspacing="0" cellpadding="5" style="font-family: Arial Narrow" width="100%">
	<tr>
		<td colspan="6"><b>Specify Order Date Limit:</b>
		&nbsp;<SCRIPT language="JavaScript">cal.writeControl();</SCRIPT>
		
		&nbsp;<input type="Button" OnClick="JavaScript: window.target='_self'; window.location.href ='TradingScheduleWithLimit.asp?Limit='+frmMain.txtDate.value" Value=" Generate " id=Button1 name=Button1 style="font-family:tahoma; font-size:8pt; font-weight:bold;">
		</td>
	</tr>
		...
		If Not (Rs.EOF Or Rs.BOF) Then
			Do Until Rs.EOF
				...
				<tr>			
					...
					<td align="right"><%=formatnumber(OrderQty,0)%> </td>
