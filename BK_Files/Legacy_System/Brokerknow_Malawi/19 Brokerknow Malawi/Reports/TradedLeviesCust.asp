...
<title>Traded Levies (Custodians)</title>
...
If genReport <> "1" Or Not IsDate(selectedFromDate)  Or Not IsDate(selectedToDate)Then%>
	...
	<form method="POST" action="TradedLeviesCust.asp" Name="frmMain" id="frmMain">
...
<table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="100%">
     <tr>
		<td nowrap><b><font face="Arial Narrow" size="4">Traded Levies (Custodians)</font></b></td>
    ...
    <tr bgColor="#000000">
      ...
      <%
		...
		TrNox = 0 
		procstr= "ContractLeviesCrossTabForCustodians '"& selectedFromDate & "', '" & selectedToDate & "'" 
				...
				for i = beginLeviesCol to rs.fields.count - 2
						...
						IF (trim(lcase(rs.fields(i).name)) = "commission") or (trim(lcase(rs.fields(i).name)) = "cdsc") then
							...
							IF (trim(lcase(rs.fields(i).name)) = "transfer") AND (skipTransfer = true) then
									...
									if (trim(lcase(rs.fields(i).name)) = "comp") then 
										compulsoryLevyValue = rs.fields(i).value/2
									else 
										compulsoryLevyValue = rs.fields(i).value
									end if
									
									%>
										<td  nowrap align=right width="60px" Style="<%= myStyle %>"><%=FormatNum(compulsoryLevyValue)%></td>		
									<%
									
									levyTotals = levyTotals + compulsoryLevyValue
												
									dailyTotalsArray(ICount) = dailyTotalsArray(ICount) + compulsoryLevyValue
									ICount = ICount + 1	
							end if
						end if
