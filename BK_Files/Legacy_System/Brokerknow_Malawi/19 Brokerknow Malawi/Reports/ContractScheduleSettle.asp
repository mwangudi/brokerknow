...
selectedToTradeDate = Request.Form("txtToDate")
If genReport <> "1" Or Not IsDate(selectedTradeDate) Then%>
	...
	<form method="POST" action="ContractScheduleSettle.asp" Name="frmMain" id="frmMain">
...
headerDescription = FormatDateFull(selectedTradeDate)  & " To " & FormatDateFull(selectedToTradeDate ) %>
...
<table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="100%">
     <tr>
		<td nowrap><b><font face="Arial Narrow" size="4">Settlement Schedule <!--Non-CSD--></font></b></td>
		<td nowrap align=right><font face="Impact" size="3"><%= Session("CompanyName") %></font></td>
	</tr>	
    <tr>
	   <td COLSPAN=2><font face="Arial" size="2">for Deals settling on:  <%= headerDescription %></font></td>
	</tr>
    <tr>
    ...
    <tr bgColor="#000000">
	  ...
	  <td bgColor="#000000" nowrap align="center"><b><font color="#FFFFFF">Settled</font></b></td>
      ...
      <td bgColor="#000000" nowrap align="center"><b><font color="#FFFFFF">Slip</font></b></td>
      ...
      <%
		...
		Set conn = GetActiveConnection("KBroker")
 		sqlStr = "ContractLeviesCrossTabNewSettleNonCSD '" & FormatDate(selectedTradeDate) & "' , '"& FormatDate(selectedToTradeDate) & "'"	
		...
		for i = beginLeviesCol + 1 to rs.fields.count - 2
				Redim Preserve dailyTotalsArray(i - beginLeviesCol)
				dailyTotalsArray(i - beginLeviesCol) = 0
				
				if (trim(lcase(rs.fields(i).name)) = "vat") then
					VatColumn= (i) - beginLeviesCol
				end if
					%>
 ...
 <%
     If Not(rs.EOF Or rs.BOF) Then
      ...
      <td nowrap><font size="1"><%= Day(rs.Fields("ContractSettlementDate")) & " " & MonthName(Month(rs.Fields("ContractSettlementDate")), True) %></font></td>
      ...
      <td style="BORDER-RIGHT: #C0C0C0 1px inset; BACKGROUND-COLOR: transparent">&nbsp;</td>
           <%	levyTotals = 0
				...
				If rs.Fields("OrderTypeSale").Value = 0 Then 
					...
					netAmt = grossAmt + rs.Fields("Total")
		...
		<tr height="30px">
			<td colspan=10 align=right>
			 ...
			 <%	
				for i = 0 + 1 to UBound(dailyTotalsArray)
						If i = 0 Then 
