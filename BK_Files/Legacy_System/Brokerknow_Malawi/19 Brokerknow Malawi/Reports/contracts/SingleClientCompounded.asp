...
<%
	...
	If Len(DocNo)>0 Then
		sqlStr = "SELECT DISTINCT Order_DPA_, OrdDetail_DPA_ FROM ContractCompoundedClients WHERE (ContractNumber LIKE '%"& DocNo &"%')"
	End If
	
	If Len(Client)>0 Then
		sqlStr = "SELECT DISTINCT Order_DPA_, OrdDetail_DPA_ FROM ContractCompoundedClients WHERE (Client_DPA_ = "& Client &") AND (LotTDate BETWEEN '" & FormatDate(selectedFromDate) & "' AND '"& FormatDate(selectedToDate)  &"')"
	End If
	...
	Do Until OrderRs.EOF
			...
			sqlStr = "SELECT     Order_DPA_, OrdDetailSecType, LevyName, LevyShortName, CASE SystemMaintained WHEN 13 THEN MAX(LevyAmount) ELSE SUM(LevyAmount) " & _
                      "END AS LevyAmount, SUM(GrossAmount) AS GrossAmount, MIN(CAST(ClientAddr AS nvarchar(4000))) AS ClientAddr, " & _	
					  "MIN(OrderRef) AS orderRef, MIN(CAST(OrderTypeSale AS INT)) AS OrderTypeSale, MIN(OrdDetailType) AS OrdDetailType, MIN(ContractNumber) " & _
					  "AS ContractNumber, OrdDetailSecurity, SUM(LotQty) AS LotQty, SUM(LotPrice) AS LotPrice, OrdDetailClient, SystemMaintained, LevyRatePercentage, Client_DPA_,ClientCDSNo,LevyVATAmount,Contract_DPA_   " & _
					  "FROM         dbo.ContractCompoundedClients " & _
					  ...
					  "GROUP BY LevyName, LevyShortName, OrdDetailSecurity, OrdDetailClient, SystemMaintained, LevyRatePercentage,OrdDetailSecType,Order_DPA_, Client_DPA_,ClientCDSNo,LevyVATAmount,Contract_DPA_  "
...
<table border="0" cellspacing=2 cellpadding=2 class="ReportsTable" align ="center" width="90%"> 
	<tr>
		...
		<td valign="top" align="center" width="30%">
			<table cellspacing="0" cellpadding="0" border="0">
				<tr>
					<td  nowrap colspan="2">Date:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%= FormatDate(prevDate) %></td>
				</tr>
				<tr>
