...
<%
	...
	If Len(DocNo)>0 Then
		sqlStr = "SELECT DISTINCT Order_DPA_,OrdDetail_DPA_ FROM AgentContractCompounded WHERE (ContractNumber LIKE '%"& DocNo &"%')"
	End If
	
	If Len(Agent)>0 Then
		sqlStr = "SELECT DISTINCT Order_DPA_,OrdDetail_DPA_ FROM AgentContractCompounded WHERE (Agent_DPA_ = "& Agent &") AND (LotTDate BETWEEN '" & FormatDate(selectedFromDate) & "' AND '"& FormatDate(selectedToDate)  &"')"
	End If
	...
	Do Until OrderRs.EOF
			...
			sqlStr = "SELECT   Order_DPA_, Client_DPA_, OrdDetailSecType,Agent_DPA_, AgentName, AgentAddr, LevyName, LevyShortName, CASE SystemMaintained WHEN 13 THEN MAX(LevyAmount) ELSE SUM(LevyAmount) " & _
					  ...
					  "AS ContractNumber, OrdDetailSecurity, SUM(LotQty) AS LotQty, SUM(LotPrice) AS LotPrice, OrdDetailClient, SystemMaintained, LevyRatePercentage ,ClientCDSNo,LevyVATAmount " & _
					  ...
					  "GROUP BY Agent_DPA_, AgentName, AgentAddr, LevyName, LevyShortName, OrdDetailSecurity, OrdDetailClient, SystemMaintained, LevyRatePercentage,OrdDetailSecType,Order_DPA_,Client_DPA_,ClientCDSNo,LevyVATAmount"
