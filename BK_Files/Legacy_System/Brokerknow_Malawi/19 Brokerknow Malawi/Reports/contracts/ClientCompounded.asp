...
<%
	...
	sqlStr = "SELECT DISTINCT Order_DPA_,OrdDetail_DPA_ FROM ClientContractCompounded WHERE (LotTDate = '" & FormatDate(selectedContractDate) & "')  ORDER BY Order_DPA_"
	...
	Do Until OrderRs.EOF
			...
			sqlStr = "SELECT     Order_DPA_, OrdDetailSecType, LevyName, LevyShortName, CASE SystemMaintained WHEN 13 THEN MAX(LevyAmount) ELSE SUM(LevyAmount) " & _
					  ...
					  "AS ContractNumber, OrdDetailSecurity, SUM(LotQty) AS LotQty, SUM(LotPrice) AS LotPrice, OrdDetailClient, SystemMaintained, LevyRatePercentage, Client_DPA_, ClientCDSNo,LevyVATAmount " & _
					  "FROM         dbo.ClientContractCompounded " & _
					  "WHERE     (LotTDate = '" & FormatDate(selectedContractDate) & "') And LevyName <> '' AND OrdDetailClient = '" & prevClient & "' AND (OrdDetail_DPA_ = " & OrderRs.Fields("OrdDetail_DPA_").Value & ")" & _
					  "GROUP BY LevyName, LevyShortName, OrdDetailSecurity, OrdDetailClient, SystemMaintained, LevyRatePercentage,OrdDetailSecType,Order_DPA_, Client_DPA_,ClientCDSNo,LevyVATAmount"
