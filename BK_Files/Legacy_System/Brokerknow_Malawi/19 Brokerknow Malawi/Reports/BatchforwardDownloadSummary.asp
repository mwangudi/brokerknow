...
<title>Summary Batch Download  Forwards</title>
...
<br>
	...
	<%
	 ...
	 if intrscount <= 0 then
		 %>
		 <script language="javascript">
		  alert("No batch forward has been downloaded.")
   ...
   <%
    for intcount = 0 to intrscount-1
     BatchID = trim(batchData(0,intCount))
	  ...
	  sqlStr = "SELECT Offerings_Date, Pal_No,Client_DPA_, ClientName, Alloted_Rights, Offering_Price,SecurityName,OfferCheque,OfferBank,BatchFileName FROM ForwardsList where Batch_No = " & BatchID & _
...
<br>
    ...
    <tr>
	   ...
	   <%
		 ...
		 for rstcount = 0 to intrstcount- 1 
				...
				<tr>
					...
					<td nowrap><%=Mid(trim(astrRecs(8,rstcount)),1,30)%></td>
					<td nowrap><%=Mid(trim(astrRecs(7,rstcount)),1,30)%></td>	
      ...
      %>
	    <tr>
			<td colspan="7" align="center"><b>Totals</b></td>
