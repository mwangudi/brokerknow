...
<div id="spiffycalendar" class="text" STYLE="z-Index: 1000"></div>
	<form method="POST" action="FineTrialBalance.asp" Name="frmMain" id="frmMain">	
			<table>			
			<tr>
				<td>Select date:</td>
				<td>
...
<table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="100%">
     <tr>
		<td nowrap><b><font face="Arial Narrow" size="4">TRIAL BALANCE</font></b></td>
	...
	Do while rs.eof=false		
		Select Case(rs("EntityType_DPA_"))
      		case 1						        	
		sqlStr = "SELECT SUM(ISNULL(dbo.StatementList1.Credit - dbo.StatementList1.Debit, 0)) + dbo.Client.ClientOpeningBal AS CurrentBal, dbo.StatementList1.Client_DPA_ as AccountCode,Client.ClientName as AccountName" & _
				 " FROM  dbo.StatementList1 INNER JOIN dbo.Client ON dbo.StatementList1.Client_DPA_ = dbo.Client.Client_DPA_" & _
                                 " WHERE (dbo.Client.Deleted = 0) AND (dbo.StatementList1.TransDate <= '" & FormatDate(selectedFromDate) & "')" & _
				 "  GROUP BY dbo.StatementList1.Client_DPA_,Client.ClientName, dbo.Client.ClientOpeningBal "
		case 2
		sqlStr = "SELECT SUM(ISNULL(dbo.AgentStatement1.Credit - dbo.AgentStatement1.Debit, 0)) + dbo.Agent.AgentOpeningBal AS CurrentBal, dbo.AgentStatement1.Agent_DPA_ as AccountCode,Agent.AgentName as AccountName" & _
				 " FROM  dbo.AgentStatement1 INNER JOIN dbo.Agent ON dbo.AgentStatement1.Agent_DPA_ = dbo.Agent.Agent_DPA_" & _
				 " WHERE (dbo.Agent.Deleted = 0) AND (dbo.AgentStatement1.TransDate <= '" & FormatDate(selectedFromDate) & "')" & _
				 "  GROUP BY dbo.AgentStatement1.Agent_DPA_,Agent.AgentName, dbo.Agent.AgentOpeningBal"
		case 3
		
		sqlStr = "SELECT SUM(ISNULL(dbo.BrokerStatement1.Credit - dbo.BrokerStatement1.Debit, 0)) + dbo.Broker.BrokerOpeningBal AS CurrentBal, dbo.BrokerStatement1.Broker_DPA_ as AccountCode,Broker.BrokerName as AccountName" & _
				 " FROM  dbo.BrokerStatement1 INNER JOIN dbo.Broker ON dbo.BrokerStatement1.Broker_DPA_ = dbo.Broker.Broker_DPA_" & _
				 " WHERE (dbo.BrokerStatement1.TransDate <= '" & FormatDate(selectedFromDate) & "')" & _
				 "  GROUP BY dbo.BrokerStatement1.Broker_DPA_,Broker.BrokerName, dbo.Broker.BrokerOpeningBal"
		case 4
		...
		sqlStr = "SELECT SUM(DB_BankAccountStatement.CreditBal - DB_BankAccountStatement.Debit) AS CurrentBal, Account.AccountCode as AccountCode,Account.AccountName" & _
			 ...
			 " WHERE  (dbo.DB_BankAccountStatement.TransDate <= '" & FormatDate(selectedFromDate) & "')" & _			
		...
		sqlStr="SELECT SUM(ISNULL(Balance, 0)) AS CurrentBal, Entity_DPA_ as AccountCode,EntityName as AccountName FROM  dbo.CommissionStatement" & _
			   " WHERE (TransDate <= '" & FormatDate(selectedFromDate) & "')  GROUP BY Entity_DPA_,EntityName"		
		case 7
		sqlStr = "SELECT SUM(ISNULL(dbo.OwnerStatement.Credit - dbo.OwnerStatement.Debit, 0)) + dbo.Owner.OwnerOpeningBal AS CurrentBal, dbo.OwnerStatement.Owner_DPA_ as AccountCode,Owner.OwnerFName as AccountName" & _
				 " FROM  dbo.OwnerStatement INNER JOIN dbo.Owner ON dbo.OwnerStatement.Owner_DPA_ = dbo.Owner.Owner_DPA_" & _
				 " WHERE (dbo.OwnerStatement.TransDate <= '" & FormatDate(selectedFromDate) & "')" & _
				 " GROUP BY dbo.OwnerStatement.Owner_DPA_,owner.OwnerFName, dbo.Owner.OwnerOpeningBal"
		case 8
		sqlStr="SELECT SUM(ISNULL(case(IsOpeningBalance) when 1 then Balance else Credit - Debit end , 0)) AS CurrentBal, Client_DPA_ as AccountCode,Entity.EntityName as AccountName FROM  dbo.CDSControlStatement inner join Entity on dbo.CDSControlStatement.Client_DPA_=Entity.Entity_DPA_" & _
			   " WHERE (TransDate <= '" & FormatDate(selectedFromDate) & "') GROUP BY Client_DPA_,Entity.EntityName"				
		...
		Do Until cloneRs.EOF 
			...
			%>
				...
				<td ><%=Mid(cloneRs.Fields("AccountName").Value,1,30)%></td>
				<% if(cloneRs.Fields("CurrentBal").Value<0) then 
