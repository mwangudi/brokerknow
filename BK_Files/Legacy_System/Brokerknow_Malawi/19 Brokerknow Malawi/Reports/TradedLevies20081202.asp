...
If genReport <> "1" Or Not IsDate(selectedTradeDate) Then%>
	...
	<form method="POST" action="TradedLevies.asp" Name="frmMain" id="frmMain">
...
<table border="0" cellspacing="0" cellpadding="2" style="font-family: Arial Narrow" width="100%">
    ...
    <tr bgColor="#000000">
      ...
      <%
		...
		for i = beginLeviesCol to rs.fields.count - 2
				...
				IF ((trim(lcase(rs.fields(i).name)) = "transfer") AND (skipTransfer = true) ) or (trim(lcase(rs.fields(i).name)) = "vat") then
						'do nonthing
						colTransfer = (i - beginLeviesCol) 'Note the column position so as to skip it when totaling.
				else
						%>
 ...
 <%
     If Not(rs.EOF Or rs.BOF) Then
      ...
      <td nowrap><%If rs.Fields("OrderTypeSale") = "1" Then
						Response.Write "Sale"
				   Else
