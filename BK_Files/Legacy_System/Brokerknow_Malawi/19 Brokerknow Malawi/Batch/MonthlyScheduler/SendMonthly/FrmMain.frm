...
Dim Client_DPA_, startDate, theDate2, urlPath, SqlStr, UpdateSqlStr, Rep, NextEndOfMonthDate

Private Sub Form_Load()
    On Error GoTo ErrApp
    
    GetSettingsMonthlyClientStatement
    ...
    Else
        ...
        " WHERE (SendClientReports.Deleted = 0) AND (GenericSetting.GenericSetting_DPA_ = 6)) a LEFT OUTER JOIN" & _
        " SendMonthly ON a.Client_DPA_ = SendMonthly.Client_DPA_"
        ...
        If Not (RS.BOF Or RS.EOF) Then
            Do Until RS.EOF
                    If IsNull(RS("ClientEmail")) Then
                        SqlStr = ""
                        SqlStr = SqlStr & " if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[SendMonthly]') and OBJECTPROPERTY(id, N'IsUserTable') = 1) "
                        SqlStr = SqlStr & " drop table [dbo].[SendMonthly] "
                        Conn.Execute SqlStr
                        ...
                        SqlStr = SqlStr & " INTO            SendMonthly "
                        ...
                        SqlStr = SqlStr & " WHERE     (SendClientReports.Deleted = 0) AND (GenericSetting.GenericSetting_DPA_ = 6) "
                        Conn.Execute SqlStr
        ...
        ''CLIENT STATEMENTS
        SqlStr = "SELECT * FROM SendMonthly WHERE Sent1 = 0 AND ClientStatements = 1"
        Set RS = New ADODB.Recordset
        ...
        If Not (RS.BOF Or RS.EOF) Then
            If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                ...
                startDate = Format("01-" & MonthName(Month(Now)) & "-" & Year(Now), "dd-mmm-yyyy")
                theDate2 = Format(GetLastDay(Now), "dd-mmm-yyyy")
                
                'startDate = Format(DateAdd("d", -30, Now), "dd-mmm-yyyy")
                ...
                Rep = "ClientStatement"
                UpdateSqlStr = "UPDATE SendMonthly SET Sent1 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                Timer1.Enabled = True
            End If
        Else
            ''CLIENT CONTRACTS
            SqlStr = "SELECT * FROM SendMonthly WHERE Sent2 = 0 AND ClientContracts = 1"
            ...
            If Not (RS2.BOF Or RS2.EOF) Then
                If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                    ...
                    Rep = "ClientContract"
                    UpdateSqlStr = "UPDATE SendMonthly SET Sent2 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                    Timer1.Enabled = True
                End If
            Else
                ''CLIENT CONTRACTS COMPOUNDED
                SqlStr = "SELECT * FROM SendMonthly WHERE Sent3 = 0 AND ClientContractsCompounded = 1"
                ...
                If Not (RS3.BOF Or RS3.EOF) Then
                    If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                        ...
                        UpdateSqlStr = "UPDATE SendMonthly SET Sent3 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                        Timer1.Enabled = True
                    End If
                Else
                    ''HOLDINGS VALUATION
                    SqlStr = "SELECT * FROM SendMonthly WHERE Sent4 = 0 AND HoldingsValuation = 1"
                    ...
                    If Not (RS4.BOF Or RS4.EOF) Then
                        If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                            ...
                            UpdateSqlStr = "UPDATE SendMonthly SET Sent4 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                            Timer1.Enabled = True
                        End If
                    Else
                        ...
                        NextEndOfMonthDate = Format(Day(Now) & "-" & MonthName(Month(Now) + 1) & "-" & Year(Now), "dd-mmm-yyyy")
                        NextEndOfMonthDate = Format(GetLastDay(NextEndOfMonthDate), "dd-mmm-yyyy")
                        
                        Open App.Path & "\TimeLogMonthly.txt" For Output As 1
                        Print #1, Format(NextEndOfMonthDate, "dd-mmm-yyyy") & " " & sendTime
                        Print #1, "Pending"
                        Close 1
...
Private Sub Timer1_Timer()
    ...
    If lblCounter.Caption = 150 Then
        RS.Close
        ...
        " VALUES('Monthly','" & Rep & "','" & Client_DPA_ & "','" & Format(Date, "dd-mmm-yyyy") & "', '" & Format(Now, "dd-mmm-yyyy hh:mm:ss") & "')"
        Conn.Execute SqlStr
        
        Conn.Close
        
        Shell App.Path & "\SendMonthly.exe", vbNormalFocus
        Unload Me
    End If
...
Public Function GetLastDay(aDate)
