...
Private Sub Form_Load()
    ...
    GetSettingsWeeklyClientStatement
    ...
    Else
        ...
        " WHERE (SendClientReports.Deleted = 0) AND (GenericSetting.GenericSetting_DPA_ = 5)) a LEFT OUTER JOIN" & _
        " SendWeekly ON a.Client_DPA_ = SendWeekly.Client_DPA_"
        ...
        If Not (RS.BOF Or RS.EOF) Then
            Do Until RS.EOF
                    If IsNull(RS("ClientEmail")) Then
                        SqlStr = ""
                        SqlStr = SqlStr & " if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[SendWeekly]') and OBJECTPROPERTY(id, N'IsUserTable') = 1) "
                        SqlStr = SqlStr & " drop table [dbo].[SendWeekly] "
                        Conn.Execute SqlStr
                        ...
                        SqlStr = SqlStr & " WHERE     (SendClientReports.Deleted = 0) AND (GenericSetting.GenericSetting_DPA_ = 5) "
                        Conn.Execute SqlStr
        ...
        ''CLIENT STATEMENTS
        SqlStr = "SELECT * FROM SendWeekly WHERE Sent1 = 0 AND ClientStatements = 1"
        Set RS = New ADODB.Recordset
        ...
        If Not (RS.BOF Or RS.EOF) Then
            If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                ...
                startDate = Format(DateAdd("d", -7, Now), "dd-mmm-yyyy")
                ...
                Rep = "ClientStatement"
                UpdateSqlStr = "UPDATE SendWeekly SET Sent1 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                Timer1.Enabled = True
            End If
        Else
            ''CLIENT CONTRACTS
            SqlStr = "SELECT * FROM SendWeekly WHERE Sent2 = 0 AND ClientContracts = 1"
            ...
            If Not (RS2.BOF Or RS2.EOF) Then
                If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                    ...
                    Rep = "ClientContract"
                    UpdateSqlStr = "UPDATE SendWeekly SET Sent2 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                    Timer1.Enabled = True
                End If
            Else
                ''CLIENT CONTRACTS COMPOUNDED
                SqlStr = "SELECT * FROM SendWeekly WHERE Sent3 = 0 AND ClientContractsCompounded = 1"
                ...
                If Not (RS3.BOF Or RS3.EOF) Then
                    If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                        ...
                        UpdateSqlStr = "UPDATE SendWeekly SET Sent3 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                        Timer1.Enabled = True
                    End If
                Else
                    ''HOLDINGS VALUATION
                    SqlStr = "SELECT * FROM SendWeekly WHERE Sent4 = 0 AND HoldingsValuation = 1"
                    ...
                    If Not (RS4.BOF Or RS4.EOF) Then
                        If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                            ...
                            UpdateSqlStr = "UPDATE SendWeekly SET Sent4 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                            Timer1.Enabled = True
                        End If
                    Else
                        ...
                        Open App.Path & "\TimeLogWeekly.txt" For Output As 1
                        Print #1, Format(DateAdd("d", 7, sendDay), "dd-mmm-yyyy") & " " & sendTime
                        Print #1, "Pending"
                        Close 1
...
Private Sub Timer1_Timer()
    ...
    If lblCounter.Caption = 100 Then
        ...
        " VALUES('Weekly','" & Rep & "','" & Client_DPA_ & "','" & Format(Date, "dd-mmm-yyyy") & "', '" & Format(Now, "dd-mmm-yyyy hh:mm:ss") & "')"
        Conn.Execute SqlStr
        
        Conn.Close
        
        Shell App.Path & "\SendWeekly.exe", vbNormalFocus
        Unload Me
    End If
