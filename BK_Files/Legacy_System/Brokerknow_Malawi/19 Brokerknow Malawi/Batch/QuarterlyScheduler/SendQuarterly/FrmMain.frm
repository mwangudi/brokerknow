...
Private Sub Form_Load()
    ...
    GetSettingsQuarterlyClientStatement
    ...
    Else
        ...
        " WHERE (SendClientReports.Deleted = 0) AND (GenericSetting.GenericSetting_DPA_ = 7)) a LEFT OUTER JOIN" & _
        " SendQuarterly ON a.Client_DPA_ = SendQuarterly.Client_DPA_"
        ...
        If Not (RS.BOF Or RS.EOF) Then
            Do Until RS.EOF
                    If IsNull(RS("ClientEmail")) Then
                        SqlStr = ""
                        SqlStr = SqlStr & " if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[SendQuarterly]') and OBJECTPROPERTY(id, N'IsUserTable') = 1) "
                        SqlStr = SqlStr & " drop table [dbo].[SendQuarterly] "
                        Conn.Execute SqlStr
                        ...
                        SqlStr = SqlStr & " INTO            SendQuarterly "
                        ...
                        SqlStr = SqlStr & " WHERE     (SendClientReports.Deleted = 0) AND (GenericSetting.GenericSetting_DPA_ = 7) "
                        Conn.Execute SqlStr
        ...
        ''CLIENT STATEMENTS
        SqlStr = "SELECT * FROM SendQuarterly WHERE Sent1 = 0 AND ClientStatements = 1"
        Set RS = New ADODB.Recordset
        ...
        If Not (RS.BOF Or RS.EOF) Then
            If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                ...
                startDate = Format(DateAdd("d", -91, Now), "dd-mmm-yyyy")
                ...
                Rep = "ClientStatement"
                UpdateSqlStr = "UPDATE SendQuarterly SET Sent1 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                Timer1.Enabled = True
            End If
        Else
            ''CLIENT CONTRACTS
            SqlStr = "SELECT * FROM SendQuarterly WHERE Sent2 = 0 AND ClientContracts = 1"
            ...
            If Not (RS2.BOF Or RS2.EOF) Then
                If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                    ...
                    Rep = "ClientContract"
                    UpdateSqlStr = "UPDATE SendQuarterly SET Sent2 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                    Timer1.Enabled = True
                End If
            Else
                ''CLIENT CONTRACTS COMPOUNDED
                SqlStr = "SELECT * FROM SendQuarterly WHERE Sent3 = 0 AND ClientContractsCompounded = 1"
                ...
                If Not (RS3.BOF Or RS3.EOF) Then
                    If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                        ...
                        UpdateSqlStr = "UPDATE SendQuarterly SET Sent3 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                        Timer1.Enabled = True
                    End If
                Else
                    ''HOLDINGS VALUATION
                    SqlStr = "SELECT * FROM SendQuarterly WHERE Sent4 = 0 AND HoldingsValuation = 1"
                    ...
                    If Not (RS4.BOF Or RS4.EOF) Then
                        If (Trim(UCase(reqdStatus)) = UCase("Pending")) And (Val(Format(Now, "yyyymmddhhmmss")) > Val(Format(reqdDate, "yyyymmddhhmmss"))) Then
                            ...
                            UpdateSqlStr = "UPDATE SendQuarterly SET Sent4 = 1 WHERE (Client_DPA_ = " & Client_DPA_ & ")"
                            Timer1.Enabled = True
                        End If
                    Else
                        ...
                        Open App.Path & "\TimeLogQuarterly.txt" For Output As 1
                        Print #1, Format(DateAdd("d", 91, sendDay), "dd-mmm-yyyy") & " " & sendTime
                        Print #1, "Pending"
                        Close 1
...
ErrApp:
    ...
    'MsgBox "Error: " & Err.Description, vbInformation + vbCritical, "ERROR"
...
Private Sub Timer1_Timer()
    ...
    If lblCounter.Caption = 100 Then
        ...
        " VALUES('Quarterly','" & Rep & "','" & Client_DPA_ & "','" & Format(Date, "dd-mmm-yyyy") & "', '" & Format(Now, "dd-mmm-yyyy hh:mm:ss") & "')"
        Conn.Execute SqlStr
        
        Conn.Close
        
        Shell App.Path & "\SendQuarterly.exe", vbNormalFocus
        Unload Me
    End If
