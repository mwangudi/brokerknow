...
Dim rsMax
	...
	set rsMax=Conn.Execute("Select Max(Proposal_DPA_)+1 as MaxNo from BondProposals ")
...
sqlStr1 =" INSERT INTO BondProposals (Proposal_DPA_, ProposalDate, SettlementDate,Security_DPA_, Client_DPA_, " &_
				" BondIssue, BondIDate, BondMDate, BondPayment, CouponRate, " &_
				" ForwardRate,BondCleanPrice,AccruedInterest,BondDirtyPrice, Validity, " &_
				" CouponPeriodDays, Basis, ChangeInYield,TradeType, FaceValue,Duration," &_ 
				" RemainingDaysToCoupon, PreviousCouponPayments,Consideration," &_ 
				" Convexity,Commission, CommissionRate, CounterParty,Salutation, Bond_DPA_,AlternatePrice,NetAmount) " &_
				" VALUES("& MaxNo &",'"& cdate(ProposalDate) &"','"& cdate(SettlementDate) &"',"& BondType & ",'"& Client &"','"& IssueNo & "'," &_
				"'"& CDate(IssueDate) &"','"& cdate(MaturityDate) &"','"& CouponPayments & "'," &_
				"'"& CouponRate &"','"& ForwardRate &"','" & BondCleanPrice1 & "'," & AccruedInterest1 & "," &_
				"" & BondDirtyPrice1 & ",'"& ValidityDate &"','"& DaysInCouponPeriod &"', " &_
				"'"& Basis &"','"& ChangeInYield &"','" & TradeType & "'," & FaceValue & "," &_
				"" & Duration1 & ",'"& DaysToNextCoupon &"','" & NoCouponPaymentsLeft& "',"& Consideration &"," &_
				"" & Convexity2 & "," & Commission & "," & CommissionRate & "," &_
				"'" & CounterParty & "','" & Salutation & "'," & SecurityCode & " ," & AlternatePrice1 & " ," & NetAmount1 & " )"	
			
				
		sqlStr = "INSERT INTO [BondProposals] Proposal_DPA_, ProposalDate, SettlementDate,Security_DPA_, Client_DPA_, " &_
				" BondIssue, BondIDate, BondMDate, BondPayment, CouponRate, " &_
				" ForwardRate,BondCleanPrice,AccruedInterest,BondDirtyPrice, Validity, " &_
				" CouponPeriodDays, Basis, ChangeInYield,TradeType, FaceValue,Duration," &_ 
				" RemainingDaysToCoupon, PreviousCouponPayments,Consideration," &_ 
				" Convexity,Commission, CommissionRate, CounterParty,Salutation, Bond_DPA_,AlternatePrice,NetAmount) SELECT " & _
				         "" & " " & "iif(isnull(max([Proposal_DPA_])),(SELECT InitialID FROM _Initial_Table_ID_ WHERE TableName = 'BondProposals'),max([Proposal_DPA_]) + 1)" & " " & " as Proposal_DPA_" & _
				         "," & "#" &  cdate(ProposalDate)  & "#" & " as ProposalDate" &_
				         "," & "#" &  cdate(SettlementDate)  & "#" & " as SettlementDate" &_
				         "," & " " & BondType & " " & " as Security_DPA_" & _
				         "," & " " & Client & " " & " as Client_DPA_" & _
				         "," & " '" & IssueNo & " '" & " as BondIssue" & _
				         "," & "#" &  cdate(BondIDate)  & "#" & " as BondIDate" &_
				         "," & "#" &  cdate(MaturityDate)  & "#" & " as BondMDate" &_
						 "," & " '" & CouponPayments & " '" & " as BondPayment" & _	
						 "," & " '" & CouponRate & " '" & " as CouponRate" & _	
						 "," & " '" & ForwardRate & " '" & " as ForwardRate" & _	
						 "," & " '" & BondCleanPrice1 & " '" & " as BondCleanPrice" & _
						 "," & " '" & AccruedInterest1 & " '" & " as AccruedInterest" & _
						 "," & " '" & BondDirtyPrice1 & " '" & " as BondDirtyPrice" & _
						 "," & "#" &  cdate(ValidityDate)  & "#" & " as Validity" &_
						 "," & " '" & DaysInCouponPeriod & " '" & " as CouponPeriodDays" & _
						 "," & " '" & Basis & " '" & " as Basis" & _
						 "," & " '" & ChangeInYield & " '" & " as ChangeInYield" & _
						 "," & " '" & TradeType & " '" & " as TradeType" & _
						 "," & " '" & FaceValue & " '" & " as FaceValue" & _
						 "," & " '" & Duration1 & " '" & " as Duration" & _
						 "," & " '" & DaysToNextCoupon & " '" & " as RemainingDaysToCoupon" & _
						 "," & " '" & NoCouponPaymentsLeft & " '" & " as PreviousCouponPayments" & _
						 "," & " '" & Consideration & " '" & " as Consideration" & _
						 "," & " '" & Convexity2 & " '" & " as Convexity" & _
						 "," & " '" & Commission & " '" & " as Commission " & _				         			         
						 "," & " '" & CommissionRate & " '" & " as CommissionRate" & _
						 "," & " '" & CounterParty & " '" & " as CounterParty" & _
						 "," & " '" & Salutation & " '" & " as Salutation" & _
						 "," & " " & SecurityCode & " " & " as Bond_DPA_" & _
						 "," & " " & AlternatePrice & " " & " as AlternatePrice " & _
						 "," & " " & NetAmt & " " & " as NetAmount " & _
				         " FROM [BondProposals]"
	
	sqlStr1 = SQLServerFormatWithCustomMax(HandleQuote(sqlStr1))
...
function CommissionCalc()
	...
	sql = "select * from Commission where Commission_DPA_ = " & CommissionID
	set rsComm = Conn.Execute(sql) 
...
<SCRIPT LANGUAGE="JavaScript1.2" src="../scripts/valjavavalidate.js" TYPE="text/javascript"></SCRIPT><SCRIPT LANGUAGE="JavaScript" >
<!--
function validate(){
	 var doc= document.frmBondCalculator;
	 if (checkcomboempty(doc.cboClient,' Please Select a client')==false) return false;
	 if (checkcomboempty(doc.TradeType,' Please Select a Trade Type')==false) return false;	 
	 if (checkcomboempty(doc.BondType,' Please Select a Bond Type')==false) return false;
	 if (checkcomboempty(doc.IssueNo,' Please Select a Bond Issue')==false) return false;
	 //if (checkcomboempty(doc.SettlementDate,' Please Select a Settlement Date ')==false) return false;
	if (checkempty(doc.CouponRate,' Please Enter a Coupon Rate')==false) return false;
	if (checkempty(doc.FaceValue,' Please Enter a Face Value')==false) return false;
	if (checkempty(doc.ForwardRate,' Please Enter a Forward Rate')==false) return false;
	if (checkempty(doc.Basis,' Please Enter a Basis')==false) return false;
	if (checkempty(doc.DaysInCouponPeriod,' Please Enter a Days In CouponPeriod')==false) return false;
	if (checkempty(doc.Basis,' Please Enter a Basis')==false) return false;
	if (checkempty(doc.ChangeInYield,' Please Enter a Change In Yield')==false) return false;
	}
//-->
</SCRIPT>
<!--END CALENDAR -->
...
<form name="frmBondCalculator" method = 'post' action = 'AddBondProposal2.asp' id = "frmBondCalculator" onsubmit="return validate();">
...
<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">
  <tr>
    ...
    <td width="70%">
    <input type ="text" Name ="ProposalNo" ID ="ProposalNo" value="<%=MaxNo%>" readonly class=readonly style= "width:256" size="20"></td>
  </tr>
    <tr>
