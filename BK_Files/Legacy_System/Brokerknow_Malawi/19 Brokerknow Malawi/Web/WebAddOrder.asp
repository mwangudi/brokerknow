	...
	if action = "EXECUTE" then					
		...
		if instr(1,buttonAction,"SAVE") > 0 Or instr(1,buttonAction,"ADD") > 0 then
				...
				if cdate(orderDate) < Date() then%>
				 ...
				 sqlStr = "INSERT INTO [WebtbOrder] (OrderDate,OrderRef,Order_DPA_,Order_EIT_,OrderSecType_DPA_,Client_DPA_,OrderType_DPA_,OrderCompounded," & _
				 			"OrdDetailPrice,OrdDetailQty,Security_DPA_) SELECT " & "#" & orderDate & "#" & " as OrderDate" & _
				         "," & "'" & ref & "'" & " as OrderRef," & " " & "iif(isnull(max([Order_DPA_])),(SELECT InitialID FROM _Initial_Table_ID_ WHERE TableName = 'WebtbOrder'),max([Order_DPA_]) + 1)" & " " & " as Order_DPA_" & _
				         "," & "'" & guidStr & "'" & " as Order_EIT_," & " " & secType & " " & " as OrderSecType_DPA_" & _
				 ...
				 conn.BeginTrans
						...
						sqlStr = "SELECT [WebtbOrder.Order_DPA_] FROM [WebtbOrder] WHERE [WebtbOrder.Order_EIT_] = " & "'" & guidStr & "'"
...
<form name = 'frmAddOrder' method = 'post' action = 'WebAddOrder.asp' id = 'frmMain'>
...
<select name = 'cboOrderSecType' id = 'cboOrderSecType' size="1" currentSecType = "S" onchange='UpdateSecurityListing(this)'>
  ...
  <!--tr id="HeldReason">
	<td>Held Reason</td>
	<td><input type="text" name="txtHeldReason"></td>
  </tr-->
  <tr>
    <td>Client</td>
    <td>
    ...
    <select name = 'cboClient' id = 'cboClient' size="1" 
        ...
        sqlStr = "SELECT * FROM [ClientList] where Client_DPA_=" & Session("Client_DPA_")
	'Response.write(sqlStr)
	'Response.end
        ...
        If Not (rs.EOF Or rs.BOF) Then
			    ...
			    NameClient=rs.Fields("Client_DPA_") & " " & Mid(ClientName,1,20)
					    %>                    
                        <option SearchCode = "<%=rs.Fields("Client_DPA_")%>" SearchText = "<%=rs.Fields("ClientName")%>" value = '<%=rs.Fields("Client_DPA_")%>'><%=NameClient%></option>
                        <%rs.MoveNext
                Loop
        End If
		%>
