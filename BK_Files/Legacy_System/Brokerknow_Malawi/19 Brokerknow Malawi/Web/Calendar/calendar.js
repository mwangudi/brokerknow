...
img_Del.src="btn_del_small_gif";
img_Close.src="btn_close_small_gif";
...
var focusHack;
var retunStrOnWriteOn = new Boolean();
var useMaxYearChoice = 0;
var useMinYearChoice = 0;

retunStrOnWriteOn = false;
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
	...
	if (useMinYearChoice == 0) this.minYearChoice = dteToday.getFullYear()-100;	
	else this.minYearChoice = useMinYearChoice;

	if (useMaxYearChoice == 0) this.maxYearChoice = dteToday.getFullYear()+10;	
	else this.maxYearChoice = useMaxYearChoice;

	

	this.textBox= function() {
		if (!blnInConstructor) {	
			return eval('document.all.item(\''+this.formName+'\').'+this.textBoxName);
		}
	}
	...
	this.getSelectedDate = function () {
		...
		if ((typeof this.formName !='undefined') && (typeof this.textBoxName!='undefined')) {		
			objEle=eval('document.all.item(\''+this.formName+'\').'+this.textBoxName);
			if (objEle && !blnInConstructor) {
				strTempVal=eval('document.all.item(\''+this.formName+'\').'+this.textBoxName+'.value');
			}
			else {
	...
	function setSelectedDate(strWhat) {
		var strTempVal=''; var objEle;
		eval('document.all.item(\''+this.formName+'\').'+this.textBoxName).value=strWhat;
		
		if (!calMgr.isDate(quote(strWhat),quote(this.dateFormat))) {
			eval('document.all.item(\''+this.formName+'\').'+this.textBoxName).className = "cal-TextBoxInvalid";
		}
		else {
			eval('document.all.item(\''+this.formName+'\').'+this.textBoxName).className = "cal-TextBox";
		}
	}
	...
	function disable() {
		...
		this.enabled=false;
		eval('document.all.item(\''+this.formName +'\').' + this.textBoxName).disabled=true;
        eval('document.all.item(\''+this.formName+'\').'+this.textBoxName).className = "cal-TextBoxDisabled";
		if (scNN) {
			eval('document.all.item(\''+this.formName+'\').'+this.textBoxName).onFocus= function() {this.blur();};
		}       
	}
	...
	function enable() {
		...
		eval('document.all.item(\''+this.formName+'\').'+this.textBoxName).disabled=false;
        eval('document.all.item(\''+this.formName+'\').'+this.textBoxName).className = "cal-TextBox";
		if (scNN) {
			eval('document.all.item(\''+this.formName+'\').'+this.textBoxName).onFocus= null;
		}
	...
	function makeCalendar(intWhatMonth,intWhatYear,bViewOnly) {
		...
		if (!(bViewOnly)) {
			strOutput += '<td class="cal-HeadCell" align="center" width="100%"><SPAN OnClick="javascript:'+this.varName+'.clearDay();"><img name="calbtn1" src="'+strDefBtnImgPath+'btn_del_small.gif" border="0" width="12" height="10"></SPAN>&nbsp;&nbsp;<SPAN OnClick="javascript:'+this.varName+'.scrollMonth(-1);" class="cal-DayLink">&lt;</SPAN>&nbsp;<SELECT class="cal-ComboBox" NAME="cboMonth" onChange="'+this.varName+'.changeMonth();">';
			...
			strOutput += '<\/SELECT>&nbsp;<SPAN OnClick="javascript:'+this.varName+'.scrollMonth(1);" class="cal-DayLink">&gt;</SPAN>&nbsp;&nbsp;<SPAN OnClick="javascript:'+this.varName+'.hide();"><img name="calbtn2" src="'+strDefBtnImgPath+'btn_close_small.gif" border="0" width="12" height="10"></SPAN><\/td><\/tr><tr><td width="100%" align="center">';
		}
		else {
	...
	function writeControl() {
		...
		if (this.readonly) {
			strTemp=' onKeyPress="event.returnValue = false;" readonly ';
		}

		if (!(this.useDateRange)) { 
			strTemp+=' onChange="calMgr.validateDate(document.all.item(\''+this.formName+'\').'+this.textBoxName+','+this.varName+'.required);" onBlur="calMgr.formatDate(document.all.item(\''+this.formName+'\').'+this.textBoxName+','+this.varName+'.dateFormat);" ';
		}
		else {
			...
			strTemp+=' onChange="calMgr.validateDate('+'document.all.item(\''+this.formName+'\').'+this.textBoxName+','+this.varName+'.required,'+quote(strTempMinDate)+','+quote(strTempMaxDate)+');" onBlur="calMgr.formatDate(document.all.item(\''+this.formName+'\').'+this.textBoxName+','+this.varName+'.dateFormat);" ';
		}

		strHold='<input class="cal-TextBox" type="text" name="' + this.textBoxName + '"' + strTemp + 'size="12" value="' + this.getSelectedDate() + '">';
		if (!scIE) {
			strTemp=' OnClick="javascript:calClick();return false;" ';
		}
		else {
			strTemp='';
		}
		strHold+='<SPAN class="so-BtnLink"'+strTemp;
...
//		strHold+=this.varName+'.show();return false;">';
		strHold+=this.varName+'.show();window.event.cancelBubble=true;">';

		strHold+='<img align="absmiddle" border="0" name="' + this.btnName + '" src="' + strBtnImg +'" width="'+ strBtnW +'" height="'+ strBtnH +'"></SPAN>';
		if (retunStrOnWriteOn==true) return (strHold);
		else document.write(strHold);
	}
	this.writeControl=writeControl;
	
	function returnOutStringOnWrite() {
		retunStrOnWriteOn = true;
		
	}


	this.returnOutStringOnWrite = returnOutStringOnWrite;
...
// put these next 2 lines in when the tiny cal btns seem to randomly disappear		
			...
			//new code to align the left position of the calendar to the left pos of the calendar input
			var theInput = document.all.item(this.textBoxName);
			var xWidth = theInput.offsetWidth;
	
			// get correct position of date btn
			if ( scIE ) {
				if (this.displayLeft) {
					scX = (getOffsetLeft(document.images[this.btnName])-192+ document.images[this.btnName].width) - xWidth  ;    
				}
				else {
					scX = getOffsetLeft(document.images[this.btnName]) - xWidth ;    
				}
				if (this.displayTop) {
	...
	//
	function clearDay() {
		eval('document.all.item(\''+this.formName+'\').'+this.textBoxName + '.value = \'\'');
		this.hide();
		...
		if (!(this.useDateRange)) {
			if (blnIsGreyDate) {
				...
				if (isDayToday(intLinkDay)) {
					if (!(bViewOnly)) {
						templink='<td align="center" class="cal-DayCell">' + '<SPAN class="cal-TodayLink" onmouseover="self.status=\' \';return true" OnClick="javascript:'+this.varName+'.changeDay(' + intLinkDay + ');">' + intLinkDay + '<\/SPAN><\/td>';
					}
					else {
					...
					if (!(bViewOnly)) {
						templink='<td align="center" class="cal-DayCell">' + '<SPAN class="cal-DayLink" onmouseover="self.status=\' \';return true" OnClick="javascript:'+this.varName+'.changeDay(' + intLinkDay + ');">' + intLinkDay + '<\/SPAN>' +'<\/td>';
					}
					else {
