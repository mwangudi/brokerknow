...
If delPhoto = "1" Then
	...
	If delPhotoPath <> "" Then
		...
		Response.Redirect "Webupload.asp"
		Response.End
	...
	function deleteImage(){
		try{
			...
			if (currServPhoto == default_Img){
				...
				//document.forms[0].action = 'Webupload.asp';    
...
<body class="Dialog" leftMargin=0 topMargin=0 marginheight="0" marginwidth="0" OnLoad="JavaScript: drawPicture();">
	<form method="post" enctype="multipart/form-data"  action="WebsavePhoto.asp?filetext=<%=filetext%>" name="frmFile">
	<input type=hidden name=delPhoto value=0>
	<input type=hidden name=delPhotoPath value=0>
	<input type=hidden name="textPath" value='<%=filetext%>'>
