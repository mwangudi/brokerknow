...
if action = "CALCULATE" then 
  ...
  elseif buttonAction = "SAVE" then
	...
	if CounterParty ="" then CounterParty=0
	...
	SqlStr =" Update BondProposals set ProposalDate='"& cdate(ProposalDate) &"',"&_
			" SettlementDate ='"& cdate(SettlementDate) & "'," &_ 
			" ForwardRate="& ForwardRate &","&_
			" Consideration="& Consideration &"," &_
			" BondCleanPrice=" & BondCleanPrice1 & "," &_ 
			" AccruedInterest=" & AccruedInterest1 & "," &_ 
			" BondDirtyPrice=" & BondDirtyPrice1 & ", "&_
			" Validity='"& cdate(ValidityDate) &"', " &_
			" CouponPeriodDays="& DaysInCouponPeriod &"," &_ 
			" Basis= "& Basis &" ," &_ 
			" RemainingDaysToCoupon= "& DaysToNextCoupon & " ,"&_
			" PreviousCouponPayments= "& NoCouponPaymentsLeft & "," &_ 
			" ChangeInYield=" & ChangeInYield & "," &_ 
			" FaceValue= " & ccur(FaceValue) & ","&_
			" Duration=" & Duration1 & "," &_ 
			" Convexity = " & Convexity2 & ","&_
			" Commission=" & Commission & ","&_
			" AlternatePrice=" & AlternatePrice1 & ","&_
			" NetAmount= "& NetAmount1 &"," &_
			" CounterParty=" & CounterParty & "," &_
			" Salutation =" & "'" & Salutation & "'" & "," &_
			" ModifiedBy=" & session("UserID") & " ,"&_
			" DateModified='" & now() & "'"&_ 
			" where Proposal_DPA_=" & clng(ProposalNo)
...
<table border="0" width="100%" bordercolor="#000000" cellspacing="0" cellpadding="0">
  <tr>
    ...
    if action = "CALCULATE" then 
     if isnumeric(BondCleanPrice1)  then Response.Write formatnumber(replace(formatnumber(((BondCleanPrice1)),4),",",""),4) else Response.Write BondCleanPrice1 
    else
...
<table border="1" width="100%" cellspacing="0" cellpadding="0" bordercolor="#000000">
  ...
  <tr>
	...
	sqlStr =" SELECT dbo.Client.*, dbo.Commission.BondCommission AS CommissionRate, " &_
			" dbo.Owner.OwnerFName + ' ' + dbo.Owner.OwnerLName AS Owner FROM dbo.Client INNER JOIN " &_
			" dbo.Commission ON dbo.Client.Commission_DPA_ = dbo.Commission.Commission_DPA_ " &_
			" LEFT OUTER JOIN dbo.Owner ON dbo.Client.Owner_DPA_ = dbo.Owner.Owner_DPA_"
 ...
 <tr>
  ...
  <tr>
    <td>&nbsp;Counter Party</td>
    <td>
		<%if CounterParty=0 then CounterParty2="" else CounterParty2=CounterParty%>
       <input type = 'text' name ='txtCounterParty' id = 'txtCounterParty' size="10" onBlur="txtval = this.value; selectItem(cboCounterParty);" value="<%=CounterParty2%>">
