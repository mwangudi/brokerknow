...
'======================= Begin_Alter_Across_Entities =================================
		...
		const DataSource = "TurnOverDetails"
		const ActionPage = "MarketTurnOverList"
		const DataEntity = "MarketTurnover"
		const DataEntityPlural = "MarketTurnovers"
...
'======================= End_Alter_Across_Entities =================================		
		...
		sqlStr = "SELECT top 100 * FROM [" & DataSource & "] ORDER BY Tradedate DESC"
...
'======================= Begin_Alter_Across_Entities =================================
			'row ID 
			rowData = rowData & quote & RTrim(LTrim(rs.Fields("Turnover_DPA_"))) & quote & " : " 
			
			'row data 
			rowData = rowData & "[" 
			rowData = rowData & quote & rs.Fields("Turnover_DPA_") & quote & ","
			rowData = rowData & quote & formatdate(rs.Fields("TradeDate")) & quote & "," 			
			rowData = rowData & quote & formatnumber(rs.Fields("MarketTurnOver")) & quote & ","
			rowData = rowData & quote & rs.Fields("CreatedBy") & quote  & ","
			rowData = rowData & quote & FormatDateTime(rs.Fields("TimeCreated"),vbshorttime) & "  " & FormatDate(rs.Fields("TimeCreated")) & quote & ","
			...
			rowIDs = rowIDs & quote & RTrim(LTrim(rs.Fields("Turnover_DPA_"))) & quote 
			rowCount = rowCount + 1
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = [ "#", "Date","Market Turnover","Created By","Time Created","Last Modified By","Time Modified"];
		    </script>
<%'======================= End_Alter_Across_Entities =================================%>			
		<script>
			//data
...
/////==========================  End_CHanged   05 Apr 2004 =============================================================///////////////////////////////
		...
		<script> 
			...
			grid.getLayoutTemplate().action("adjustSize");
				...
				<input type = 'hidden' name ='ViewPage' id = "ViewPage" value = "<%=ActionFolder%>/View<%=DataEntity%>.asp">
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="TradeDate:Date*1;MarketTurnover:Market Turnover*0">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="TradeDate:Date;MarketTurnover:Market Turnover">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="TradeDate:Date*1;MarketTurnover:Market Turnover*0">
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:13em;width:20em">
