...
%>
<form name = 'frmEditSecurity' method = 'post' id="frmMain" action = 'EditOffering.asp' >
<table border="0" width="100%">
  <tr><%
		...
		sqlStr = "SELECT Immobilised,SecurityAddr,SecurityCode,SecurityMktPrice,SecurityName, ImportCode" & _
                "       ,OrderSecType_DPA_,Security_DPA_,Sector_DPA_,ClosingDate,CanTrade,BankAccount_DPA_ FROM [Security] WHERE Security_DPA_  = " & ID
  ...
  <td width="30%"><select name = 'cboSector' id = 'cboSector' size="1">
	...
	<%
		...
		rsEdit.CursorLocation=adUseClient
        ...
        If Not (rsEdit.EOF Or rsEdit.BOF) Then
                rsEdit.MoveFirst
                Do Until rsEdit.EOF
				if rsEdit.Fields("Sector_DPA_") = rs.Fields("Sector_DPA_") Then%>
                			<option selected value = '<%=rsEdit.Fields("Sector_DPA_")%>'><%=rsEdit.Fields("ShortDescription")%></option>
                		<%else%>                   						
			   				<option value = '<%=rsEdit.Fields("Sector_DPA_")%>'><%=rsEdit.Fields("ShortDescription")%></option>
             <%  end if         
                    rsEdit.MoveNext
                Loop
        End If
  ...
  <tr>
	...
	<td width="30%">
		...
		Set rsEdit = conn.Execute(SQLServerFormat(HandleQuote(sqlStr)))
			...
			rsEdit.MoveFirst
				Do Until rsEdit.EOF
					if(rsEdit("Account_DPA_")=rs("BankAccount_DPA_")) then					
					%>
					<option selected value = '<%=rsEdit.Fields("Account_DPA_")%>'><%=Mid(rsEdit.Fields("AccountName"),1,30)%></option>
					<%
					else
					%>
					<option value = '<%=rsEdit.Fields("Account_DPA_")%>'><%=Mid(rsEdit.Fields("AccountName"),1,30)%></option>		
					<%
					end if
				rsEdit.MoveNext
				Loop
  ...
  <td width="30%">Import Code</td>
    <td width="70%"><input type = 'text' name ='txtImportCode' id = 'txtImportCode' value = '<%=rs.Fields("ImportCode")%>' size="30"></td>
  </tr>
  <tr>
    <td>Can Trade</td>
    <td>
		...
		%>
			<input type=checkbox Class="BorderLess"   <%=chkState%> name='chkCanTrade' onClick = 'UpdateCanTrade(this);'> 
			<input type = 'hidden' name ='txtCanTrade' id = 'txtCanTrade' value='<%=applicable%>'>
      </td>
  </tr>  
  <tr>
    <td>Immobilised</td>
  ...
  <%
  if(isnull(rs("ClosingDate"))) then
  ClosingDate=Date
  else
	ClosingDate = CDate(rs("ClosingDate"))
  end if
  %>
  <tr>
  ...
  <td width="70%">
		<SCRIPT language="JavaScript">			
			var cal1=new ctlSpiffyCalendarBox("cal1", "frmEditSecurity", "txtADate1","cmdDate1","<%= FormatDate(ClosingDate) %>",1);
			cal1.writeControl();
		</SCRIPT>
