   ...
   Dim rsEdit
	...
	case "EXECUTE_HEADER"
       ...
       secType =  3
        ...
        'save data
        sqlStr = "UPDATE [SecurityOffering] SET SecurityAddr = " & "'" & Addr & "'" & ",SecurityCode = " & "'" & Code & "'" & "" & _
...
<table border="0" width="100%">
  <tr><%
		...
		sqlStr = "SELECT Immobilised,CanTrade,SecurityAddr,SecurityCode,SecurityMktPrice,SecurityName" & _
                "       ,OrderSecType_DPA_,Security_DPA_ FROM [SecurityOffering] WHERE Security_DPA_  = " & ID
        
        sqlStr = SQLServerFormat(HandleQuote(sqlStr))
        Set rs = conn.Execute(sqlStr)
        If rs.EOF Or rs.BOF Then%>
                <script language = 'vbscript'>
                		ShowMessage "The selected Offering cannot be retrieved for editing"
