...
'action = "EXECUTE"              
	...
	if action = "EXECUTE" then
		...
		if buttonAction = "CANCEL" then
				...
				'save data
				sqlStr = "UPDATE [Client] SET ClientAddr = " & "'" & handlequote(addr) & "'" & ",ClientBDate = " & "'" & FormatDate(bDate) & "'" & "" & _
						",ClientCellTel = " & "'" & cell & "'" & ",ClientContact = " & "'" & handlequote(contact) & "'" & "" & _
						",ClientEmail = " & "'" & handleQuote(email) & "'" & ",ClientFax = " & "'" & fax & "'" & "" & _
						",GenericSetting_DPA_ = " & " " & gen1 & " " & _
						",GenericSetting_DPA_2 = " & " " & gen2 & " " & "" & _
						",GenericSetting_DPA_3 = " & " " & gen3 & " " & "" & _
						",ClientPAddr = " & "'" & handlequote(pAddr) & "'" & "" & _
						",ClientComment = " & "'" & comment & "'" & "" & _
						",ClientHomeTel = " & "'" & handlequote(home )& "'" & ",ClientIDPass = " & "'" & idPass & "'" & "" & _
						",ClientName = " & "'" & handlequote(name) & "'" & ",ClientOfficeTel = " & "'" & Office & "'" & "" & _
				...
				conn.BeginTrans
					...
					if (not(bankAccExists2)) and (bankAccSpecified2) then
						'new account
						sqlStr = "INSERT INTO BankAcc (BankAccNumber,BankAccName,BnkBranch,Client_DPA_,BankNo,Bank_DPA_)" & _
						" VALUES('" & accNum2 & "', '" & accName2 & "', '" & bnkBranch2 & "', " & ID & ", 2,"& bnk2 &")"
					...
					elseif (bankAccExists2) and (bankAccSpecified2) then
						'update account
						sqlStr = "UPDATE [BankAcc] SET BankAccNumber = " & "'" & accNum2 & "'" & _
							",BankAccName = " & "'" & accName2 & "'" & _
							",BnkBranch = " & "'" & bnkBranch2 & "'" & _
							",Bank_DPA_ = " & " " & bnk2 & " " & _
					...
					if (not(bankAccExists3)) and (bankAccSpecified3) then
						'new account
						sqlStr = "INSERT INTO BankAcc (BankAccNumber,BankAccName,BnkBranch,Client_DPA_,BankNo,Bank_DPA_)" & _
						" VALUES('" & accNum3 & "', '" & accName3 & "', '" & bnkBranch3 & "', " & ID & ", 3,"& bnk3 &")"
					...
					elseif (bankAccExists3) and (bankAccSpecified3) then
						'update account
						sqlStr = "UPDATE [BankAcc] SET BankAccNumber = " & "'" & accNum3 & "'" & _
							",BankAccName = " & "'" & accName3 & "'" & _
							",BnkBranch = " & "'" & bnkBranch3 & "'" & _
							",Bank_DPA_ = " & " " & bnk3 & " " & _
...
<table border="0" width="60%" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<td><input type="text" name="txtCDSNumber" id="txtCDSNumber" value="<%=rs.Fields("ClientCDSNo")%>"></td>
	</tr>    
	<tr>
			...
			<tr>
				<td>			
					...
					<table border=0 style="border: 1px ridge #000080" WIDTH="100%">	
						...
						<tr>
							<td width="30%">Branch</td>
							<td width="70%"><input type = 'text' name ='txtBnkBranch2' id = 'txtBnkBranch2' size="20" tabIndex='24' value="<%=BnkBranch1%>"></td>
						</tr>
			...
			<tr>
				<td>				
					...
					<table border=0 style="border: 1px ridge #000080" WIDTH="100%">	
						...
						<tr>
							<td width="30%">Branch</td>
							<td width="70%"><input type = 'text' name ='txtBnkBranch3' id = 'txtBnkBranch3' size="20" tabIndex='24' value="<%=BnkBranch1%>"></td>
						</tr>
