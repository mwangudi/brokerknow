...
'======================= Begin_Alter_Across_Entities =================================
		...
		const DataSource = "OrderHoldOptions"
		const DataEntity = "OrderHoldOptions"
		const ActionPage = "OrderHoldList"
		const DataEntityPlural = "Order Hold Options"
...
'======================= Begin_Alter_Across_Entities =================================
			if CLng(rs.Fields("RequiresDate")) = 1 then
				RequiresDate = "Yes"
			else
				RequiresDate = "No"
			end if
			...
			'row ID 
			rowData = rowData & quote & rs.Fields("OrderHoldOptionID") & quote & " : " 
			
			'row data 
			rowData = rowData & "[" 
			rowData = rowData & quote & rs.Fields("Description") & quote & ","
			rowData = rowData & quote & RequiresDate & quote & ","
			rowData = rowData & quote & rs.Fields("DefaultSelection") & quote & ","
			rowData = rowData & "]" 
			'build the row IDs array 
			rowIDs = rowIDs & quote & rs.Fields("OrderHoldOptionID") & quote 
			rowCount = rowCount + 1
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			//column titles 
			var colCount = 3;
			var colNames = ["Description", "", ""];
			
			var myColumns = ["Description", "Requires Date", "Default Selection"];
		</script>
<%'======================= End_Alter_Across_Entities =================================%>			
		<script>
			//data
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="Description:Description*0;RequiresDate:Requires Date*2;DefaultSelection: Default Selection*0">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="Description:Description;RequiresDate:Requires Date;DefaultSelection: Default Selection">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="Description:Description*0;RequiresDate:Requires Date*2;DefaultSelection: Default Selection*0">
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:12em;width:18em">
