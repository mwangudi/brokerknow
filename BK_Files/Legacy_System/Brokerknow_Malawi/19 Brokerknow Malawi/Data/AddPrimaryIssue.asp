	...
	if action = "EXECUTE" then
			...
			'save data
		     ...
		     Set conn = GetActiveConnection("KBroker")
				 ...
				 sqlStr = "INSERT INTO [PrimaryIssues] ( PrimaryIssues_DPA_, Client_DPA_, Bond_DPA_, " & _
									"ValueDate, FaceValue, Price, ApplicationNo, AcceptedAmount, " & _
									"PaymentTypes_DPA_, PaymentDate, Reference, PaidAmount, Narrative)" &_
									" SELECT " & " " & "iif(isnull(max([PrimaryIssues_DPA_])),(SELECT InitialID FROM _Initial_Table_ID_ WHERE TableName = 'PrimaryIssues'),max([PrimaryIssues_DPA_]) + 1)" & " " & " as PrimaryIssues_DPA_" & _
                                    "," & " " & client & " " & " as Client_DPA_" & _
									"," & " " & BondIssue & " " & " as Bond_DPA_" & _
									"," & "#" & FormatDate(ValueDate) & "#" & " as ValueDate" & _
									"," & " " & Ccur(FaceValue) & " " & " as FaceValue" & _
									"," & " " & Ccur(Price) & " " & " as Price" & _
									"," & "'" & AppNo & "'" & " as ApplicationNo" & _
									"," & " " & Ccur(AccAmount) & " " & " as AcceptedAmount" & _
									"," & "'" & PaymentType & "'" & " as PaymentTypes_DPA_" & _
									"," & "#" & FormatDate(PaymentDate) & "#" & " as PaymentDate" & _
									"," & "'" & Reference & "'" & " as Reference" & _
									"," & " " & Ccur(PaidAmount) & " " & " as PaidAmount" & _
									"," & "'" & Narrative & "'" & " as Narrative" & _
									" FROM [PrimaryIssues]"
...
<script language='javascript'>
	...
	function UpdateAmount(){
	 ...
	 frm.PAmount.value = formatNum(Price*AcceptedAmount/100)
	}
...
<SCRIPT language="JavaScript">
	var cal=new ctlSpiffyCalendarBox("cal", "frm<%=DataSource%>", "txtValueDate","cmdValueDate","<%=FormatDate(Date)%>",1);
	var cal1=new ctlSpiffyCalendarBox("cal1", "frm<%=DataSource%>", "txtPDate","cmdPDate","<%=FormatDate(Date)%>",1);
</SCRIPT>

<form name = 'frm<%=DataSource%>' method = 'post' action = 'AddPrimaryIssue.asp' id = "frm<%=DataSource%>" >
 ...
 <TR>
  ...
  <TR>
   ...
   <TR>
    ...
    <TD width="80%"><input type ="text" Name ="ApplicationNo"  id="ApplicationNo" size="20" ></TD></TR>
    <tr>
    <td width="15%">Code</td>
...
%>
  ...
  <TR>
    <TD width="20%">Bond Type</TD>
    <TD width="80%"><select name = 'cboBondType' id = 'cboBondType' size="1" onChange='FetchIssues(this);'>
  ...
  <TR>
    <TD width="20%">Bond Issue</TD>
    <TD width="80%"><select Name="cboBondIssue" id="cboBondIssue">
    </select>
    </td>
  </tr>
  <TR>
    <TD width="20%">Face Value</TD>
    <TD width="80%"><input type ="text" Name ="FaceValue"  id="FaceValue" size="20" onChange="Javascript: FormatNumber2(this);"></TD></TR>
  <TR>
    <TD width="20%">Price</TD>
    <TD width="80%"><input type ="text" Name ="Price"  id="Price" size="20" value="0.0000" onChange="Javascript: UpdateAmount(); FormatNumber(this,4);"></TD></TR>
  ...
  <TR>
    ...
    <TD width="80%"><input type ="text" Name ="AcceptedAmount"  id="AcceptedAmount" size="20" value="0.00" onChange="Javascript: UpdateAmount(); FormatNumber2(this)"></TD></TR>
  <TR>
    <TD ></TD>
    <TD></TD></TR>
    <TR>
  ...
  <TR>
    ...
    <%
        ...
        If Not (rs.EOF Or rs.BOF) Then
                rs.MoveFirst
                Do Until rs.EOF
		         if rs.Fields("PaymentTypes_DPA_") = 4 then ' Default = Cheque				
				 %><option value = '<%=rs.Fields("PaymentTypes_DPA_")%>' selected><%=rs.Fields("Description")%></option><%  
				 else%>
...
%>
  ...
  <TR>
    ...
    <TD width="20%">Date of Payment</TD>
    <TD width="80%"><SCRIPT language="JavaScript">cal1.writeControl();</SCRIPT></TD></TR>
  <TR>
    <TD width="20%">Reference</TD>
    <TD width="80%"><textarea name="Reference" id ="Reference"></textarea></TD></TR>
  <TR>
    <TD width="20%">Amount</TD>
    <TD width="80%"><input type ="text" Name ="PAmount"  class="readonly" id="PAmount" size="20" readonly></TD></TR>
  <TR>
    <TD width="20%">Narrative</TD>
    <TD width="80%"><textarea name="Narrative" id ="Narrative"></textarea></TD></TR>
  <TR>
    <TD></TD>
    <TD></TD></TR>
  <tr>
