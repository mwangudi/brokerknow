	...
	if action = "EXECUTE" then
			...
			'save data
				Set conn = GetActiveConnection("KBroker")
				 ...
				 sqlStr = "INSERT INTO [ForwardRate] (ForwardRate_DPA_, ForwardRate, ActivationDate, Bond_DPA_) " & _
									"SELECT " & " " & "iif(isnull(max([ForwardRate_DPA_])),(SELECT InitialID FROM _Initial_Table_ID_ WHERE TableName = 'ForwardRate'),max([ForwardRate_DPA_]) + 1)" & " " & " as ForwardRate_DPA_" & _
                                    "," & " " & ForwardRate & " " & " as ForwardRate" & _
									"," & "#" & FormatDate(ActivationDate) & "#" & " as ActivationDate" & _
									"," & " " & ID & " " & " as Bond_DPA_" & _
									" FROM [ForwardRate]"
...
<form name = 'frm<%=DataSource%>' method = 'post' action = 'AddForwardRate.asp' id = "frm<%=DataSource%>" >
<%
Set conn = GetActiveConnection("KBroker")
...
<table border="0" width="100%" cellspacing="1" cellpadding="1">
 ...
 <tr>
  ...
  <tr>
    ...
    <td width="54%"><input type="text" name="ForwardRate" id="ForwardRate" value="" size="20"></td>
  </tr>
  <tr>
