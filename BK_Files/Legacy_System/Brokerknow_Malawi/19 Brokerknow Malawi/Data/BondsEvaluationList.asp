...
'======================= Begin_Alter_Across_Entities =================================
		...
		const ActionPage = "BondsEvaluationList"
...
'======================= End_Alter_Across_Entities =================================		
	    ...
	    if filterStr = "" AND searchStr = "" AND sortQryStr = "" AND InStr(1, sqlStr, " ORDER BY ") =< 0 then
	    'Apply default sorting
	     sqlStr = sqlStr & " ORDER BY ClientName ASC, TradeDate DESC, Type ASC, Reference ASC " 
	    end if
		...
		<LINK REL="STYLESHEET" Rate="TEXT/CSS" HREF="../STYLE/default.css">
			<LINK REL="STYLESHEET" Rate="TEXT/CSS" HREF="../STYLE/webparts.css">
				...
				<style> 
					...
					.active-column-4 {width: 60px; text-align: left;}					
					...
					.active-column-13 {width: 60px; text-align: left;}
					.active-column-14 {width: 60px; text-align: left;}	
...
'======================= Begin_Alter_Across_Entities =================================
		    ...
		    FRate = iif(isnull(rs.Fields("ForwardRate")),0,rs.Fields("ForwardRate"))
		    
			'row ID 
			rowData = rowData & quote & rs.Fields("Evaluation_DPA_") & quote & " : " 
			
			'row data 
			rowData = rowData & "[" 
			rowData = rowData & quote & Include & quote & "," 
			...
			rowData = rowData & quote & FormatNumber(rs.Fields("CouponRate"),2) & quote & ","
			rowData = rowData & quote & FormatNumber(rs.Fields("FaceValue"),2) & quote & ","
			rowData = rowData & quote & FRate & quote & ","
			rowData = rowData & quote & FormatNumber(rs.Fields("BPrice"),4) & quote & ","
			rowData = rowData & quote & rs.Fields("DaysInCoupon") & quote & "," 
			rowData = rowData & quote & rs.Fields("Basis") & quote & ","
			rowData = rowData & quote & rs.Fields("User") & quote & ","
			rowData = rowData & quote & ModifiedDate & quote  			
			rowData = rowData & "]" 
			'build the row IDs array 
			rowIDs = rowIDs & quote & rs.Fields("Evaluation_DPA_") & quote 
			rowCount = rowCount + 1
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = ["Included", "Type", "Reference", "Purchase Date", "Code", "Client", "Bond Issue", "Issue Date", "Coupon Rate", "Face Value", "Rate", "Price", "Days In Coupon", "Basis", "Last Modified By", "Date Modified"];
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="Included: Included*2;Type: Type*0;Reference: Reference*0;TradeDate: Purchase Date*1;Client_DPA_: Code*2;ClientName: Client*0;BondIssue: Bond Issue*0;IssueDate: Issue Date*1;CouponRate: Coupon Rate*2;FaceValue: Face Value*2;ForwardRate: Rate*2;BPrice: Price*2;DaysInCoupon: DaysInCoupon*2;Basis: Basis*2">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="Included: Included;Type: Type;Reference: Reference;TradeDate: Purchase Date;Client_DPA_: Code;ClientName: Client;BondIssue: Bond Issue;IssueDate: Issue Date;CouponRate: Coupon Rate;FaceValue: Face Value;ForwardRate: Rate;BPrice: Price;DaysInCoupon: DaysInCoupon;Basis: Basis">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="Included: Included*2;Type: Type*0;Reference: Reference*0;TradeDate: Purchase Date*1;Client_DPA_: Code*2;ClientName: Client*0;BondIssue: Bond Issue*0;IssueDate: Issue Date*1;CouponRate: Coupon Rate*2;FaceValue: Face Value*2;ForwardRate: Rate*2;BPrice: Price*2;DaysInCoupon: DaysInCoupon*2;Basis: Basis*2">
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:30em;width:30em">
