...
<%
	...
	if action = "EXECUTE" then
		...
		if buttonAction = "SAVE" then
				...
				CDSNumber = trim(Request.Form("txtCDSNumber"))
				...
				'Check valid 
				dim numericBit, alphaBit
				numericBit = left(Trim(CDSNumber),len(Trim(CDSNumber))-2)
				alphaBit = right(Trim(CDSNumber),2)

				response.write  alphaBit & vbcrlf
				if not  IsNumeric(numericBit) then
					
				end  if
				response.write numericBit
				...
				UniqueSql =	"SELECT     RTRIM(LTRIM(ClientCDSNo)) AS CDS " & _
						...
						if not(rsUnique2.EOF and rsUnique2.BOF) then
								if(trim(cdaID) & Trim(CDSNo)=Trim(rsUnique2("CDS")) ) then%>
					
								   <script language = 'vbscript'>
									  ShowMessage "CDS Number Must be Unique"						
								   </script>
								   <% 
								   set rsUnique = nothing
								   Set conn=nothing
				...
				sqlStr = "INSERT INTO [Client] (ClientAddr,ClientBDate,ClientCellTel,ClientContact,ClientEmail" & _
						...
						",ClientOpeningBal, Owner_DPA_,Residency_DPA_,Client_EIT_, CreditLimit, ClientCDSNo, IsCustodian,updateOnDebt,updateOnContract,UseContactNameInPortfolioReports,ChangedBY) SELECT " & "'" & addr & "'" & " as ClientAddr," & "#" & FormatDate(bDate) & "#" & " as ClientBDate" & _
				...
				conn.BeginTrans
					...
					if bankAccSpecified then
						...
						" VALUES('" & accNum & "', '" & accName & "', '" & bnkBranch & "', " & rs.Fields("Client_DPA_") & ", 1,"& bnk &")"
					...
					if bankAccSpecified2 then
						'save detail data
						sqlStr = "INSERT INTO BankAcc (BankAccNumber,BankAccName,BnkBranch,Client_DPA_, BankNo,Bank_DPA_)" & _
						" VALUES('" & accNum2 & "', '" & accName2 & "', '" & bnkBranch2 & "', " & rs.Fields("Client_DPA_") & ", 2,"& bnk2 &")"
					...
					if bankAccSpecified3 then
						'save detail data
						sqlStr = "INSERT INTO BankAcc (BankAccNumber,BankAccName,BnkBranch,Client_DPA_, BankNo,Bank_DPA_)" & _
						" VALUES('" & accNum3 & "', '" & accName3 & "', '" & bnkBranch3 & "', " & rs.Fields("Client_DPA_") & ", 3,"& bnk3 &")"
...
<form name = 'frmAddClient' method = 'post' action = 'AddClienttest.asp' id = "frmMain">
...
<table align=right>
	<tr>
	  ...
	  <td width="100%" align="right" valign=absBottom >
		<input type = 'submit' Class=Buttons name ='cmdAdd' id = 'cmdAdd' value=" Save " tabIndex='35' onclick = "AllowedNavigation()">
