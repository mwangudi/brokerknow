...
<script language="javascript">
		...
		function  UpdateCSDStatus(theChk)
		{
			if (theChk.checked)
			{
				document.frmMain.elements("chkCDSstatus").value = "1";
			}
			else
			{
				document.frmMain.elements("chkCDSstatus").value = "0";
			}					
		}
		...
		{
			setOpener();
			document.frmEditSecurity.method='post';
			document.frmEditSecurity.target='_self';
			document.frmEditSecurity.submit();		
		}
...
<%
   ...
   Dim rsEdit
	...
	case "EXECUTE_HEADER"
       ...
       secType =  2
        ...
        If Len(Code) > 50 Then%>
				<script language="vbscript">
								ShowMessage "Code cannot be more than 50 characters length"
	...
	sqlStr = "SELECT Immobilised,SecurityAddr,SecurityCode,SecurityMktPrice,SecurityName, ImportCode" & _
			"       ,OrderSecType_DPA_,Security_DPA_,Sector_DPA_, ExpiryDate FROM [Security] WHERE Security_DPA_  = " & ID
   ...
   if trim(rs.fields("ExpiryDate")) <> "" then
	ExpiryDate = formatdate(trim(rs.fields("ExpiryDate")))
   end if
       
%>
<SCRIPT language="JavaScript">
	var cal=new ctlSpiffyCalendarBox("cal", "frmEditSecurity", "txtADate","cmdDate","<%= FormatDate(Date) %>",1);
	var cal1=new ctlSpiffyCalendarBox("cal1", "frmEditSecurity", "txtEDate","cmdEDate","<%=ExpiryDate%>",1);
</SCRIPT>
<form name = 'frmEditSecurity' method = 'post' id="frmMain" action = 'EditSecurity.asp' >
<table border="0" width="100%">
  ...
  <td width="30%">Expiry Date</td>
    <td width="70%">
	<SCRIPT language="JavaScript">cal1.writeControl();</SCRIPT></td>
  </tr><br><br>
  <tr>
