...
'======================= Begin_Alter_Across_Entities =================================
		...
		const DataSource = "PaymentTypesList"
		const DataEntity = "PaymentTypes"
		const DataEntityPlural = "Payment Types"
...
'======================= End_Alter_Across_Entities =================================		
		...
		sqlStrOrig = "SELECT PaymentTypes_DPA_, Description, (CASE WHEN reference = 1 THEN 'Yes' "
		sqlStrOrig = sqlStrOrig & "ELSE 'No' END) AS Reference FROM [" & DataSource & "]" 
		
		'sqlStr = "SELECT * FROM [" & DataSource & "]"
		sqlStr = "SELECT PaymentTypes_DPA_, Description, (CASE WHEN reference = 1 THEN 'Yes' "
		sqlStr = sqlStr & "ELSE 'No' END) AS Reference FROM [" & DataSource & "]" 
		
		If filterStr <> "" Then
			'sqlStr = "SELECT * FROM [" & DataSource & "] WHERE " & filterStr
			sqlStr = "SELECT PaymentTypes_DPA_, Description, (CASE WHEN reference = 1 THEN 'Yes' "
		    sqlStr = sqlStr & "ELSE 'No' END) AS Reference FROM [" & DataSource & "] WHERE " & filterStr 
		End If
...
'======================= Begin_Alter_Across_Entities =================================
			'row ID 
			rowData = rowData & quote & rs.Fields("PaymentTypes_DPA_") & quote & " : " 
			
			'row data 
			rowData = rowData & "[" 
			rowData = rowData & quote & rs.Fields("Description") & quote & "," 
			rowData = rowData & quote & rs.Fields("Reference") & quote 
			rowData = rowData & "]" 
			'build the row IDs array 
			rowIDs = rowIDs & quote & rs.Fields("PaymentTypes_DPA_") & quote 
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns =  ["Short Description", "Requires Reference"];
		</script>
<%'======================= End_Alter_Across_Entities =================================%>			
		<script>
			//data
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="Description:Description*0;Reference:Reference*0">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="Description:Description*0;Reference:Reference*0">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="Description:Description*0;Reference:Reference*0">
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:10;width:20em">
