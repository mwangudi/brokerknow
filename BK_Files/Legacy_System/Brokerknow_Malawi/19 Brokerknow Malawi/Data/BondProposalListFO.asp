...
'======================= Begin_Alter_Across_Entities =================================
		...
		const DataSource = "BondOrderList"
		const ActionPage = "OrderList"
		const DataEntity = "RegisterOrders"
		const DataEntityPlural = "RegisterOrders"
		...
		<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/default.css">
			<LINK REL="STYLESHEET" TYPE="TEXT/CSS" HREF="../STYLE/webparts.css">
				...
				<style> 
					...
					.active-column-0 {width:20px;}
					.active-column-1 {width: 60px;}
					.active-column-2 {width: 60px; text-align: left;}
					...
					.active-column-11 {width: 50px;}
					.active-column-12 {width: 200px;}	
					.active-column-13 {width: 30px; text-align: left;}			
					.active-column-14 {width: 50px; text-align: left;}	
					.active-column-15 {width: 50px; }			
...
'======================= Begin_Alter_Across_Entities =================================
			...
			if not isnull(rs.Fields("OrderType_DPA_")) or rs.Fields("OrderType_DPA_")<>"" then
			sale= IIf((rs.Fields("OrderType_DPA_")) = 1, "Purchase", "Sale")
			end if
			if isdate(rs.Fields("TimeChanged")) then
			ModifiedDate = FormatDateTime(rs.Fields("TimeChanged"),vbshorttime) & "  " & FormatDate(rs.Fields("TimeChanged"))
		else
			ModifiedDate=""
		end if
			'row data 
			...
			rowData = rowData & quote & sale & quote & "," 
			...
			rowData = rowData & quote & Best & quote & ","
				if(rs("Amount")<>0) then
					if(rs("Price")<>0) then
					rowData = rowData & quote & FormatNumCommasOnly(rs.Fields("Amount")/(rs("Price")*1.105)) & quote & ","			 
					else
					rowData = rowData & quote & FormatNumCommasOnly(rs.Fields("Amount")/(1.105)) & quote & ","			 
					end if
				else
...
'======================= Begin_Alter_Across_Entities =================================%> 
		<script>
			...
			var myColumns = ["No","Proposal Dt.", "Order Dt.",  "Type","Order No", "Security", "Price", "Quantity","Amount",  
			"Reference","Code","CDS No", "Client","Hold","Inter Bank","Balance", "Last Modified By","Time Modified"];
		    </script>
<%'======================= End_Alter_Across_Entities =================================%>			
		<script>
			//data
...
<%'======================= Begin_Alter_Across_Entities =================================%>
				...
				<input type = 'hidden' name ='FilterArgs' id="FilterArgs" value="OrderDate:Date*3;OrderTypeName:Type*0;Order_DPA_:Order No*0;OrdDetailSecurity:Security*0;OrdDetailPrice:Price*0;OrdDetailQty:Quantity*2;OrderRef:Reference*0;Client_DPA_:Client Code*0;ClientName:Client*0;OrderHold:Hold*5;OrderReleasedBy:Released By*5;OrderDateReleased:Date Released*5;ClientCDSNo:CDS No*0">
				<input type = 'hidden' name ='SortArgs' id="SortArgs" value="OrderDate:Date;OrderTypeName:Type;Order_DPA_:Order No;OrdDetailSecurity:Security;OrdDetailPrice:Price;OrdDetailQty:Quantity;OrderRef:Reference;Client_DPA_:Client Code;ClientName:Client;OrderHold:Hold">
				<input type = 'hidden' name ='SearchArgs' id="SearchArgs" value="OrderDate:Date*3;OrderTypeName:Type*0;Order_DPA_:Order No*0;OrdDetailSecurity:Security*0;OrdDetailPrice:Price*0;OrdDetailQty:Quantity*2;OrderRef:Reference*0;Client_DPA_:Client Code*0;ClientName:Client*0;OrderHold:Hold*5;OrderReleasedBy:Released By*5;OrderDateReleased:Date Released*5;ClientCDSNo:CDS No*0">
				<input type = 'hidden' name ='dialogLayout' id="dialogLayout" value="height:40em;width:50em">
