...
Else
	...
	If Contracts = 1 Then	
		...
		sqlStr= "SELECT * FROM ShowSMSAllContractsCompounded WHERE (ClientCellTel IS Not NULL) AND (ClientCellTel <> N'') AND (updateOnContract=1)"
		'sqlStr= "SELECT * FROM ShowSMSAllContractsCompounded WHERE (ClientCellTel IS Not NULL) AND (ClientCellTel <> N'')"
		...
		Else
						...
						'	cNo=0
							...
							If cint(cnNo)= cint(0) Then
								If rs("OrderType_DPA_")=1 Then
									smsStr =smsStr & cellNo &"|"& formatnumber(rs("LotQty"),0) & " " & rs("SecurityCode")& " SHARES PURCHASED ON " & formatdate(rs("lotTdate")) & " @ KES " & formatnumber(rs("LotPrice"),2) &" PER SHARE. SETTLEMENT OF KES " & formatnumber(rs("contractAmount"),2) & " DUE " & formatdate(rs("contractSettlementDate"))& ". " & vbCrLf
								Else
									smsStr =smsStr & cellNo  &"|"& formatnumber(rs("LotQty"),0) & " " & rs("SecurityCode")& " SHARES SOLD ON "& formatdate(rs("lotTdate")) &" @ KES " & formatnumber(rs("LotPrice"),2) & " PER SHARE. " & vbCrLf
								End If
							End If
						'End If
					End If
			...
			HeaderText ="api-id:1062866" & vbCrLf
			HeaderText =HeaderText  & "user:jthiore" & vbCrLf 
			HeaderText =HeaderText  & "password:jthiore1" & vbCrLf 
			HeaderText =HeaderText  & "text: #field1#" & vbCrLf
			HeaderText =HeaderText  & "Delimiter:|" & vbCrLf
			...
			Response.Write "<br>&nbsp;<br><b>Contracts List</b><br><table><tr><td nowrap>"
			Response.Write replace(HeaderText & "" & smsStr, vbCrLf ,"</td></tr></table><br>")
		End If
	End If
